...
foreach ($object->fields as $key => $val) {
	...
	// Si le champ a trbreak n'est pas null, on ajouter une tr vide avec bordure
    if (!empty($val['trbreak'])) {
		if($val['trbreak'] == 'top') $class_border = 'trbreaktop';
		elseif($val['trbreak'] == 'bottom') $class_border = 'trbreakbottom';
		elseif($val['trbreak'] == 'top_bottom') $class_border = 'trbreaktopbottom';
		print '<tr class="'.$class_border.'" style="'.$css_border.'"></tr>';
    }
    ...
    if (!empty($val['separation']) && $val['separation'] == 1) {
        ...
        $detail_open && $detail_open != $langs->trans($val['label_separation']) ? print '<details close>' : print '<details open>';
