...
if (empty($reshook) && !empty($object->table_element) && isset($extrafields->attributes[$object->table_element]['label']) && is_array($extrafields->attributes[$object->table_element]['label'])) {
	...
	foreach ($extrafields->attributes[$object->table_element]['label'] as $tmpkeyextra => $tmplabelextra) {
		...
		if ($extrafields->attributes[$object->table_element]['type'][$tmpkeyextra] == 'separate') {
			...
			if (strtolower($tmpkeyextra) == 'rubrique') { // Assurez-vous de vérifier avec la casse correcte
				$customLabel = 'Rubrique';
			} elseif (strtolower($tmpkeyextra) == 'impact') {
				$customLabel = 'Impact';
			} else {
				$customLabel = $langs->trans($tmplabelextra);
			}

			// Affichage avec ou sans aide
			if (!empty($extrafields->attributes[$object->table_element]['help'][$tmpkeyextra])) {
				...
				print $form->textwithpicto(
					$customLabel,
					$langs->trans($tmptooltip[0]),
					1,
					'help',
					'',
					0,
					3,
					(empty($tmptooltip[1]) ? '' : 'extra_'.$tmpkeyextra.'_'.$tmptooltip[1])
				);
			} else {
				// Affichage simple du label
				print $customLabel;
			}
