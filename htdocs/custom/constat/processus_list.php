...
/**
 *   	\file       processus_list.php
 *		\ingroup    constat
 *		\brief      List page for processus
 */
...
require_once __DIR__.'/class/processus.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('constat', 'processus', 'write');
	$permissiontodelete = $user->hasRight('constat', 'processus', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Processus';
	$objectlabel = 'Processus';
...
$title = $langs->trans("Processuss");
//$help_url = "EN:Module_Processus|FR:Module_Processus_FR|ES:Módulo_Processus";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/constat/processus_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/constat/processus_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendProcessusRef";
$modelmail = "processus";
$objecttmp = new Processus($db);
