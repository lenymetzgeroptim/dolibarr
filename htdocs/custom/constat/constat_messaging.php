...
//if (empty($conf->constat->enabled)) accessforbidden();
...
print dol_get_fiche_head($head, 'agenda', $langs->trans("Constat"), -1, $object->picto);
...
if (!empty($object->id)) {
	...
	$messagingUrl = DOL_URL_ROOT.'/custom/constat/constat_messaging.php?id='.$object->id;
	...
	print_barre_liste($langs->trans("ActionsOnConstat"), 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlcenter, '', 0, 1, 0);
	...
	show_actions_messaging_custom($conf, $langs, $db, $object, null, 0, $actioncode, '', $filters, $sortfield, $sortorder);
}
