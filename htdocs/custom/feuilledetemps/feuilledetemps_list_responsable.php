...
$help_url = '';
$title = $langs->trans('ListOfValidate', $langs->transnoentitiesnoconv("FeuillesDeTemps"));
$morejs = array();
$morecss = array();
...
$i = 0;
while($user_static->fk_user != 16) {
	$user_static->fetch($user_static->fk_user);
	if($i >= 3){
		break;
	}
	$i++;
}*/
// $child = $user->getAllChildIds(0);

// if(!empty($child)){
// 	$sql .= " OR (t.status =".FeuilleDeTemps::STATUS_DRAFT;
// 	$sql .= " AND tv.fk_user_validation IS NULL";
// 	$sql .= " AND t.fk_user IN (".implode($child)."))";
// }
$sql .= ")";
