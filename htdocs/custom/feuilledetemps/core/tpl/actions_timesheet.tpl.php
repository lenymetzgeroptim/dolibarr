...
if ($action == 'addtime' && GETPOST('formfilteraction') != 'listafterchangingselectedfields' && $massaction != 'validate1' && $massaction != 'validate2' && $massaction != 'verification' && $massaction != 'refus') {
	...
	foreach ($timetoadd as $taskid => $value) {     // Loop on each task
		...
		//for ($idw = 0; $idw < $nb_jour; $idw++) {	
			...
			if ($task->timespent_id > 0) {
				...
				if($heure_sup->heure_sup_25_duration != $newduration_hs_25 || $heure_sup->heure_sup_50_duration != $newduration_hs_50) {
					// Agenda HS 25
					...
					elseif ($res > 0 && ($heure_sup->heure_sup_25_duration != $newduration_hs_25 || $heure_sup->heure_sup_50_duration != $newduration_hs_50)){
						if($is_day_anticipe) {
							if($heure_sup->heure_sup_25_duration != $newduration_hs_25) {
								$task->timespent_note = (!empty($task->timespent_note) ? $task->timespent_note.' / ' : '');
			...
			if ($task->timespent_id > 0) {	
				...
				if($heure_other->heure_nuit != $newduration_heure_nuit || $heure_other->port_epi != $newduration_epi) {
					// Agenda Heure nuit
					...
					if($res > 0 && ($newduration_heure_nuit == 0 && $newduration_epi == 0)){
						if($is_day_anticipe) {
					...
					else if($res == 0 && ($newduration_heure_nuit != 0 || $newduration_epi != 0)){
						if($is_day_anticipe) {
					...
					elseif ($res > 0 && ($heure_other->heure_nuit != $newduration_heure_nuit || $heure_other->port_epi != $newduration_epi)){
						if($is_day_anticipe) {
							if($heure_other->heure_nuit != $newduration_heure_nuit){
								$task->timespent_note = (!empty($task->timespent_note) ? $task->timespent_note.' / ' : '');
	...
	if($object->status != $object::STATUS_VALIDATED && $object->status != $object::STATUS_EXPORTED) {
		for ($idw = 0; $idw < $nb_jour; $idw++) {
			...
			if($deplacement_ponctuel[$idw] != $deplacement->deplacement_ponctuel || $type_deplacement[$idw] != $deplacement->type_deplacement || $moyen_transport[$idw] != $deplacement->moyen_transport || $notes_deplacement[$idw] != $deplacement->note) {
				...
				elseif($res == 0 && (!empty($deplacement_ponctuel[$idw]) || $moyen_transport[$idw] != 0 || $type_deplacement[$idw] != 0)) {
					if($is_day_anticipe) {
						...
						$deplacement->note = $notes_deplacement[$idw];
					}
...
if ($action == 'addtimeVerification' && GETPOST('formfilteraction') != 'listafterchangingselectedfields' && $massaction != 'validate1' && $massaction != 'validate2' && $massaction != 'verification' && $massaction != 'refus') {
	...
	for ($idw = 0; $idw < $nb_jour; $idw++) {
		...
		if(/*$newduration_heure_sup00 != $silae->heure_sup00 || $newduration_heure_sup25 != $silae->heure_sup25 || $newduration_heure_sup50 != $silae->heure_sup50 || */$newduration_heure_nuit != $silae->heure_nuit || $newduration_heure_route != $silae->heure_route || $repas[$idw] != $silae->repas || $kilometres[$idw] != $silae->kilometre || $indemnite_tt[$idw] != $silae->indemnite_tt) {
			...
			(empty($kilometres[$idw]) || $kilometres[$idw] == 0) && (empty($indemnite_tt[$idw]) || $indemnite_tt[$idw] == 0)) {
		...
		if(!empty($_POST['task']) && dol_print_date($dayinloopfromfirstdaytoshow, '%a') == 'Dim') {
			$silae = new Silae($db);
	...
	|| $newdurationHeureRoute != $regul->heure_route) {
		...
		if((int)$regulD1 != (int)$regul->d1) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul D1</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if((int)$regulD2 != (int)$regul->d2) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul D2</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if((int)$regulD3 != (int)$regul->d3) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul D3</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if((int)$regulD4 != (int)$regul->d4) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul D4</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if((int)$regulGD1 != (int)$regul->gd1) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul GD1</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if((int)$regulGD2 != (int)$regul->gd2) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul GD2</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if((int)$regulRepas1 != (int)$regul->repas1) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Repas 1</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if((int)$regulRepas2 != (int)$regul->repas2) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Repas 2</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if($regulKilometres != $regul->kilometres) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Kilomètres</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if((int)$regulIndemniteTT != (int)$regul->indemnite_tt) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Indemnite de TT</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if($newdurationHeureNuit50 != $regul->heure_nuit_50) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Heure Nuit 50%</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if($newdurationHeureNuit75 != $regul->heure_nuit_75) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Heure Nuit 75%</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if($newdurationHeureNuit100 != $regul->heure_nuit_100) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Heure Nuit 100%</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if($newdurationHeureRoute != $regul->heure_route) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Heure Route</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if($regulHeureSup00 != $regul->heure_sup00) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Heure Sup 0%</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if($regulHeureSup25 != $regul->heure_sup25) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Heure Sup 25%</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
		...
		if($regulHeureSup50 != $regul->heure_sup50) {
			...
			$modification_silae .= ($old_value != $new_value ? '<li><strong>Regul Heure Sup 50%</strong> ('.dol_print_date($tmpday, '%d/%m/%Y').") : $old_value ➔ $new_value</li>" : '');
		}
...
if ($action == 'confirm_transmettre' && $confirm == 'yes' && $object->id > 0){
	...
	if(!$userIsRA){
		...
		foreach($listtask as $taskid){
			...
			$task->fetch($taskid);
			$liste_resp_tache = $task->liste_contact(-1, 'internal', 1, 'TASKEXECUTIVE');
			if($liste_resp_tache[0] > 0){
				$user_static = new User($db);
				$user_static->fetch($liste_resp_tache[0]);
				if($user_static->statut == 1 && !in_array($user_static->id, $list_validation1)){
					$object->createTaskValidation($user_static->id, 1, 1);
					$list_validation1[] = $user_static->id;
				}
			}
			...
			$liste_resp_projet = $projet->liste_contact(-1, 'internal', 1, 'PROJECTLEADER');
			if($liste_resp_projet[0] > 0){
				$user_static = new User($db);
				$user_static->fetch($liste_resp_projet[0]);
				if($user_static->statut == 1 && !in_array($user_static->id, $list_validation2)){
					$object->createTaskValidation($user_static->id, 1, 2);
					$list_validation2[] = $user_static->id;
				}
			}
		}
	}
