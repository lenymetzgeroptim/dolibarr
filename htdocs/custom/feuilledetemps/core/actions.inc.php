...
if ($action == 'confirm_validate1' && $confirm == 'yes' && $userIsResp && $resp_pas_valide) {
...
if ($action == 'confirm_validate2' && $confirm == 'yes' && $userIsRespProjet) {
...
if ($action == 'confirm_sendMail' && $confirm == 'yes' && $permissionToVerification) {
	...
	global $dolibarr_main_url_root;
	$subject = '[OPTIM Industries] Notification Feuille de temps';
...
if ($action == 'removeFavoris' && $user->id == $usertoprocess->id) {
	if (!($object->id > 0)) {
		dol_print_error('', 'Error, object must be fetched');
		exit;
	}
	...
	if ($result > 0) {
		// Delete OK
		setEventMessages("FavorisDeleted", null, 'mesgs');
...
if ($action == 'addFavoris' && $user->id == $usertoprocess->id) {
	...
	if ($result > 0) {
		// Delete OK
		setEventMessages("FavorisCreated", null, 'mesgs');
