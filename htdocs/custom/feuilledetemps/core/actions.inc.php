...
if ($action == 'confirm_validate1' && $confirm == 'yes' && !$conf->global->FDT_RESP_TASKPROJECT_APPROVER && $permissiontovalidate1) {
	$modification = '<ul>';
	...
	for ($idw = 0; $idw < $nb_jour; $idw++) { 
		...
		if(dol_print_date($dayinloopfromfirstdaytoshow, '%a') == 'Dim') {
			...
			if($conf->global->HEURE_SUP_SUPERIOR_HEURE_MAX_SEMAINE && $heure_sup50ht_before != $silae_tmpday->heure_sup50ht) {
	...
	if($result && !$conf->global->FDT_DISPLAY_COLUMN) {
		...
		if($regulHeureSup00 != $regul->heure_sup00 || $regulHeureSup25 != $regul->heure_sup25 || $regulHeureSup50 != $regul->heure_sup50 || ($regulHeureSup50HT != $regul->heure_sup50ht && $conf->global->HEURE_SUP_SUPERIOR_HEURE_MAX_SEMAINE)) {
			// Agenda Heure Sup 0%
...
elseif ($action == 'confirm_validate1' && $confirm == 'yes' && $conf->global->FDT_RESP_TASKPROJECT_APPROVER && $permissiontovalidate1) {
	$modification = '<ul>';
	$regulHeureSup00 = ($regul->heure_sup00 != 0 ? (double)$regul->heure_sup00 : 0);
	$regulHeureSup25 = ($regul->heure_sup25 != 0 ? (double)$regul->heure_sup25 : 0);
	$regulHeureSup50 = ($regul->heure_sup50 != 0 ? (double)$regul->heure_sup50 : 0);
	...
	elseif(!in_array(0, $list_valideur[1]) && (empty($list_valideur2[0]) || (sizeof($list_valideur2[0]) == 1 && in_array($user->id, $list_valideur2[0])))){
		...
		if(empty($conf->global->FDT_MANAGE_EMPLOYER) || in_array($usertoprocess->array_options['options_fk_employeur'], explode(",", $conf->global->FDT_MANAGE_EMPLOYER))){
...
if ($action == 'confirm_validate2' && !$conf->global->FDT_RESP_TASKPROJECT_APPROVER && $confirm == 'yes' && $permissiontovalidate2) {
	$modification = '<ul>';
	$regulHeureSup00 = ($regul->heure_sup00 != 0 ? (double)$regul->heure_sup00 : 0);
	$regulHeureSup25 = ($regul->heure_sup25 != 0 ? (double)$regul->heure_sup25 : 0);
	$regulHeureSup50 = ($regul->heure_sup50 != 0 ? (double)$regul->heure_sup50 : 0);
...
elseif ($action == 'confirm_validate2' && $conf->global->FDT_RESP_TASKPROJECT_APPROVER && $confirm == 'yes' && $permissiontovalidate2) {
	$modification = '<ul>';
	$regulHeureSup00 = ($regul->heure_sup00 != 0 ? (double)$regul->heure_sup00 : 0);
	$regulHeureSup25 = ($regul->heure_sup25 != 0 ? (double)$regul->heure_sup25 : 0);
	$regulHeureSup50 = ($regul->heure_sup50 != 0 ? (double)$regul->heure_sup50 : 0);
...
if ($action == 'update' && !empty($permissiontoadd)) {
	...
	if (!$error && $conf->global->FDT_RESP_TASKPROJECT_APPROVER) {
