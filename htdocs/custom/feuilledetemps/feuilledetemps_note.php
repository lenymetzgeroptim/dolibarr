...
/**
 *  \file       feuilledetemps_note.php
 *  \ingroup    feuilledetemps
 *  \brief      Tab for notes on FeuilleDeTemps
 */
...
$langs->loadLangs(array("feuilledetemps@feuilledetemps", "companies"));
...
$hookmanager->initHooks(array('feuilledetempsnote', 'globalcard')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $userIsResp || $userIsRespProjet || $user->admin || (empty($list_resp_task[0]) && empty($list_resp_projet[0]) && $user->id == $object->fk_user); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
...
$help_url = '';
llxHeader('', $langs->trans('FeuilleDeTemps'), $help_url);
