...
/**
 *  \file       htdocs/modulebuilder/template/feuilledetemps_agenda.php
 *  \ingroup    feuilledetemps
 *  \brief      Tab of events on feuilledetemps
 */
...
dol_include_once('/feuilledetemps/class/deplacement.class.php');
...
dol_include_once('/feuilledetemps/class/projet_task_time_heure_sup.class.php');
dol_include_once('/feuilledetemps/class/projet_task_time_other.class.php');
dol_include_once('/feuilledetemps/class/regul.class.php');
dol_include_once('/feuilledetemps/class/silae.class.php');
...
$hookmanager->initHooks(array('feuilledetempsagenda', 'globalcard')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $user->rights->feuilledetemps->feuilledetemps->read; // Used by the include of actions_addupdatedelete.inc.php
...
if ($object->id > 0) {
	...
	print dol_get_fiche_head($head, 'agenda', $langs->trans("feuilledetemps"), -1, $object->picto);
	...
	$messagingUrl = DOL_URL_ROOT.'/custom/feuilledetemps/feuilledetemps_messaging.php?id='.$object->id;
	...
	$messagingUrl = DOL_URL_ROOT.'/custom/feuilledetemps/feuilledetemps_agenda.php?id='.$object->id;
	...
	if (isModEnabled('agenda') && ($user->hasRight('agenda', 'myactions', 'read') || $user->hasRight('agenda', 'allactions', 'read'))) {
		...
		$cachekey = 'count_events_feuilledetemps_'.$object->id;
		$nbEvent = dol_getcache($cachekey);

		print_barre_liste($langs->trans("ActionsOnfeuilledetemps").(is_numeric($nbEvent) ? '<span class="opacitymedium colorblack paddingleft">('.$nbEvent.')</span>': ''), 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlright, '', 0, 1, 1);
		*/
		print_barre_liste($langs->trans("ActionsOnFeuilleDeTemps"), 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlcenter, '', 0, 1, 0);
		...
		if(!$permissionToVerification) {
			$exclude_actioncode = array('\'AC_FDT_VERIF\'');
		}

		// TODO Replace this with same code than into list.php
		show_actions_done($conf, $langs, $db, $object, null, 0, $actioncode, '', $filters, $sortfield, $sortorder, property_exists($object, 'module') ? $object->module : '', $exclude_actioncode);
	}
}
