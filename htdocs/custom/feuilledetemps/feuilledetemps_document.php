...
/**
 *  \file       feuilledetemps_document.php
 *  \ingroup    feuilledetemps
 *  \brief      Tab for documents linked to FeuilleDeTemps
 */
...
$langs->loadLangs(array("feuilledetemps@feuilledetemps", "companies", "other", "mails"));
...
$hookmanager->initHooks(array('feuilledetempsdocument', 'globalcard')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $permissiontoread; // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
...
$title = $langs->trans("FeuilleDeTemps").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id) {
	...
	$modulepart = 'feuilledetemps';
	//$permissiontoadd = $user->rights->feuilledetemps->feuilledetemps->write;
	$permissiontoadd = 1;
	//$permtoedit = $user->rights->feuilledetemps->feuilledetemps->write;
	$permtoedit = 1;
	$param = '&id='.$object->id;

	//$relativepathwithnofile='feuilledetemps/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile = 'feuilledetemps/'.dol_sanitizeFileName($object->ref).'/';
