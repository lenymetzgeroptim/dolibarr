...
include DOL_DOCUMENT_ROOT.'/custom/feuilledetemps/lib/generic.lib.php';
include DOL_DOCUMENT_ROOT.'/custom/feuilledetemps/lib/timesheet.lib.php';
...
if(!$user->rights->feuilledetemps->feuilledetemps->facturertemps){
	accessforbidden();
}
...
//steps
    switch($step) {
        case 2:{
           $fields = ($mode == 'user')?'fk_user':(($mode == 'taskUser')?'fk_user, fk_element':'fk_element');
            ...
            $Form .= '<tr class="liste_titre"><th colspan="8" class="center"><strong>'.$langs->trans('invoicedServiceSelectoin')."</strong></th></tr>";
            $Form .= '<tr class="liste_titre"><th class="center">'.$langs->trans("User").'</th>';
            $Form .= '<th class="center">'.$langs->trans("Task").'</th><th class="center">'.$langs->trans("Service").' ('.$langs->trans("Existing")."/".$langs->trans("Custom").')</th>';
            $Form .= '<th class="center">'.$langs->trans("Custom").' : '.$langs->trans("Description").'</th><th class="center">'.$langs->trans("Custom").' : '.$langs->trans("UnitPriceHT").'</th>';
            $Form .= '<th class="center">'.$langs->trans("Custom").' : '.$langs->trans("VAT").'</th><th class="center">'.$langs->trans("unitDuration").'</th><th class="center">'.$langs->trans("savedDuration").'</th>';
            $form = new Form($db); 
            ...
            $Form .= '</table>';
            if (empty($resArray)){
                $Form .= '<div class="center" style="color: red">Aucun Résultat</div>';
            }
            else {
                $Form .= '<br><div class="center"><input type = "submit"  class = "butAction" value = "'.$langs->trans('Next')."\"></div>\n</form>";
            } 
            break;}
        ...
        default:
            ...
            $Form .= '<tr class = "liste_titre" width = "100%" ><th class="center" colspan = "2"><strong>'.$langs->trans('generalInvoiceProjectParam').'</strong></th></tr>';
    ...
    //all ts or only approved
           $ts2Invoice = $conf->global->TIMESHEET_INVOICE_TASKTIME;
            $Form .= '<tr class = "oddeven"><th align = "left" width = "80%">'.$langs->trans('TimesheetToInvoice').'</th><th align = "left">';
            //$Form .= '<input type = "radio" name = "ts2Invoice" value = "approved" ';
            //$Form .= ($ts2Invoice == "approved"?"checked":"").'> '.$langs->trans("approvedOnly").' <br>';
            $Form .= '<input type = "radio" name = "ts2Invoice" value = "all" ';
            $Form .= "checked".'> '.$langs->trans("All")."</th></tr>";
    // not alreqdy invoice
            /*if(version_compare(DOL_VERSION, "4.9.9")>=0) {
            ...
            $Form .= '</table>';
            $Form .= '<br><div class="center"><input type = "submit" onclick = "return checkEmptyFormFields(event,\'settings\',\'';
...
dol_fiche_head($headProject, 'invoice', $langs->trans("Project"), -1, 'project');
...
if (!empty($conf->global->PROJECT_USE_OPPORTUNITIES) || empty($conf->global->PROJECT_HIDE_TASKS) || !empty($conf->eventorganization->enabled)) {
    ...
    if (!empty($conf->global->PROJECT_USE_OPPORTUNITIES)) {
        print '<input type="checkbox" disabled name="usage_opportunity"'.(GETPOSTISSET('usage_opportunity') ? (GETPOST('usage_opportunity', 'alpha') != '' ? ' checked="checked"' : '') : ($project->usage_opportunity ? ' checked="checked"' : '')).'"> ';
        $htmltext = $langs->trans("ProjectFollowOpportunity");
        print $form->textwithpicto($langs->trans("ProjectFollowOpportunity"), $htmltext);
        print '<br>';
    }
    if (empty($conf->global->PROJECT_HIDE_TASKS)) {
        print '<input type="checkbox" disabled name="usage_task"'.(GETPOSTISSET('usage_task') ? (GETPOST('usage_task', 'alpha') != '' ? ' checked="checked"' : '') : ($project->usage_task ? ' checked="checked"' : '')).'"> ';
        $htmltext = $langs->trans("ProjectFollowTasks");
        print $form->textwithpicto($langs->trans("ProjectFollowTasks"), $htmltext);
        print '<br>';
    }
    if (empty($conf->global->PROJECT_HIDE_TASKS) && !empty($conf->global->PROJECT_BILL_TIME_SPENT)) {
        print '<input type="checkbox" disabled name="usage_bill_time"'.(GETPOSTISSET('usage_bill_time') ? (GETPOST('usage_bill_time', 'alpha') != '' ? ' checked="checked"' : '') : ($project->usage_bill_time ? ' checked="checked"' : '')).'"> ';
...
if ($conf->categorie->enabled) {
    ...
    print $form->showCategories($project->id, 'project', 1);
    print "</td></tr>";
}
...
function htmlPrintServiceChoice($user, $task, $class, $duration, $tasktimelist, $seller, $buyer)
{
    ...
    $html = '<tr class = "'.$class.'"><th class="center">'.$userName;
    $html .= '</th><th class="center">'.$taskHTML;
    ...
    $html .= select_sellist(array('table'=> 'product', 'keyfield'=> 'rowid', 'fields'=>'ref,label', 'where'=>' tosell = 1 AND fk_product_type = 1'),
                        ...
                        $addchoices);
    $html .= '</th>';
    $html .= '<th class="center" ><input type = "text"  size = "30" name = "userTask['.$user.']['.$task.'][Desc]" ></th>';
    $html .= '<th class="center" ><input type = "text"  size = "6" name = "userTask['.$user.']['.$task.'][PriceHT]" ></th>';
    //$html .= '<th><input type = "text" size = "6" name = "userTask['.$user.']['.$task.']["VAT"]" ></th>';
    $html .= '<th class="center" >'.$form->load_tva('userTask['.$user.']['.$task.'][VAT]', -1, $seller, $buyer, 0, 0, '', false, 1).'</th>';
    $html .= '<th class="center" ><input type = "text" size = "2" maxlength = "2" name = "userTask['.$user.']['.$task.'][unit_duration]" value = "1" >';
    ...
    $html .= '<th class="center"><input type = "text" size = "2" onkeypress="return regexEvent(this,event,\'timeChr\')" maxlength = "5" name = "userTask['.$user.']['.$task.'][duration]" value = "'.$duration.'" >';
    $html .= '</th</tr>';
    return $html;
}
/**
