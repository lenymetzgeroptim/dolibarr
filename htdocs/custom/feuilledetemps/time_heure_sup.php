...
$object = new ExtendedTask($db);
$projectstatic = new Project($db);
$extrafields = new ExtraFields($db);
$extrafields->fetch_name_optionals_label($projectstatic->table_element);
$extrafields->fetch_name_optionals_label($object->table_element);

// Vérifie si l'utilisateur actif est un Chef du projet
$userIsCDP = 0;
$projet = New Project($db);
$projet->fetch($projectid);
if ($projet->id > 0){
...
$permissiontoAdd = $userIsCDP || $user->admin || $user->rights->feuilledetemps->timespent->add;
$permissiontoReadValue = $userIsCDP || $user->admin || $user->rights->feuilledetemps->timespent->read_value;
...
if ($action == 'addtimespent' && $user->rights->projet->lire && $permissiontoAdd) {
	$error = 0;
	...
	$newduration_hs_25 = (GETPOSTINT('timespent_hs25_durationhour') ? GETPOSTINT('timespent_hs25_durationhour') : 0) * 60 * 60;
	$newduration_hs_25 += (GETPOSTINT('timespent_hs25_durationmin') ? GETPOSTINT('timespent_hs25_durationmin') : 0) * 60;
	$newduration_hs_50 = (GETPOSTINT('timespent_hs50_durationhour') ? GETPOSTINT('timespent_hs50_durationhour') : 0) * 60 * 60;
	$newduration_hs_50 += -(GETPOSTINT('timespent_hs50_durationmin') ? GETPOSTINT('timespent_hs50_durationmin') : 0) * 60;
	$new_duration = GETPOSTINT("timespent_durationhour") * 60 * 60; // We store duration in seconds
	$new_duration += (GETPOSTINT('timespent_durationmin') ? GETPOSTINT('timespent_durationmin') : 0) * 60; // We store duration in seconds
	if($new_duration < $newduration_hs_25 + $newduration_hs_50){
		setEventMessages('Les heures sup doivent être inferieur à la durée', null, 'errors');
		$error++;
	}
	...
	if (!$error) {
		...
		if (!$error) {
			...
			if (empty($object->project->statut)) {
				...
				$res = $object->fetchTimeSpentWithoutId($object->id, dol_mktime(0, 0, 0, GETPOST("timemonth", 'int'), GETPOST("timeday", 'int'), GETPOST("timeyear", 'int')), GETPOST("userid", 'int'));

				if($res > 0){
					setEventMessages("Impossible de créer un temps consommé car il en existe déja un pour le même utilisateur, le même jour, et la même tache", null, 'errors');
				}
				else{
					...
					$object->timespent_duration = $new_duration;
					...
					if($result >= 0){
						$hs = new Projet_task_time_heure_sup($db);
						$hs->heure_sup_25_duration = $newduration_hs_25;
						$hs->heure_sup_50_duration = $newduration_hs_50;
						$hs->fk_projet_task_time = $object->timespent_id;
						$hs->create($user);
					}
...
if (($action == 'updateline' || $action == 'updatesplitline') && !$cancel && $user->rights->projet->lire && $permissiontoAdd) {
	$error = 0;
	...
	$newduration_hs_25 = (GETPOSTINT('new_duration_hs25hour') ? GETPOSTINT('new_duration_hs25hour') : 0) * 60 * 60;
	$newduration_hs_25 += (GETPOSTINT('new_duration_hs25min') ? GETPOSTINT('new_duration_hs25min') : 0) * 60;
	$newduration_hs_50 = (GETPOSTINT('new_duration_hs50hour') ? GETPOSTINT('new_duration_hs50hour') : 0) * 60 * 60;
	$newduration_hs_50 += (GETPOSTINT('new_duration_hs50min') ? GETPOSTINT('new_duration_hs50min') : 0) * 60;
	$new_duration = GETPOSTINT("new_durationhour") * 60 * 60; // We store duration in seconds
	$new_duration += (GETPOSTINT("new_durationmin") ? GETPOSTINT('new_durationmin') : 0) * 60; // We store duration in seconds
	if($new_duration < $newduration_hs_25 + $newduration_hs_50){
		setEventMessages('Les heures sup doivent être inferieur à la durée', null, 'errors');
		$error++;
	}
	...
	if (!$error) {
		if (GETPOST('taskid', 'int') != $id) {		// GETPOST('taskid') is id of new task
			...
			if($result >= 0){
				$hs = new Projet_task_time_heure_sup($db);
				$res = $hs->getID($object->timespent_id);
				if($res > 0 && $newduration_hs_25 == 0 && $newduration_hs_50 == 0){
					$hs->fetch($res);
					$result2 = $hs->delete($user);
				}
				else if($res == 0 && ($newduration_hs_25 != 0 || $newduration_hs_50 != 0)){
					$hs->fk_projet_task_time = $object->timespent_id ;
					...
					$result2 = $hs->create($user);
				}
				elseif ($res > 0 && ($newduration_hs_25 != 0 || $newduration_hs_50 != 0)){
					$hs->fetch($res);
					if($hs->heure_sup_25_duration != $newduration_hs_25 || $hs->heure_sup_50_duration != $newduration_hs_50){
						if($newduration_hs_25 >= 0){
							$hs->heure_sup_25_duration = $newduration_hs_25;
						}
						if($newduration_hs_50 >= 0){
...
if ($action == 'confirm_delete' && $confirm == "yes" && $user->rights->projet->lire && $permissiontoAdd) {
	$object->fetchTimeSpent(GETPOST('lineid', 'int'));
	// TODO Check that ($task_time->fk_user == $user->id || in_array($task_time->fk_user, $childids))
	$result = $object->delTimeSpent($user);
...
if (($id > 0 || !empty($ref)) || $projectidforalltimes > 0) {
	...
	if ($projectstatic->id > 0) {
		if ($withproject) {
			// Tabs for project
			if (empty($id) || $tab == 'timespent_heure_sup') {
				$tab = 'timespent_heure_sup';
			} else {
				$tab = 'tasks';
			}
	...
	if (empty($projectidforalltimes)) {
		...
		print dol_get_fiche_head($head, 'task_time_heure_sup', $langs->trans("Task"), -1, 'projecttask', 0, '', 'reposition');
	...
	if ($projectstatic->id > 0) {
		...
		$arrayfields['hs_50'] = array('label'=>'HS à 50%', 'checked'=>1);
		$arrayfields['value'] = array('label'=>$langs->trans("Value"), 'checked'=>1, 'enabled'=>($permissiontoReadValue ? 1 : 0));
		$arrayfields['value_hs'] = array('label'=>$langs->trans("Value").' (HS)', 'checked'=>1, 'enabled'=>($permissiontoReadValue ? 1 : 0));
		...
		$sql .= " WHERE t.fk_user = u.rowid AND t.fk_element = pt.rowid";
		if (empty($projectidforalltimes)) {
		...
		if ($action == 'createtime' && $user->rights->projet->lire) {
			...
			// Duration - Heure sup 25%
			print '<td class="nowraponall">';
			$durationtouse_hs25 = ($_POST['timespent_hs25_duration'] ? $_POST['timespent_hs25_duration'] : '');
			if (GETPOSTISSET('timespent_hs25_durationhour') || GETPOSTISSET('timespent_hs25_durationmin')) {
				$durationtouse_hs25 = (GETPOST('timespent_hs25_durationhour') * 3600 + GETPOST('timespent_hs25_durationmin') * 60);
			}
			print $form->select_duration('timespent_hs25_duration', $durationtouse_hs25, 0, 'text');
			print '</td>';

			// Duration - Heure sup 50%
			print '<td class="nowraponall">';
			$durationtouse_hs50 = ($_POST['timespent_hs50_duration'] ? $_POST['timespent_hs50_duration'] : '');
			if (GETPOSTISSET('timespent_hs50_durationhour') || GETPOSTISSET('timespent_hs50_durationmin')) {
				$durationtouse_hs50 = (GETPOST('timespent_hs50_durationhour') * 3600 + GETPOST('timespent_hs50_durationmin') * 60);
			}
			print $form->select_duration('timespent_hs50_duration', $durationtouse_hs50, 0, 'text');
			print '</td>';
		...
		// Heure Sup 25%
		if (!empty($arrayfields['hs_25']['checked'])) {
			print '<td class="liste_titre right"></td>';
		}
		// Heure Sup 50%
		if (!empty($arrayfields['hs_50']['checked'])) {
			print '<td class="liste_titre right"></td>';
		}
		...
		// Value avec heures sup
		if (!empty($arrayfields['value_hs']['checked'])) {
			print '<td class="liste_titre"></td>';
		}
		// Value billed
		...
		*/
		...
		if (!empty($arrayfields['hs_25']['checked'])) {
			print_liste_field_titre($arrayfields['hs_25']['label'], $_SERVER['PHP_SELF'], 'hs.heure_sup_25_duration', '', $param, '', $sortfield, $sortorder, 'right ');
		}
		if (!empty($arrayfields['hs_50']['checked'])) {
			print_liste_field_titre($arrayfields['hs_50']['label'], $_SERVER['PHP_SELF'], 'hs.heure_sup_50_duration', '', $param, '', $sortfield, $sortorder, 'right ');
		}
		if (!empty($arrayfields['value']['checked'])) {
			print_liste_field_titre($arrayfields['value']['label'], $_SERVER['PHP_SELF'], '', '', $param, '', $sortfield, $sortorder, 'right ');
		}
		if (!empty($arrayfields['value_hs']['checked'])) {
			print_liste_field_titre($arrayfields['value_hs']['label'], $_SERVER['PHP_SELF'], '', '', $param, '', $sortfield, $sortorder, 'right ');
		}
		if (!empty($arrayfields['valuebilled']['checked'])) {
		...
		*/
		...
		foreach ($tasks as $task_time) {
			...
			// Time spent heure sup à 25%
			if (!empty($arrayfields['hs_25']['checked'])) {
				print '<td class="right nowraponall" style="color: rgb(0, 159, 255)">';
				if ($action == 'editline' && $_GET['lineid'] == $task_time->rowid) {
					print '<input type="hidden" name="old_duration_hs25" value="'.$task_time->hs25.'">';
					print $form->select_duration('new_duration_hs25', $task_time->hs25, 0, 'text');
				} else {
					if (!empty($task_time->hs25)){
						print convertSecondToTime($task_time->hs25, 'allhourmin');
					}				}
				print '</td>';
				if (!$i) {
					$totalarray['nbfield']++;
				...
				if (!$i) {
					$totalarray['pos'][$totalarray['nbfield']] = 'hs_25';
				}
				$totalarray['val']['hs_25'] += $task_time->hs25;
				if (!$i) {
					$totalarray['totalduration_hs25field'] = $totalarray['nbfield'];
				}
				$totalarray['totalduration_hs25'] += $task_time->hs25;
			}

			// Time spent heure sup à 50%
			if (!empty($arrayfields['hs_50']['checked'])) {
				print '<td class="right nowraponall" style="color: #d97a00">';
				if ($action == 'editline' && $_GET['lineid'] == $task_time->rowid) {
					print '<input type="hidden" name="old_duration_hs50" value="'.$task_time->hs50.'">';
					print $form->select_duration('new_duration_hs50', $task_time->hs50, 0, 'text');
				} else {
					if (!empty($task_time->hs50)){
						print convertSecondToTime($task_time->hs50, 'allhourmin');
					}
				}
				print '</td>';
				if (!$i) {
				...
				if (!$i) {
					$totalarray['pos'][$totalarray['nbfield']] = 'hs_50';
				}
				$totalarray['val']['hs_50'] += $task_time->hs50;
				if (!$i) {
					$totalarray['totalduration_hs50field'] = $totalarray['nbfield'];
				}
				$totalarray['totalduration_hs50'] += $task_time->hs50;
			}
			...
			// Value spent avec heures sup
			if (!empty($arrayfields['value_hs']['checked'])) {
				$langs->load("salaries");

				print '<td class="nowraponall right">';
				if(dol_print_date($date1, '%a') != 'Dim' && !num_public_holiday($date1 + 86400, $date1 + 172800, $mysoc->country_code, 0, 0, 0)){
					$value = price2num(($task_time->thm * ($task_time->element_duration - $task_time->hs25 - $task_time->hs50) / 3600) + ($task_time->thm * 1.25 * $task_time->hs25 / 3600) + ($task_time->thm * 1.50 * $task_time->hs50 / 3600), 'MT', 1);
				}
				else{
					$value = price2num(($task_time->thm * $task_time->element_duration * 2 / 3600), 'MT', 1);
				}
				print '<span class="amount" title="'.$langs->trans("THM").': '.price($task_time->thm).'">';
				print price($value, 1, $langs, 1, -1, -1, $conf->currency);
				print '</span>';
				...
				if (!$i) {
					$totalarray['pos'][$totalarray['nbfield']] = 'value_hs';
				}
				$totalarray['val']['value_hs'] += $value;
				if (!$i) {
					$totalarray['totalvalue_hsfield'] = $totalarray['nbfield'];
				}
				$totalarray['totalvalue_hs'] += $value;
			}
			...
			if (($action == 'editline' || $action == 'splitline') && $_GET['lineid'] == $task_time->rowid) {
				...
				if ($permissiontoAdd > 0) {
					if ($conf->MAIN_FEATURES_LEVEL >= 2) {
						print '&nbsp;';
						print '<a class="reposition" href="'.$_SERVER["PHP_SELF"].'?id='.$task_time->fk_task.'&amp;action=splitline&amp;lineid='.$task_time->rowid.$param.((empty($id) || $tab == 'timespent_heure_sup') ? '&tab=timespent_heure_sup' : '').'">';
						print img_split();
						print '</a>';
					}

					print '&nbsp;';
					print '<a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?id='.$task_time->fk_task.'&amp;action=editline&amp;lineid='.$task_time->rowid.$param.((empty($id) || $tab == 'timespent_heure_sup') ? '&tab=timespent_heure_sup' : '').'">';
					print img_edit();
					print '</a>';

					print '&nbsp;';
					print '<a class="reposition paddingleft" href="'.$_SERVER["PHP_SELF"].'?id='.$task_time->fk_task.'&amp;action=deleteline&amp;token='.newToken().'&amp;lineid='.$task_time->rowid.$param.((empty($id) || $tab == 'timespent_heure_sup') ? '&tab=timespent_heure_sup' : '').'">';
		...
		if (isset($totalarray['totaldurationfield']) || isset($totalarray['totalvaluefield']) || isset($totalarray['totalduration_hs25field']) || isset($totalarray['totalduration_hs50field']) || isset($totalarray['totalvalue_hsfield'])) {
			...
			while ($i < $totalarray['nbfield']) {
				...
				elseif ($totalarray['totalduration_hs25field'] == $i) {
					print '<td class="right">'.convertSecondToTime($totalarray['totalduration_hs25'], 'allhourmin').'</td>';
				}
				elseif ($totalarray['totalduration_hs50field'] == $i) {
					print '<td class="right">'.convertSecondToTime($totalarray['totalduration_hs50'], 'allhourmin').'</td>';
				}
				elseif ($totalarray['totalvalue_hsfield'] == $i) {
					print '<td class="right">'.price($totalarray['totalvalue_hs']).'</td>';
				} 
