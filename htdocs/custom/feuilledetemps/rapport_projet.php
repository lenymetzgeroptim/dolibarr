...
if (!$user->rights->feuilledetemps->reports->read) {
	accessforbidden();
}
...
if(!$user->admin) {
    ...
    $sql .= ' WHERE ((ctc.element in (\'project_task\') AND ctc.code LIKE \'%EXECUTIVE%\')OR (ctc.element in (\'project\') AND ctc.code LIKE \'%LEADER%\') OR (ctc.element in (\'project\') AND ctc.code LIKE \'%CONSULTANT%\')) AND ctc.active = \'1\'  ';
