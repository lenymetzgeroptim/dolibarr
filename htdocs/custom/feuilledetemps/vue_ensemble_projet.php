...
if (!$user->rights->feuilledetemps->feuilledetemps->vueensemble) {
	accessforbidden();
}
...
print dol_get_fiche_head($head, 'vue_ensemble_HS', $langs->trans("Project"), -1, ($object->public ? 'projectpub' : 'project'));
...
// Heures sup 
$task_extend = new ExtendedTask($db);
$res = $task_extend->getSumOfAmount_HS($id);
$montant_ht_hs = price2num(($res['amount_hs25'] + $res['amount_hs50']) * -1, 'MT');
$res = $task_extend->getDuration($id);
foreach($res as $ligne){
	if(dol_print_date($ligne['date'], '%a') == 'Dim' || num_public_holiday($ligne['date'] + 86400, $ligne['date'] + 172800, $mysoc->country_code, 0, 0, 0)){
		$montant_ht_hs += $ligne['duration'] / 3600 * (!empty($ligne['thm']) ? $ligne['thm'] : 0) * -1;
	}
}
$balance_ht += $montant_ht_hs;
$montant_ttc_hs = price2num($montant_ht_hs * (1 + ($defaultvat / 100)), 'MT');
$balance_ttc += $montant_ttc_hs;
print '<tr class="oddeven">';
print '<td class="left">HS Temps consommé</td>';
print '<td class="right"></td>';
print '<td class="right">'.price($montant_ht_hs).'</td>';
print '<td class="right">'.price($montant_ttc_hs).'</td></tr>';
