...
function select_sellist(
    ...
    if($ajaxNbChar && $ajax) {
        ...
        $select .= '<input type = "text" class = "maxwidth400 '.(isset($htmlarray['class'])?$htmlarray['class']:'').'" name = "'.$htmlName.'" id = "'.(isset($htmlarray['id'])?$htmlarray['id']:$htmlName).'" value = "'.$selectedValue.'"'.$htmlarray['otherparam'].' />';
    } else{
        $select .= '<select class = "flat maxwidth400 '.(isset($htmlarray['class'])?$htmlarray['class']:'').'" id = "'.(isset($htmlarray['id'])?$htmlarray['id']:$htmlName).'" name = "'.$htmlName.'"'.$nodatarole.' '.$htmlarray['otherparam'].'>';
