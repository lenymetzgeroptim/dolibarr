			...
			if (empty($mine) || str_contains($tasksrole[$lines[$i]->id], 'TASKCONTRIBUTOR')) {
				...
				if(!$noTimespentInTask || ($object->status != $object::STATUS_VALIDATED && $object->status!= $object::STATUS_VERIFICATION && $object->status != $object::STATUS_EXPORTED)) {
					...
					if (empty($oldprojectforbreak) || ($oldprojectforbreak != -1 && $oldprojectforbreak != $projectstatic->id)) {
						...
						//$object->test_pdf .= '<td style="" colspan="'.(2 + $nb_jour + 3).'">';
						...
						//$object->test_pdf .= $projectstatic->getNomUrl(1, '', 0, '<strong>'.$langs->transnoentitiesnoconv("YourRole").':</strong> '.$projectsrole[$lines[$i]->fk_project], ' - ', 0, -1, 'valignmiddle');
						...
						if ($projectstatic->array_options['options_tiers_secondaire'] > 0) {
							...
							//$object->test_pdf .= ' - '.$thirdpartystatic->getNomUrl(1);
						}
						if ($projectstatic->title) {
							print ' - ';
							print '<span class="secondary valignmiddle">'.$projectstatic->title.'</span>';
							//$object->test_pdf .= ' - ';
							//$object->test_pdf .= '<span class="secondary valignmiddle">'.$projectstatic->title.'</span>';
						}
						print '</div>';
						print '</td>';
						//$object->test_pdf .= '</td>';
					...
					for ($idw = 0; $idw < $nb_jour; $idw++) { // Gestion des cases de chaque jour
						...
						////$object->test_pdf .= $dayWorkLoad / 3600;
						...
						if($temps_cons_semaine > $tmp_heure_semaine_hs && dol_print_date($dayinloopfromfirstdaytoshow, '%a') != 'Dim' && !$disabledtask && $alreadyspent){
							...
							$tableCell .= '<input type="text"'.($temps_cons_semaine <= $conf->global->HEURE_SUP1 || $disabled ? ' disabled' : '').' alt="Ajoutez ici les heures sup entre '.$conf->global->HEURE_SUP1.' et 48h"
											...
											onblur="regexEvent_TS(this,event,\'hours\'); validateTime_HS(this,'.$idw.','.$ecart_lundi.',\''.$modeinput.'\','.$nb_jour.','.$inc.','.$temps.','.$hs_25.','.$hs_50.', '.$tmp_heure_semaine_hs.');" />';
							////$object->test_pdf .= $hs_Load['25'] / 3600;
							////$object->test_pdf .= $hs_Load['50'] / 3600;
					...
					////$object->test_pdf .= '<span>'.($total_work_task / 3600).'</span>';
					if($has_heure_nuit) {
...
function FeuilleDeTempsVerification($firstdaytoshow, $lastdaytoshow, $nb_jour, $usertoprocess, $css, $css_holiday, $num_first_day, $disabled, $userInDeplacement = 0, $userInGrandDeplacement = 0, $dayinloopfromfirstdaytoshow_array) {
	...
	//$object->test_pdf .= '<tr>';
		...
		//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>DP</strong>';
		print $form->textwithpicto('', 'Déplacement ponctuel : déplacement inhabituel dans le cadre d\'une formation, d\'une visite médicale...');
		print '</td>';
		//$object->test_pdf .= '</td>';
		for ($idw = 0; $idw < $nb_jour; $idw++) {
			...
			//$object->test_pdf .= '<td align="center" style="font-size: small; border: 0.5px solid #001b40;">';
			//$object->test_pdf .= ($array_deplacement['deplacement_ponctuel'][$dayinloopfromfirstdaytoshow] > 0 ? 'X' : '');
			//$object->test_pdf .= '</td>';
		}
		print '<td class="liste_total center fixed"><div class="'.($nb_deplacement_ponctuel != 0 ? 'noNull' : '').'" id="totalDeplacementPonctuel">'.$nb_deplacement_ponctuel.' DP</div></td>';
		//$object->test_pdf .= '<td align="center" colspan="2" style="border: 0.5px solid #001b40;">'.$nb_deplacement_ponctuel.' DP</td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	...
	//$object->test_pdf .= '<tr>';
		...
		//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>Dep';
		print $form->textwithpicto('', 'Dx : détail dans la partie supérieure de votre FdT en déroulant "Fonctionnement"<br>GD1, GD3 et GD4 = grand déplacement ou déplacement régional<br>GD2 : jour de retour en détente');
		print '</strong>';
		//$object->test_pdf .= '</strong>';
		...
		if($userInDeplacement || $regul->d1 != 0) {
			...
			//$object->test_pdf .= ($regul->d1 != 0 ? '<br>'.$regul->d1.' D1' : '');
		}
		if($userInDeplacement || $regul->d2 != 0) {
			...
			onkeypress="return regexEvent_TS(this,event,\'timeChar\', 1)" 
			onchange="updateTotal_TypeDeplacement('.$nb_jour.', '.$num_first_day.')"'.($disabled ? ' disabled' : '').'>';
			//$object->test_pdf .= ($regul->d2 != 0 ? '<br>'.$regul->d2.' D2' : '');
		}
		if($userInDeplacement || $regul->d3 != 0) {
			...
			onkeypress="return regexEvent_TS(this,event,\'timeChar\', 1)" 
			onchange="updateTotal_TypeDeplacement('.$nb_jour.', '.$num_first_day.')"'.($disabled ? ' disabled' : '').'>';
			//$object->test_pdf .= ($regul->d3 != 0 ? '<br>'.$regul->d3.' D3' : '');
		}
		if($userInDeplacement || $regul->d4 != 0) {
			...
			onkeypress="return regexEvent_TS(this,event,\'timeChar\', 1)" 
			onchange="updateTotal_TypeDeplacement('.$nb_jour.', '.$num_first_day.')"'.($disabled ? ' disabled' : '').'>';
			//$object->test_pdf .= ($regul->d4 != 0 ? '<br>'.$regul->d4.' D4' : '');
		}
		...
		if($userInGrandDeplacement || $regul->gd1 != 0) {
			...
			onkeypress="return regexEvent_TS(this,event,\'timeChar\', 1)" 
			onchange="updateTotal_TypeDeplacement('.$nb_jour.', '.$num_first_day.')"'.($disabled ? ' disabled' : '').'>';
			//$object->test_pdf .= ($regul->gd1 != 0 ? '<br>'.$regul->gd1.' GD1' : '');
		}
		if($userInGrandDeplacement || $regul->gd2 != 0) {
			...
			onkeypress="return regexEvent_TS(this,event,\'timeChar\', 1)" 
			onchange="updateTotal_TypeDeplacement('.$nb_jour.', '.$num_first_day.')"'.($disabled ? ' disabled' : '').'>';
			//$object->test_pdf .= ($regul->gd2 != 0 ? '<br>'.$regul->gd2.' GD2' : '');
		}
		if($regul->dom != 0) {
			...
			onkeypress="return regexEvent_TS(this,event,\'timeChar\', 1)" 
			onchange="updateTotal_TypeDeplacement('.$nb_jour.', '.$num_first_day.')"'.($disabled ? ' disabled' : '').'>';
			//$object->test_pdf .= ($regul->dom != 0 ? '<br>'.$regul->dom.' DOM' : '');
		}
		print '</div>';
		print '</td>';
		//$object->test_pdf .= '</td>';
		for ($idw = 0; $idw < $nb_jour; $idw++) {
			...
			//$object->test_pdf .= '<td align="center" style="padding: unset; font-size: small; border: 0.5px solid #001b40;">'.$deplacement->fields['type_deplacement']['arrayofkeyval'][$array_deplacement['type_deplacement'][$dayinloopfromfirstdaytoshow]].'</td>';
		}
		...
		print '<td class="liste_total center fixed"><div class="'.($totalTypeDeplacement != 0 ? 'noNull' : '').'" id="totalTypeDeplacement">'.$totalTypeDeplacement.'</div></td>';
		//$object->test_pdf .= '<td align="center" colspan="2" style="font-size: small; border: 0.5px solid #001b40;"><strong>'.$totalTypeDeplacement.'</strong></td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	...
	//$object->test_pdf .= '<tr>';
		...
		//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>Transport</strong>';
		print $form->textwithpicto('', "VS = Véhicule de Service<br>A = Avion<br>T = Train");
		print '</td>';
		//$object->test_pdf .= '</td>';
		for ($idw = 0; $idw < $nb_jour; $idw++) {
			...
			//$object->test_pdf .= '<td align="center" style="font-size: small; border: 0.5px solid #001b40;">'.$deplacement->fields['moyen_transport']['arrayofkeyval'][$array_deplacement['moyen_transport'][$dayinloopfromfirstdaytoshow]].'</td>';
		}
		...
		print '<td class="liste_total center fixed"><div class="'.($totalMoyenTransport != 0 ? 'noNull' : '').'" id="totalMoyenTransport">'.$totalMoyenTransport.'</div></td>';
		//$object->test_pdf .= '<td align="center" colspan="2" style="font-size: small; border: 0.5px solid #001b40;"><strong>'.$totalMoyenTransport.'</strong></td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	...
	//$object->test_pdf .= '<tr>';
		print '<td colspan="2" class="fixed"><strong>Heure Sup 0%</strong>';
		//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>HS0</strong>';
		...
		//$object->test_pdf .= ($heure_sup00 != 0 ? '<br>'.$heure_sup00 : '');
		print '</td>';
		//$object->test_pdf .= '</td>';
		for ($idw = 0; $idw < $nb_jour; $idw++) {
			$dayinloopfromfirstdaytoshow = $dayinloopfromfirstdaytoshow_array[$idw]; // $firstdaytoshow is a date with hours = 0
		...
		//$object->test_pdf .= '<td align="center" colspan="2" style="font-size: small; border: 0.5px solid #001b40;"><strong>'.($total_heure_sup00 / 3600).' HS0</strong></td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	...
	print '<tr class="nostrong">';
	//$object->test_pdf .= '<tr>';
		print '<td colspan="2" class="fixed"><strong>Heure Sup 25%</strong>';
		//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>HS25</strong>';
		$heure_sup25 = $regul->heure_sup25 / 3600;
		$total_heure_sup25 += $regul->heure_sup25;
		print '<input disabled type="text" alt="Ajoutez ici les régulations des heures sup de 25%" title="Ajoutez ici les régulations des heures sup de 25%" name="regulHeureSup25" 
		id="regulHeureSup25" class="smallpad" placeholder="Regul" value="'.($heure_sup25 ? $heure_sup25 : '').'" 
		onkeypress="return regexEvent_TS(this,event,\'timeChar\', 1)" 
		onblur="ValidateTimeDecimal(this);" 
		onchange="updateTotal_HeureSup25('.$nb_jour.', '.$num_first_day.');"'.($disabled ? ' disabled' : '').'>';
		//$object->test_pdf .= ($heure_sup25 != 0 ? '<br>'.$heure_sup25 : '');
		print '</td>';
		//$object->test_pdf .= '</td>';
		for ($idw = 0; $idw < $nb_jour; $idw++) {
			$dayinloopfromfirstdaytoshow = $dayinloopfromfirstdaytoshow_array[$idw]; // $firstdaytoshow is a date with hours = 0
		...
		//$object->test_pdf .= '<td align="center" colspan="2" style="font-size: small; border: 0.5px solid #001b40;"><strong>'.($total_heure_sup25 / 3600).' HS25</strong></td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	...
	print '<tr class="nostrong">';
	//$object->test_pdf .= '<tr>';
		print '<td colspan="2" class="fixed"><strong>Heure Sup 50%</strong>';
		//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>HS50</strong>';
		...
		//$object->test_pdf .= ($heure_sup50 != 0 ? '<br>'.$heure_sup50 : '');
		print '</td>';
		//$object->test_pdf .= '</td>';
		for ($idw = 0; $idw < $nb_jour; $idw++) {
			$dayinloopfromfirstdaytoshow = $dayinloopfromfirstdaytoshow_array[$idw]; // $firstdaytoshow is a date with hours = 0
		...
		//$object->test_pdf .= '<td align="center" colspan="2" style="font-size: small; border: 0.5px solid #001b40;"><strong>'.($total_heure_sup50 / 3600).' HS50</strong></td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	...
	//$object->test_pdf .= '<tr>';
		...
		//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>HN</strong>';
		...
		//$object->test_pdf .= ($heure_nuit50 != 0 ? '<br>'.$heure_nuit50.' 50' : '');
		//$object->test_pdf .= ($heure_nuit75 != 0 ? '<br>'.$heure_nuit75.' 75' : '');
		//$object->test_pdf .= ($heure_nuit100 != 0 ? '<br>'.$heure_nuit100.' 100' : '');
		print '</td>';
		//$object->test_pdf .= '</td>';
		for ($idw = 0; $idw < $nb_jour; $idw++) {
			$dayinloopfromfirstdaytoshow = $dayinloopfromfirstdaytoshow_array[$idw]; // $firstdaytoshow is a date with hours = 0
		...
		//$object->test_pdf .= '<td align="center" colspan="2" style="font-size: small; border: 0.5px solid #001b40;"><strong>'.($total_heure_nuit/3600).' HN</strong></td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	...
	//$object->test_pdf .= '<tr>';
		...
		//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>HR</strong>';
		...
		//$object->test_pdf .= ($heure_route != 0 ? '<br>'.$heure_route : '');
		print '</td>';
		//$object->test_pdf .= '</td>';
		for ($idw = 0; $idw < $nb_jour; $idw++) {
			$dayinloopfromfirstdaytoshow = $dayinloopfromfirstdaytoshow_array[$idw]; // $firstdaytoshow is a date with hours = 0
		...
		//$object->test_pdf .= '<td align="center" colspan="2" style="font-size: small; border: 0.5px solid #001b40;"><strong>'.($total_heure_route/3600).' HR</strong></td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	...
	//$object->test_pdf .= '<tr>';
		...
		//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>IK</strong>';
		...
		//$object->test_pdf .= ($regul->kilometres != 0 ? '<br>'.$regul->kilometres : '');
		print '</td>';
		//$object->test_pdf .= '</td>';
		for ($idw = 0; $idw < $nb_jour; $idw++) {
			$dayinloopfromfirstdaytoshow = $dayinloopfromfirstdaytoshow_array[$idw]; // $firstdaytoshow is a date with hours = 0
			...
			//$object->test_pdf .= $arraySilae['kilometres'][$dayinloopfromfirstdaytoshow];
			print '</td>';
			//$object->test_pdf .= '</td>';
		}
		...
		//$object->test_pdf .= '<td align="center" colspan="2" style="font-size: small; border: 0.5px solid #001b40;"><strong>'.$total_kilometres.' IK</strong></td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	...
	//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>TT</strong>';
	...
	//$object->test_pdf .= ($regul->indemnite_tt != 0 ? '<br>'.$regul->indemnite_tt : '');
	print '</td>';
	//$object->test_pdf .= '</td>';
	for ($idw = 0; $idw < $nb_jour; $idw++) {
		$dayinloopfromfirstdaytoshow = $dayinloopfromfirstdaytoshow_array[$idw]; // $firstdaytoshow is a date with hours = 0
		...
		//$object->test_pdf .= $arraySilae['indemnite_tt'][$dayinloopfromfirstdaytoshow];
		print '</td>';
		//$object->test_pdf .= '</td>';
	}
	print '<td class="liste_total center fixed"><div class="'.($nb_indemnite_tt != 0 ? 'noNull' : '').'" id="totalIndemniteTT">'.$nb_indemnite_tt.' TT</div></td>';
	//$object->test_pdf .= '<td align="center" colspan="2" style="font-size: small; border: 0.5px solid #001b40;"><strong>'.$nb_indemnite_tt.' TT</strong></td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	...
	//$object->test_pdf .= '<td align="center" style="border: 0.5px solid #001b40; font-size: small;" colspan="2"><strong>Repas</strong>';
	...
	//$object->test_pdf .= ($regul->repas1 != 0 ? '<br>'.$regul->repas1.' R1' : '');
	//$object->test_pdf .= ($regul->repas2 != 0 ? '<br>'.$regul->repas2.' R2' : '');
	print '</td>';
	//$object->test_pdf .= '</td>';
	for ($idw = 0; $idw < $nb_jour; $idw++) {
		$dayinloopfromfirstdaytoshow = $dayinloopfromfirstdaytoshow_array[$idw]; // $firstdaytoshow is a date with hours = 0
		...
		print '<td class="center hide'.$idw.($css[$dayinloopfromfirstdaytoshow] ? ' '.$css[$dayinloopfromfirstdaytoshow] : '').'">';
		//$object->test_pdf .= '<td align="center" style="vertical-align: middle; text-align: center; font-size: small; border: 0.5px solid #001b40;">';
		...
		//$object->test_pdf .= $arraySilae['repas'][$dayinloopfromfirstdaytoshow];
		print '</td>';
		//$object->test_pdf .= '</td>';
	}
	...
	//$object->test_pdf .= '<td align="center" colspan="2" style="font-size: small; border: 0.5px solid #001b40;"><strong>'.$totalRepas.'</strong></td>';
	print '</tr>';
	//$object->test_pdf .= '</tr>';
	print '<tbody>';
	//$object->test_pdf .= '<tbody>';
}
