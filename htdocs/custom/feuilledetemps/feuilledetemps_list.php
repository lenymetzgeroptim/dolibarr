...
/**
 *   	\file       feuilledetemps_list.php
 *		\ingroup    feuilledetemps
 *		\brief      List page for feuilledetemps
 */
...
// load feuilledetemps libraries
require_once __DIR__.'/class/feuilledetemps.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'feuilledetempslist'; // To manage different context of search
...
if (!$sortfield) {
	...
	$sortfield = "t.date_debut"; // Set here default search field. By default 1st field in definition.
}
if (!$sortorder) {
	$sortorder = "DESC";
}
...
foreach ($object->fields as $key => $val) {
	...
	if (preg_match('/^(date|timestamp|datetime)/', $val['type'])) {
		...
		if(!empty(GETPOST('search_'.$key.'_dtstart', 'int')) && empty(GETPOST('search_'.$key.'_dtstartmonth', 'int')) && empty(GETPOST('search_'.$key.'_dtstartday', 'int')) && empty(GETPOST('search_'.$key.'_dtstartyear', 'int'))) {
			$search[$key.'_dtstart'] = GETPOST('search_'.$key.'_dtstart', 'int');
		}
		if(!empty(GETPOST('search_'.$key.'_dtend', 'int')) && empty(GETPOST('search_'.$key.'_dtendmonth', 'int')) && empty(GETPOST('search_'.$key.'_dtendday', 'int')) && empty(GETPOST('search_'.$key.'_dtendyear', 'int'))) {
...
// Security check
if (empty($conf->feuilledetemps->enabled)) {
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'FeuilleDeTemps';
	$objectlabel = 'FeuilleDeTemps';
	$uploaddir = $conf->feuilledetemps->dir_output;
...
//$help_url="EN:Module_FeuilleDeTemps|FR:Module_FeuilleDeTemps_FR|ES:Módulo_FeuilleDeTemps";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("FeuillesDeTemps"));
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/feuilledetemps/feuilledetemps_card.php', 1).'?id='.$id);
	exit;
}
...
//$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/feuilledetemps/feuilledetemps_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendFeuilleDeTempsRef";
$modelmail = "feuilledetemps";
$objecttmp = new FeuilleDeTemps($db);
