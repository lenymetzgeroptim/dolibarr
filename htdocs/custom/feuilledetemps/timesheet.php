...
if($object_id > 0) {
	...
	if($object->status == FeuilleDeTemps::STATUS_VERIFICATION || $object->status == FeuilleDeTemps::STATUS_APPROBATION2 || $object->status == FeuilleDeTemps::STATUS_VALIDATED || $object->status == FeuilleDeTemps::STATUS_APPROBATION1){
		$can_modify_fdt = 0;
	}
}
else {
...
print '<tr class="nostrong liste_titre fixed favoris">';
	print '<th colspan="'.(2 + $addcolspan).'" class="fixed"><strong>Congés</strong>';
	...
	for ($idw = 0; $idw < $nb_jour; $idw++) {
		...
		if(!empty($holidayWithoutCanceled[$dayinloopfromfirstdaytoshow]['rowid'])) {
			...
			print '<th class="center hide'.$idw.($css_holiday[$dayinloopfromfirstdaytoshow] ? $css_holiday[$dayinloopfromfirstdaytoshow] : '').'" colspan="'.($dayinloopfromfirstdaytoshow_array[$idw] < $first_day_month && ($dayinloopfromfirstdaytoshow_array[$idw + $numberDay] > $first_day_month || empty($dayinloopfromfirstdaytoshow_array[$idw + $numberDay]))? $numberDay + 1 : $numberDay).'">';
...
// -	print "\n});\n";
	...
	print "$('#favoris').mouseover(function() {
		if (!$('#favoris').hasClass('clicked')) {
			$('#favoris span').addClass('fas');
			$('#favoris span').removeClass('far');
		}
	});";

	print "$('#favoris').mouseout(function() {
		if (!$('#favoris').hasClass('clicked')) {
			$('#favoris span').addClass('far');
			$('#favoris span').removeClass('fas');
		}
	});";

	print "$('#favoris').click(function() {
		if ($('#favoris').hasClass('clicked')) {
			$('#favoris').removeClass('clicked');
		}
		else {
			$('#favoris').addClass('clicked');
		}
	});";
