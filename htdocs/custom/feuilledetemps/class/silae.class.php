...
/**
 * \file        class/silae.class.php
 * \ingroup     feuilledetemps
 * \brief       This file is a CRUD class file for Silae (Create/Read/Update/Delete)
 */
 ...
 * Class for Silae
 */
class Silae extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'silae';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'feuilledetemps_silae';
	...
	/**
	 * @var string String with name of icon for silae. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'silae@feuilledetemps' if picto is file 'img/object_silae.png'.
	 */
	...
	//  */
	// public $table_element_line = 'feuilledetemps_silaeline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_silae';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Silaeline';
	...
	//  */
	// protected $childtablesoncascade = array('feuilledetemps_silaedet');

	// /**
	//  * @var SilaeLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->feuilledetemps->silae->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->feuilledetemps->silae->write))
		 || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->feuilledetemps->silae->silae_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('SILAE_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'silae/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'silae/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->feuilledetemps->dir_output.'/silae/'.$oldref;
				$dirdest = $conf->feuilledetemps->dir_output.'/silae/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->feuilledetemps->dir_output.'/silae/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'SILAE_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'SILAE_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'SILAE_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Silae").'</u>';
		...
		$url = dol_buildpath('/feuilledetemps/silae_card.php', 1).'?id='.$this->id;
	...
	*/
	...
	public function getLinesArray()
	{
		...
		$objectline = new SilaeLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_silae = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->FEUILLEDETEMPS_SILAE_ADDON)) {
			$conf->global->FEUILLEDETEMPS_SILAE_ADDON = 'mod_silae_standard';
		}

		if (!empty($conf->global->FEUILLEDETEMPS_SILAE_ADDON)) {
			$mybool = false;

			$file = $conf->global->FEUILLEDETEMPS_SILAE_ADDON.".php";
			$classname = $conf->global->FEUILLEDETEMPS_SILAE_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_silae';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->SILAE_ADDON_PDF)) {
				$modele = $conf->global->SILAE_ADDON_PDF;
			}
		}
	 ...
	 */
	public function getAllSilae($dateStart, $dateEnd, $userId)
    {
		$arraySilae = array();
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				if(!empty($obj->heure_sup00)) {
					$arraySilae['heure_sup00'][$day] = $obj->heure_sup00;
				}
				if(!empty($obj->heure_sup25)) {
					$arraySilae['heure_sup25'][$day] = $obj->heure_sup25;
				}
				...
				if(!empty($obj->heure_route)) {
					$arraySilae['heure_route'][$day] = $obj->heure_route;
				}
				if(!empty($obj->kilometres)) {
					$arraySilae['kilometres'][$day] = $obj->kilometres;
				}
				if(!empty($obj->repas)) {
					$arraySilae['repas'][$day] = $obj->repas;
				}
				if(!empty($obj->indemnite_tt)) {
					$arraySilae['indemnite_tt'][$day] = $obj->indemnite_tt;
				}
			...
			dol_syslog(get_class($this)."::getAllSilae ".$this->error, LOG_ERR);
			return -1;
		}
	}
    ...
    */
    public function fetchSilaeWithoutId($date, $fk_user)
    {
        global $langs;
        ...
        dol_syslog(get_class($this)."::fetchSilaeWithoutId", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
	...
	public function calculHS($heure_semaine, $heure_semaine_hs, $timeSpentWeek, $timeHoliday, $dayinloopfromfirstdaytoshow) {
		...
		$timeSpentWeekMax = ($timeSpentWeek[(int)date("W", $dayinloopfromfirstdaytoshow)] > $conf->global->HEURE_SUP1 ? $conf->global->HEURE_SUP1 : $timeSpentWeek[(int)date("W", $dayinloopfromfirstdaytoshow)]);
...
/**
 * Class SilaeLine. You can also remove this and generate a CRUD class for lines objects.
 */
class SilaeLine extends CommonObjectLine
{
	// To complete with content of an object SilaeLine
	// We should have a field rowid, fk_silae and position
