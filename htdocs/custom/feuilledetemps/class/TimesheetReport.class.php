...
class TimesheetReport
{
    ...
    public function initBasic($projectid, $userid, $name, $startDate, $stopDate, $mode, $invoiceableOnly = '0', $taskarray = null)
    {
        ...
        switch($mode){
            case 'PDT': //project  / task / Days //FIXME dayoff missing
                $this->modeSQLOrder = 'ORDER BY usr.rowid,prj.rowid, DATE(ptt.element_datehour), tsk.rowid ASC   ';
                //title
            ...
            case 'DPT'://day /project /task
                $this->modeSQLOrder = 'ORDER BY usr.rowid,DATE(ptt.element_datehour), prj.rowid, tsk.rowid ASC   ';
                //title
                $this->lvl0Title='userName';
            ...
            case 'PTD'://day /project /task
                $this->modeSQLOrder = 'ORDER BY usr.rowid,prj.rowid, tsk.rowid, DATE(ptt.element_datehour) ASC   ';
                //title
                $this->lvl0Title='userName';
                $this->lvl1Title = 'projectLabel';
            ...
            case 'UDT': //project  / task / Days //FIXME dayoff missing
                $this->modeSQLOrder = 'ORDER BY prj.rowid,usr.rowid, DATE(ptt.element_datehour), tsk.rowid ASC   ';
                //title
            ...
            case 'DUT'://day /project /task
                $this->modeSQLOrder = 'ORDER BY prj.rowid,DATE(ptt.element_datehour), usr.rowid, tsk.rowid ASC   ';
                //title
                $this->lvl0Title='projectLabel';
                $this->lvl1Title = 'dateDisplay';
            ...
            case 'UTD'://day /project /task
                $this->modeSQLOrder = ' ORDER BY prj.rowid,usr.rowid, tsk.rowid, DATE(ptt.element_datehour) ASC   ';
                $this->lvl0Title='projectLabel';
                $this->lvl1Title = 'userName';
                $this->lvl2Title = 'taskLabel';
                $this->lvl3Title = 'dateDisplay';
                ...
                $this->lvl2Key = 'taskId';
                break;
                case 'TUD'://day /project /task
                $this->modeSQLOrder = ' ORDER BY prj.rowid, tsk.rowid,usr.rowid, DATE(ptt.element_datehour) ASC   ';
                $this->lvl0Title='projectLabel';
                ...
                $this->lvl2Key = 'userId';
                break;
                case 'TDU'://day /project /task
                $this->modeSQLOrder = ' ORDER BY prj.rowid, tsk.rowid,DATE(ptt.element_datehour),usr.rowid ASC   ';
                $this->lvl0Title='projectLabel';
                $this->lvl1Title = 'taskLabel';
                $this->lvl2Title = 'dateDisplay';
    ...
    public function getReportArray()
    {
        ...
        $sql.= ' JOIN '.MAIN_DB_PREFIX.'projet_task as tsk ON tsk.rowid = fk_element ';
        ...
        if($this->invoiceableOnly == 1) {
         ...
         /*if(!empty($startDay))$sql .= 'AND task_date>=\''.$this->db->idate($startDay).'\'';
          else */$sql .= ($first?'':'AND ').' DATE(element_datehour)>=\''.$this->db->idate($this->startDate).'\'';
          /*if(!empty($stopDay))$sql.= ' AND task_date<=\''.$this->db->idate($stopDay).'\'';
          else */$sql.= ' AND DATE(element_datehour)<=\''.$this->db->idate($this->stopDate).'\'';
        $sql .= " AND ptt.elementtype = 'task'";
         $sql .= ' GROUP BY usr.rowid, DATE(ptt.element_datehour),  prj.rowid, tsk.rowid ';
        /*if(!empty($sqltail)) {
            $sql .= $sqltail;
        }*/
     ...
     * @return array()
     */
    // public function getReportArray2()
    // {
    //     global $conf, $db;
    //     $resArray = array();
    //     $first = true;
    //     $sql = 'SELECT prj.rowid as projectid, usr.rowid as userid, tsk.rowid as taskid, fk_task_parent, ';
    //     if($db->type!='pgsql') {
    //         $sql.= ' MAX(prj.title) as projecttitle, MAX(prj.ref) as projectref, MAX(usr.firstname) as firstname, MAX(usr.lastname) as lastname, ';
    //         $sql.= " MAX(tsk.ref) as taskref, MAX(tsk.label) as tasktitle, GROUP_CONCAT(ptt.note SEPARATOR '. ') as note, MAX(tske.invoiceable) as invoicable, ";
    //     } else {
    ...
    //     $sql.= ' DATE(ptt.element_datehour) AS element_date, SUM(ptt.element_duration) as duration ';
    //     $sql.= ' FROM '.MAIN_DB_PREFIX.'element_time as ptt ';
    //     $sql.= ' JOIN '.MAIN_DB_PREFIX.'projet_task as tsk ON fk_task_parent = fk_element ';
    //     $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'projet_task_extrafields as tske ON tske.fk_object = fk_task_parent ';
    //     $sql.= ' JOIN '.MAIN_DB_PREFIX.'projet as prj ON prj.rowid = tsk.fk_projet ';
    //     $sql.= ' JOIN '.MAIN_DB_PREFIX.'user as usr ON ptt.fk_user = usr.rowid ';
    //     $sql.= ' WHERE ';
    //     if(!empty($this->userid)) {
    ...
    */
    public function getHTMLreportExport()
    {
        ...
        //merge children task array
        //$resArray = array_merge($this->getReportArray(), $this->getReportArray2());
        $resArray = $this->getReportArray();

        //sort array values by userid, taskid and childern taskid and date 
        ...
        $HTMLRes .= $this->getHTMLReportHeaders(0);
        foreach($resArray as $key => $item) {
           $item['date'] = dol_print_date($item['date'], 'day');
           $HTMLRes .= '<tr class = "oddeven center">';//<th width = "200px">'.$this->name.'</th>';
     ...
     * @return string
     */
    public function getHTMLReportHeaders($short)
    {
        global $langs;
        ...
        $HTMLHeaders .= '<tr class="liste_titre wrapcolumntitle">';//<th>'.$langs->trans('Name').'</th>';
        $HTMLHeaders .= '<th class="center">'.$langs->trans($title[$this->lvl0Title]).'</th>';
        $HTMLHeaders .= '<th class="center">'.$langs->trans($title[$this->lvl1Title]).'</th>';
        $HTMLHeaders .= '<th class="center">'.$langs->trans($title[$this->lvl2Title]).'</th>';
        $HTMLHeaders .= (!$short ? '<th class="center">'.$langs->trans($title[$this->lvl3Title]).'</th>' : '');
        $HTMLHeaders .= '<th class="center">'.$langs->trans('Duration').' (h)'.'</th>';
        $HTMLHeaders .= '<th class="center">'.$langs->trans('Duration').' ('.$langs->trans('Days').')'.'</th>';
        $HTMLHeaders .= '<th class="center">'.$langs->trans('Note').'</th></tr>';
        return $HTMLHeaders;
    }
 ...
 */
    ...
    {
        $lvl2HTML .= '<tr class = "oddeven center" style="color: #0052aa;"><td colspan="2"></td><td>'
            .$lvl2title.'</td>';
 ...
 */
    ...
    {
        $lvl1HTML .= '<tr class = "oddeven center"><td colspan="1"></td><td>'
            .$lvl1title.'</td><td></td>';
        // add an empty cell on row if short version(in none short mode there is an additionnal column
        if(!$short)$lvl1HTML .= '<td></td>';
        $lvl1HTML .= '<td>'.formatTime($lvl2Total, 0).'</td>';
        $lvl1HTML .= '<td>'.formatTime($lvl2Total, -3).'</td></td><td></tr>';
 ...
 */
    ...
    {
        // make the whole result
            $lvl0HTML .= $this->getHTMLReportHeaders($short);
            $lvl0HTML .= '<tr class="liste_titre wrapcolumntitle"><th class="center"><strong>'.$lvl0title.'</strong><th>';
            $lvl0HTML .=((!$short)?'<th></th>':'').'<th></th>';
            $lvl0HTML .= '<th class="center"><strong>'.formatTime($lvl1Total, 0).'</strong></th>';
            $lvl0HTML .= '<th class="center"><strong>'.formatTime($lvl1Total, -3).'</strong></th><th></th></tr>';
           //add the LVL 3 HTML content in lvl1
            $lvl0HTML .= $lvl1HTML;
            $lvl0HTML .= '</table>';
        return $lvl0HTML;
 ...
 */
    ...
    {
        $lvl3HTML .= '<tr class = "oddeven center"><td colspan="3" ></td><td>'
