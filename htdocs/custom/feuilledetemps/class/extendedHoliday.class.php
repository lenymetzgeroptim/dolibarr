...
class extendedHoliday extends Holiday
{
	...
	public function verifDateHolidayForTimestamp($fk_user, $timestamp, $status = '-1', $excluded_types = array())
	{
		...
		$sql = "SELECT cp.rowid, cp.date_debut as date_start, cp.date_fin as date_end, cp.halfday, cp.statut, ht.code, he.hour, he.statutfdt";
		...
		if ($resql) {
			...
			if ($num_rows > 0) {
				...
				while ($i < $num_rows) {
					...
					$arrayofrecord[$obj->rowid] = array('date_start'=>$this->db->jdate($obj->date_start), 'date_end'=>$this->db->jdate($obj->date_end), 'halfday'=>$obj->halfday, 'statut'=>$obj->statut, 'code'=>$obj->code, 'rowid'=>$obj->rowid, 'hour'=>$obj->hour, 'statutfdt'=>$obj->statutfdt);
					$i++;
				}
		...
		$result = array('morning'=>$isavailablemorning, 'afternoon'=>$isavailableafternoon, 'statut'=>$statut, 'code'=>$code, 'rowid'=>$rowid, 'hour'=>$hour, 'statutfdt'=>$statutfdt);
	...
	public function getTypesNoCP($active = -1, $affect = -1)
	{
		...
		$sql .= " AND code NOT IN ('CP_N-1_ACQUIS', 'CP_N-1_PRIS', 'CP_N_ACQUIS', 'CP_N_PRIS', 'CP_FRAC_ACQUIS', 'CP_FRAC_PRIS', 'CP_RTT_ACQUIS', 'CP_RTT_PRIS')";
