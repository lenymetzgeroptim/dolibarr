...
class FeuilleDeTemps extends CommonObject
{
	...
	public $fields=array(
		...
		'fk_user' => array('type'=>'integer:User:user/class/user.class.php:0:(statut:=:1)', 'label'=>'Utilisateur', 'enabled'=>'1', 'position'=>110, 'notnull'=>1, 'visible'=>1, 'index'=>1,),
	...
	function timeHolidayWeek($user_id, $firstdate = '', $lastdate = '') {
		...
		for ($idw = 0; $idw < $nb_jour; $idw++) { 
			...
			if (dol_print_date($tmpday, '%a') != 'Sam' && dol_print_date($tmpday, '%a') != 'Dim') {
				...
				elseif(($isavailablefordayanduser['morning'] == false || $isavailablefordayanduser['afternoon'] == false) && $isavailablefordayanduser['hour']%12600 != 0){ // Congés en heure
	...
	public function MailFDT_WeeklySave()
	{
		...
		if(dol_print_date($now, '%a') == 'Sam' || dol_print_date($now, '%a') == 'Dim') {
			...
			if ($result) {
				...
				$link = '<a href="'.$urlwithroot.'/custom/feuilledetemps/timesheetByMonth.php">'.'ici'.'</a>';
