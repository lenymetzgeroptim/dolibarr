...
/**
 * \file    test/phpunit/CommunicationFunctionalTest.php
...
/**
 * Class CommunicationFunctionalTest
 *
 ...
 */
class CommunicationFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modCommunication')->click();
		} else {
			// Disable the module
			$this->byHref('modCommunication')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/communication/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('communication/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/communication/admin/about.php');
		$this->authenticate();
		return $this->assertContains('communication/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('communicationwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modCommunication_CommunicationTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modCommunication_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
