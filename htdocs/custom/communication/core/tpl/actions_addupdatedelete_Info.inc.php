...
if ($action == 'add' && !empty($permissiontoadd)) {
	...
	//Update de la table notifications (version modifiée) : ---------------------------
	require_once DOL_DOCUMENT_ROOT.'/custom/communication/class/notification.class.php';
		...
		if($req > 0) {
			foreach ($userTab as $var_user) {
					//Vérifier si la personne possède une ligne dans la table Notification :
						$res = $notif->checkNotificationLine($var_user->id);
						if ($res){
							//Si elle possède déjà une ligne en base, update le statut d'activated
							$sql= 'UPDATE `llx_communication_notification` SET `activated_info`= 1 WHERE `fk_user`= ' . $var_user->id;
							$result = $db->query($sql);
						} else {
							//Sinon créer une ligne et mettre activated à true
							$sql = 'INSERT INTO `llx_communication_notification`(`activated_info`,`activated_post`, `fk_user`) VALUES (1,0,'. $var_user->id.')';
							$result = $db->query($sql);
						}
			}
		} else {
