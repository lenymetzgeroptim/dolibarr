		...
		foreach ($userTab as $var_user) {
				//Vérifier si la personne possède une ligne dans la table Notification :
					...
					if ($res){
						...
						$sql= 'UPDATE `llx_communication_notification` SET `activated_post`= 1 WHERE `fk_user`= ' . $var_user->id;
						$result = $db->query($sql);
					} else {
						//Sinon créer une ligne et mettre activated à true
						$sql = 'INSERT INTO `llx_communication_notification`(`activated_info`,`activated_post`, `fk_user`) VALUES (0,1,'. $var_user->id.')';
						$result = $db->query($sql);
					}
		}
	} else {
