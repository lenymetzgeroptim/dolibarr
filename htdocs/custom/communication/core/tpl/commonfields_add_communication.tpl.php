...
foreach ($object->fields as $key => $val) {
	...
	if (!empty($val['noteditable'])) {
		...
		elseif ($key == 'content') {
			...
			$doleditor = new DolEditor($key, $value, '', 200, 'dolibarr_notes', 'In', false, true, isModEnabled('fckeditor') && $conf->global->FCKEDITOR_ENABLE_SOCIETE, ROWS_5, '90%');
			print $doleditor->Create(1, '', true, '', '', '', '');
		} else {
			print $object->showInputField($val, $key, $value, '', '', '', 0);
		}
	}
