...
/**
 *  \file       formation_document.php
 *  \ingroup    formationhabilitation
 *  \brief      Tab for documents linked to Formation
 */
...
$hookmanager->initHooks(array('formationdocument', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->formationhabilitation->multidir_output[$object->entity ? $object->entity : $conf->entity]."/formation/".get_exdir(0, 0, 0, 1, $object);
}
...
$title = $langs->trans("Formation").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'document', $langs->trans("Formation"), -1, $object->picto);
	...
	$relativepathwithnofile = 'formation/'.dol_sanitizeFileName($object->ref).'/';
