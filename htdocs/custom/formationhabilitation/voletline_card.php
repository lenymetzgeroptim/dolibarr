...
/**
 *    \file       voletline_card.php
 *    \ingroup    formationhabilitation
 *    \brief      Page to create/edit/view voletline
 */
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('formationhabilitation', 'voletline', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('formationhabilitation', 'voletline', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('formationhabilitation', 'voletline', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('formationhabilitation', 'voletline', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->formationhabilitation->multidir_output[isset($object->entity) ? $object->entity : 1].'/voletline';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/formationhabilitation/voletline_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/formationhabilitation/voletline_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
$title = $langs->trans("VoletLine")." - ".$langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("VoletLine"));
}
$help_url = '';
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("VoletLine"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("VoletLine"), -1, $object->picto, 0, '', '', 0, '', 1);

	$formconfirm = '';

	// Confirmation to delete (using preloaded confirm popup)
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteVoletLine'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	if ($action == 'xxx') {
		$text = $langs->trans('ConfirmActionVoletLine', $object->ref);
	...
	*/
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('formationhabilitation:VoletLine', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('voletline'));
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/formationhabilitation/voletline_agenda.php', 1).'?id='.$object->id);
