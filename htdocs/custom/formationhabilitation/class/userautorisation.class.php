...
/**
 * \file        class/userautorisation.class.php
 * \ingroup     formationhabilitation
 * \brief       This file is a CRUD class file for UserAutorisation (Create/Read/Update/Delete)
 */
 ...
 */
class UserAutorisation extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'userautorisation';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'formationhabilitation_userautorisation';
	...
	/**
	 * @var string String with name of icon for userautorisation. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'userautorisation@formationhabilitation' if picto is file 'img/object_userautorisation.png'.
	 */
	public $picto = 'fa-user-check_fas_green';
	...
	//  */
	// public $table_element_line = 'formationhabilitation_userautorisationline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_userautorisation';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'UserAutorisationline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'UserAutorisation', 'fk_element'=>'fk_userautorisation'));
	...
	//  */
	// protected $childtablesoncascade = array('formationhabilitation_userautorisationdet');

	// /**
	//  * @var UserAutorisationLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('formationhabilitation', 'userautorisation', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'userautorisation/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'userautorisation/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++; $this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'userautorisation/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'userautorisation/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++; $this->error = $this->db->lasterror();
				}
				...
				$dirsource = $conf->formationhabilitation->dir_output.'/userautorisation/'.$oldref;
				$dirdest = $conf->formationhabilitation->dir_output.'/userautorisation/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->formationhabilitation->dir_output.'/userautorisation/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowUserAutorisation")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("UserAutorisation").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/formationhabilitation/userautorisation_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowUserAutorisation");
	...
	public function getLinesArray()
	{
		...
		$objectline = new UserAutorisationLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_userautorisation = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('FORMATIONHABILITATION_MYOBJECT_ADDON')) {
			$conf->global->FORMATIONHABILITATION_MYOBJECT_ADDON = 'mod_userautorisation_standard';
		}
	...
	/**
	 * 	Return l'id de userautorisation à partir de l'id de l'autorisation et de l'utilisateur
	 *
	 * 	@param	int		$id_habilitation	Id de l'autorisation
	 * 	@param	int		$id_user			Id de l'utilisateur
	 * 	@return	int							Id de userautorisation
	 */
	public function getId($id_autorisation, $id_user)
	{
		global $conf, $user;
...
/**
 * Class UserAutorisationLine. You can also remove this and generate a CRUD class for lines objects.
 */
class UserAutorisationLine extends CommonObjectLine
{
	// To complete with content of an object UserAutorisationLine
	// We should have a field rowid, fk_userautorisation and position
