...
/**
 * \file        class/convocation.class.php
 * \ingroup     formationhabilitation
 * \brief       This file is a CRUD class file for Convocation (Create/Read/Update/Delete)
 */
 ...
 */
class Convocation extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'convocation';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'formationhabilitation_convocation';
	...
	/**
	 * @var string String with name of icon for convocation. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'convocation@formationhabilitation' if picto is file 'img/object_convocation.png'.
	 */
	public $picto = 'fa-envelope-open-text_fas_#808080';
	...
	public $fields=array(
		...
		"datedebut" => array("type"=>"datetime", "label"=>"DateDebut", "enabled"=>"1", 'position'=>30, 'notnull'=>1, "visible"=>"1",),
		"datefin" => array("type"=>"datetime", "label"=>"DateFin", "enabled"=>"1", 'position'=>31, 'notnull'=>0, "visible"=>"1",),
		"nature" => array("type"=>"integer", "label"=>"NatureConvoc", "enabled"=>"1", 'position'=>35, 'notnull'=>1, "visible"=>"1", "arrayofkeyval"=>array("1" => "Formation", "2" => "Visite médicale", "3" => "Examen médical", "4" => "Vaccination"),),
		"type" => array("type"=>"integer", "label"=>"Type", "enabled"=>"1", 'position'=>40, 'notnull'=>0, "visible"=>"1", "default"=>"1", "arrayofkeyval"=>array("1" => "Externe", "2" => "Interne"),),
	...
	//  */
	// public $table_element_line = 'formationhabilitation_convocationline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_convocation';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Convocationline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Convocation', 'fk_element'=>'fk_convocation'));
	...
	//  */
	// protected $childtablesoncascade = array('formationhabilitation_convocationdet');

	// /**
	//  * @var ConvocationLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('formationhabilitation', 'convocation', 'read')) {
	 ...
	 public function generationWithFormation(UserFormation $userFormation, User $user, $date_debut, $date_fin, $notrigger = false)
	{
		...
		$this->lieu_formation = GETPOST("lieu_formation_programmer", 'alpha');
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('formationhabilitation', 'convocation', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('formationhabilitation', 'convocation_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'convocation/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'convocation/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'convocation/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'convocation/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->formationhabilitation->dir_output.'/convocation/'.$oldref;
				$dirdest = $conf->formationhabilitation->dir_output.'/convocation/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->formationhabilitation->dir_output.'/convocation/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowConvocation")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Convocation").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/formationhabilitation/convocation_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowConvocation");
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('FORMATIONHABILITATION_MYOBJECT_ADDON')) {
			$conf->global->FORMATIONHABILITATION_MYOBJECT_ADDON = 'mod_convocation_standard';
		}
...
/**
 * Class ConvocationLine. You can also remove this and generate a CRUD class for lines objects.
 */
class ConvocationLine extends CommonObjectLine
{
	// To complete with content of an object ConvocationLine
	// We should have a field rowid, fk_convocation and position
