...
/**
 * \file        class/autorisation.class.php
 * \ingroup     formationhabilitation
 * \brief       This file is a CRUD class file for Autorisation (Create/Read/Update/Delete)
 */
...
/**
 * Class for Autorisation
 */
class Autorisation extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'autorisation';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'formationhabilitation_autorisation';
	 ...
	 */
	public $picto = 'fa-check_fas_green';
	...
	public $fields=array(
		...
		"label" => array("type"=>"varchar(255)", "label"=>"Label", "enabled"=>"1", 'position'=>30, 'notnull'=>0, "visible"=>"1", "searchall"=>"1", "css"=>"minwidth300", "cssview"=>"wordbreak", "validate"=>"1", "showoncombobox"=>"1",),
	...
	//  */
	public $table_element_line = 'formationhabilitation_userautorisation';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	public $fk_element = 'fk_autorisation';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	public $class_element_line = 'UserAutorisation';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Autorisation', 'fk_element'=>'fk_autorisation'));
	...
	//  */
	protected $childtablesoncascade = array('formationhabilitation_userautorisation');

	// /**
	//  * @var AutorisationLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('formationhabilitation', 'autorisation', 'read')) {
	...
	public function fetchLines($morewhere = '')
	{
		...
		$objectlineclassname = 'UserAutorisation';
		...
		$sql .= " FROM ".$this->db->prefix()."formationhabilitation_userautorisation as l";
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('formationhabilitation','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && !empty($user->rights->formationhabilitation->autorisation->autorisation_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'autorisation/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'autorisation/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++; $this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'autorisation/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'autorisation/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++; $this->error = $this->db->lasterror();
				}
				...
				$dirsource = $conf->formationhabilitation->dir_output.'/autorisation/'.$oldref;
				$dirdest = $conf->formationhabilitation->dir_output.'/autorisation/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->formationhabilitation->dir_output.'/autorisation/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function cloture($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('AUTORISATION_CLOTURE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowAutorisation")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Autorisation").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/formationhabilitation/autorisation_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowAutorisation");
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('FORMATIONHABILITATION_MYOBJECT_ADDON')) {
			$conf->global->FORMATIONHABILITATION_MYOBJECT_ADDON = 'mod_autorisation_standard';
		}
	...
	/**
	 * 	Return toutes les autorisations actives dont la formation $formationid est requise
	 *
	 * 	@param  int		$formationId       Id of Formation
	 * 	@return	array(int)|int						
	 */
	public function getAutorisationsWhereFormationIsRequire($formationId)
	{
		global $conf, $user;
		$res = array();
	...
	/**
	 * 	Return toutes les autorisations actives
	 *
	 * 	@return	array(int)|int						
	 */
	public function getAllAutorisations()
	{
		global $conf, $user;
		$res = array();
		...
		dol_syslog(get_class($this)."::getAllAutorisations", LOG_DEBUG);
		$resql = $this->db->query($sql);
		if ($resql) {
			while($obj = $this->db->fetch_object($resql)) {
				$res[] = $obj->rowid;
	...
	/**
	 * 	Attribuer une autorisation à un utilisateur
	 *
	...
	function AsignToUser($user_id, $userformation, $date_finvalidite) {
		...
		if($userformation == null) {
			$userAutorisation->date_autorisation = dol_now();
		}
		else {
	...
	/**
	 * 	Génère les autorisations lors de la validation d'une formation
	 *
	 * 	@param  int				$user_id       			Id of User
	 *  @param  UserFormation	$userformation     		new UserFormation
	 * 	@param  string			$txtListAutorisation  	Contain label of autorisation generate
	 *  @param  int				$disable_generation  	1 to disable generation and return autorisations that can be generated
	 *  @param  array			$only_autorisations  	Generation only autorisations on this array
	 * 	@return	int|array						<0 if KO, > 0 if OK				
	 */
	function generateAutorisationsForUser($user_id, $userformation, &$txtListAutorisation, $disable_generation = 0, $only_autorisations = null) { // TODOLeny - gestion des erreurs
		$autorisations_ok = array(); 
		...
		if($userformation == null) {
			$autorisations = $this->getAllAutorisations();
		}
		else {
		...
		foreach ($autorisations as $autorisation_id) {
			if($only_autorisations != null && !in_array($autorisation_id, $only_autorisations)) {
				continue;
			}
			...
			elseif($all_conditions_met == 1 && $disable_generation) {
				$autorisations_ok[$autorisation->id] = $autorisation->label;
			}
		}
...
/**
 * Class AutorisationLine. You can also remove this and generate a CRUD class for lines objects.
 */
class AutorisationLine extends CommonObjectLine
{
	// To complete with content of an object AutorisationLine
	// We should have a field rowid, fk_autorisation and position
