...
/**
 * \file        class/voletline.class.php
 * \ingroup     formationhabilitation
 * \brief       This file is a CRUD class file for VoletLine (Create/Read/Update/Delete)
 */
 ...
 * Class for VoletLine
 */
class VoletLine extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'voletline';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'formationhabilitation_voletline';
	...
	/**
	 * @var string String with name of icon for voletline. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'voletline@formationhabilitation' if picto is file 'img/object_voletline.png'.
	 */
	...
	public $fields=array(
		...
		"label" => array("type"=>"varchar(255)", "label"=>"Label1", "enabled"=>"1", 'position'=>30, 'notnull'=>0, "visible"=>"1", "searchall"=>"1", "css"=>"minwidth300", "cssview"=>"wordbreak", "help"=>"Help text", "validate"=>"1",),
		...
		"label2" => array("type"=>"varchar(255)", "label"=>"Label2", "enabled"=>"1", 'position'=>31, 'notnull'=>0, "visible"=>"1",),
	);
	public $rowid;
	public $label;
	public $qty;
	...
	//  */
	// public $table_element_line = 'formationhabilitation_voletlineline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_voletline';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'VoletLineline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'VoletLine', 'fk_element'=>'fk_voletline'));
	...
	//  */
	// protected $childtablesoncascade = array('formationhabilitation_voletlinedet');

	// /**
	//  * @var VoletLineLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('formationhabilitation', 'voletline', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('formationhabilitation', 'voletline', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('formationhabilitation', 'voletline_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'voletline/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'voletline/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'voletline/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'voletline/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->formationhabilitation->dir_output.'/voletline/'.$oldref;
				$dirdest = $conf->formationhabilitation->dir_output.'/voletline/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->formationhabilitation->dir_output.'/voletline/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowVoletLine")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("VoletLine").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/formationhabilitation/voletline_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowVoletLine");
	...
	public function getLinesArray()
	{
		...
		$objectline = new VoletLineLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_voletline = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('FORMATIONHABILITATION_MYOBJECT_ADDON')) {
			$conf->global->FORMATIONHABILITATION_MYOBJECT_ADDON = 'mod_voletline_standard';
		}
...
/**
 * Class VoletLineLine. You can also remove this and generate a CRUD class for lines objects.
 */
class VoletLineLine extends CommonObjectLine
{
	// To complete with content of an object VoletLineLine
	// We should have a field rowid, fk_voletline and position
