...
/**
 * \file        class/volettable.class.php
 * \ingroup     formationhabilitation
 * \brief       This file is a CRUD class file for VoletTable (Create/Read/Update/Delete)
 */
...
/**
 * Class for VoletTable
 */
class VoletTable extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'volettable';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'formationhabilitation_volettable';
	...
	/**
	 * @var string String with name of icon for volettable. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'volettable@formationhabilitation' if picto is file 'img/object_volettable.png'.
	 */
	...
	//  */
	// public $table_element_line = 'formationhabilitation_volettableline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_volettable';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'VoletTableline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'VoletTable', 'fk_element'=>'fk_volettable'));
	...
	//  */
	// protected $childtablesoncascade = array('formationhabilitation_volettabledet');

	// /**
	//  * @var VoletTableLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('formationhabilitation', 'volettable', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('formationhabilitation', 'volettable', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('formationhabilitation', 'volettable_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'volettable/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'volettable/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'volettable/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'volettable/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->formationhabilitation->dir_output.'/volettable/'.$oldref;
				$dirdest = $conf->formationhabilitation->dir_output.'/volettable/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->formationhabilitation->dir_output.'/volettable/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowVoletTable")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("VoletTable").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/formationhabilitation/volettable_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowVoletTable");
	...
	public function getLinesArray()
	{
		...
		$objectline = new VoletTableLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_volettable = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('FORMATIONHABILITATION_MYOBJECT_ADDON')) {
			$conf->global->FORMATIONHABILITATION_MYOBJECT_ADDON = 'mod_volettable_standard';
		}
...
/**
 * Class VoletTableLine. You can also remove this and generate a CRUD class for lines objects.
 */
class VoletTableLine extends CommonObjectLine
{
	// To complete with content of an object VoletTableLine
	// We should have a field rowid, fk_volettable and position
