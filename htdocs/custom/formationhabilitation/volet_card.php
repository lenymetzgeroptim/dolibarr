 ...
 *    \brief      Page to create/edit/view volet
 */
...
$upload_dir = $conf->formationhabilitation->multidir_output[isset($object->entity) ? $object->entity : 1].'/volet';
...
$title = $langs->trans("Volet")." - ".$langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("Volet"));
}
$help_url = '';
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Volet"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("Volet"), -1, $object->picto, 0, '', '', 0, '', 1);

	$formconfirm = '';

	// Confirmation to delete (using preloaded confirm popup)
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteVolet'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	if ($action == 'xxx') {
		$text = $langs->trans('ConfirmActionVolet', $object->ref);
