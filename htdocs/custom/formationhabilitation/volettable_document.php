...
/**
 *  \file       volettable_document.php
 *  \ingroup    formationhabilitation
 *  \brief      Tab for documents linked to VoletTable
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->formationhabilitation->multidir_output[$object->entity ? $object->entity : $conf->entity]."/volettable/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('formationhabilitation', 'volettable', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("VoletTable")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("VoletTable"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='volettable/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'volettable/'.dol_sanitizeFileName($object->ref).'/';
