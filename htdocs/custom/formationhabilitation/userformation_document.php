...
/**
 *  \file       htdocs/modulebuilder/template/userformation_document.php
 *  \ingroup    formationhabilitation
 *  \brief      Tab for documents linked to UserFormation
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->formationhabilitation->multidir_output[$object->entity ? $object->entity : $conf->entity]."/userformation/".get_exdir(0, 0, 0, 1, $object);
}

// Permissions
// (There are several ways to check permission.)
$permissiontoread = $user->hasRight('formationhabilitation', 'userformation', 'readall') || ($object->fk_user == $user->id && $user->hasRight('formationhabilitation', 'userformation', 'read'));
$permissiontoadd  = $user->hasRight('formationhabilitation', 'userformation', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
...
// Header
// ------
$title = $langs->trans("UserFormation")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("UserFormation"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='userformation/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'userformation/'.dol_sanitizeFileName($object->ref).'/';
