...
/**
 *   	\file       userformation_card.php
 *		\ingroup    formationhabilitation
 *		\brief      Page to create/edit/view userformation
 */
...
dol_include_once('/formationhabilitation/lib/formationhabilitation_userformation.lib.php');
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'userformationcard'; // To manage different context of search
$backtopage = GETPOST('backtopage', 'alpha');
$backtopageforcancel = GETPOST('backtopageforcancel', 'alpha');
$lineid   = GETPOST('lineid', 'int');

// Initialize technical objects
$object = new UserFormation($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction = $conf->formationhabilitation->dir_output.'/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('userformationcard', 'globalcard')); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/formationhabilitation/userformation_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/formationhabilitation/userformation_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'FORMATIONHABILITATION_USERFORMATION_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	// $triggersendname = 'FORMATIONHABILITATION_USERFORMATION_SENTBYMAIL';
	// $autocopy = 'MAIN_MAIL_AUTOCOPY_USERFORMATION_TO';
...
$title = $langs->trans("UserFormation");
$help_url = '';
...
// 	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("UserFormation")), '', 'object_'.$object->picto);
...
// 	print load_fiche_titre($langs->trans("UserFormation"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	$head = userformationPrepareHead($object);
	print dol_get_fiche_head($head, 'card', $langs->trans("UserFormation"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	// if ($action == 'delete') {
	// 	$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteUserFormation'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	// }
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/formationhabilitation/userformation_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	$linkback .= '<a href="'.dol_buildpath('/formationhabilitation/userformation.php', 1).'?id='.$object->fk_user.'&onglet=formation&restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToUserList").'</a>';
