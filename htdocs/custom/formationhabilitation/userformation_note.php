...
/**
 *  \file       htdocs/modulebuilder/template/userformation_note.php
 *  \ingroup    formationhabilitation
 *  \brief      Tab for notes on UserFormation
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->formationhabilitation->multidir_output[empty($object->entity) ? $conf->entity : $object->entity]."/".$object->id;
}
...
$permissiontoadd = $user->hasRight('formationhabilitation', 'userformation', 'write');
$permissionnote = $user->hasRight('formationhabilitation', 'userformation', 'write'); // Used by the include of actions_setnotes.inc.php
...
$title = $langs->trans('UserFormation').' - '.$langs->trans("Notes");
//$title = $object->ref." - ".$langs->trans("Notes");
$help_url = '';
//$help_url='EN:Customers_Orders|FR:Commandes_Clients|ES:Pedidos de clientes';

llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-formationhabilitation page-card_notes');
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("UserFormation"), -1, $object->picto);
