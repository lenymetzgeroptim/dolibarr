...
/**
 *  \file       autorisation_contact.php
 *  \ingroup    formationhabilitation
 *  \brief      Tab for contacts linked to Autorisation
 */
...
$hookmanager->initHooks(array('autorisationcontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->formationhabilitation->autorisation->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Autorisation')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Autorisation"), -1, $object->picto);
