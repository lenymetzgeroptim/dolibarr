...
$extrafields->fetch_name_optionals_label($elementPrerequis->table_element);
//$extrafields->fetch_name_optionals_label($elementPrerequis->table_element_line);

$search_array_options = $extrafields->getOptionalsFromPost($elementPrerequis->table_element, '', 'search_');

// Default sort order (if not yet defined by previous GETPOST)
if (!$sortfield) {
	reset($elementPrerequis->fields);					// Reset is required to avoid key() to return null.
	$sortfield = "t.".key($elementPrerequis->fields); // Set here default search field. By default 1st field in definition.
}
if (!$sortorder) {
	$sortorder = "ASC";
}
...
$search = array();
foreach ($elementPrerequis->fields as $key => $val) {
...
// $reshook = $hookmanager->executeHooks('completeFieldsToSearchAll', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by some hooks
// if ($reshook > 0) {
...
$elementPrerequis->fields = dol_sort_array($elementPrerequis->fields, 'position');
...
//$isdraft = (($elementPrerequis->status == $elementPrerequis::STATUS_DRAFT) ? 1 : 0);
//restrictedArea($user, $elementPrerequis->module, 0, $elementPrerequis->table_element, $elementPrerequis->element, 'fk_soc', 'rowid', $isdraft);
...
$parameters = array();
$reshook = $hookmanager->executeHooks('doActions', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by some hooks
if ($reshook < 0) {
...
$parameters = array();
$reshook = $hookmanager->executeHooks('printFieldListSelect', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
$sql .= $hookmanager->resPrint;
$sql = preg_replace('/,\s*$/', '', $sql);

$sqlfields = $sql; // $sql fields to remove for count total

$sql .= " FROM ".MAIN_DB_PREFIX.$elementPrerequis->table_element." as t";
//$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."anothertable as rc ON rc.parent = t.rowid";
if (isset($extrafields->attributes[$elementPrerequis->table_element]['label']) && is_array($extrafields->attributes[$elementPrerequis->table_element]['label']) && count($extrafields->attributes[$elementPrerequis->table_element]['label'])) {
	$sql .= " LEFT JOIN ".MAIN_DB_PREFIX.$elementPrerequis->table_element."_extrafields as ef on (t.rowid = ef.fk_object)";
}
// Add table from hooks
$parameters = array();
$reshook = $hookmanager->executeHooks('printFieldListFrom', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
$sql .= $hookmanager->resPrint;
if ($elementPrerequis->ismultientitymanaged == 1) {
	$sql .= " WHERE t.entity IN (".getEntity($elementPrerequis->element, (GETPOST('search_current_entity', 'int') ? 0 : 1)).")";
} else {
	$sql .= " WHERE 1 = 1";
}
if($object->id > 0) {
	$sql .= " AND fk_source = $object->id";
	$sql .= " AND sourcetype = '$object->element'";
}
foreach ($search as $key => $val) {
	if (array_key_exists($key, $elementPrerequis->fields)) {
		if ($key == 'status' && $search[$key] == -1) {
			continue;
		}
		$mode_search = (($elementPrerequis->isInt($elementPrerequis->fields[$key]) || $elementPrerequis->isFloat($elementPrerequis->fields[$key])) ? 1 : 0);
		if ((strpos($elementPrerequis->fields[$key]['type'], 'integer:') === 0) || (strpos($elementPrerequis->fields[$key]['type'], 'sellist:') === 0) || !empty($elementPrerequis->fields[$key]['arrayofkeyval'])) {
			if ($search[$key] == '-1' || ($search[$key] === '0' && (empty($elementPrerequis->fields[$key]['arrayofkeyval']) || !array_key_exists('0', $elementPrerequis->fields[$key]['arrayofkeyval'])))) {
				$search[$key] = '';
			}
			$mode_search = 2;
		}
		...
		if (preg_match('/(_dtstart|_dtend)$/', $key) && $search[$key] != '') {
			...
			if (preg_match('/^(date|timestamp|datetime)/', $elementPrerequis->fields[$columnName]['type'])) {
...
$parameters = array();
$reshook = $hookmanager->executeHooks('printFieldListWhere', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
$sql .= $hookmanager->resPrint;
...
if (!empty($extrafields->attributes[$elementPrerequis->table_element]['label'])) {
	foreach ($extrafields->attributes[$elementPrerequis->table_element]['label'] as $key => $val) {
		$sql .= ($extrafields->attributes[$elementPrerequis->table_element]['type'][$key] != 'separate' ? "ef.".$key.', ' : '');
	}
}
// Add groupby from hooks
$parameters = array();
$reshook = $hookmanager->executeHooks('printFieldListGroupBy', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
$sql .= $hookmanager->resPrint;
$sql = preg_replace('/,\s*$/', '', $sql);
*/
...
/*
$parameters = array();
$reshook = $hookmanager->executeHooks('printFieldListHaving', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
...
$reshook = $hookmanager->executeHooks('printFieldListSearchParam', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
...
$newcardbutton = '';
// $newcardbutton .= dolGetButtonTitle($langs->trans('ViewList'), '', 'fa fa-bars imgforviewmode', $_SERVER["PHP_SELF"].'?id='.$object->id.'&mode=common'.preg_replace('/(&|\?)*mode=[^&]+/', '', $param), '', ((empty($mode) || $mode == 'common') ? 2 : 1), array('morecss'=>'reposition'));
// $newcardbutton .= dolGetButtonTitle($langs->trans('ViewKanban'), '', 'fa fa-th-list imgforviewmode', $_SERVER["PHP_SELF"].'?id='.$object->id.'&mode=kanban'.preg_replace('/(&|\?)*mode=[^&]+/', '', $param), '', ($mode == 'kanban' ? 2 : 1), array('morecss'=>'reposition'));
...
print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$elementPrerequis->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendElementPrerequisRef";
$modelmail = "elementprerequis";
$objecttmp = new ElementPrerequis($db);
$trackid = 'xxxx'.$elementPrerequis->id;
...
$parameters = array();
$reshook = $hookmanager->executeHooks('printFieldPreListTitle', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
if (empty($reshook)) {
...
foreach ($elementPrerequis->fields as $key => $val) {
	...
	if (!empty($arrayfields['t.'.$key]['checked'])) {
		...
		if (!empty($val['arrayofkeyval']) && is_array($val['arrayofkeyval'])) {
			...
			print $elementPrerequis->showInputField($val, $key, (isset($search[$key]) ? $search[$key] : ''), '', '', 'search_', $cssforfield.' maxwidth250', 1);
...
$reshook = $hookmanager->executeHooks('printFieldListOption', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
print $hookmanager->resPrint;
...
$reshook = $hookmanager->executeHooks('printFieldListTitle', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
print $hookmanager->resPrint;
/*if (!empty($arrayfields['anotherfield']['checked'])) {
...
if (isset($extrafields->attributes[$elementPrerequis->table_element]['computed']) && is_array($extrafields->attributes[$elementPrerequis->table_element]['computed']) && count($extrafields->attributes[$elementPrerequis->table_element]['computed']) > 0) {
	foreach ($extrafields->attributes[$elementPrerequis->table_element]['computed'] as $key => $val) {
		if (!is_null($val) && preg_match('/\$elementPrerequis/', $val)) {
			$needToFetchEachLine++; // There is at least one compute field that use $elementPrerequis
		}
	}
}
...
while ($i < $imaxinloop) {
	...
	// Store properties in $elementPrerequis
	$elementPrerequis->setVarsFromFetchObj($obj);

	/*
	$elementPrerequis->thirdparty = null;
	if ($obj->fk_soc > 0) {
		...
		$elementPrerequis->thirdparty = $companyobj;
	}*/
	...
	// 		$selected = 0;
	// 		if (in_array($elementPrerequis->id, $arrayofselected)) {
	// 			$selected = 1;
	// 		}
	// 	}
	// 	//print $elementPrerequis->getKanbanView('', array('thirdparty'=>$elementPrerequis->thirdparty, 'selected' => $selected));
	// 	print $elementPrerequis->getKanbanView('', array('selected' => $selected));
	// 	if ($i == ($imaxinloop - 1)) {
	// 		print '</div>';
	// 		print '</td></tr>';
	// 	}
	// } else {
		// Show line of result
		$j = 0;
		print '<tr data-rowid="'.$elementPrerequis->id.'" class="oddeven">';
		...
		if (getDolGlobalString('MAIN_CHECKBOX_LEFT_COLUMN')) {
			...
			if ($massactionbutton || $massaction) { // If we are in select mode (massactionbutton defined) or if we have already selected and sent an action ($massaction) defined
				...
				print '<input id="cb'.$elementPrerequis->id.'" class="flat checkforselect" type="checkbox" name="toselect[]" value="'.$elementPrerequis->id.'"'.($selected ? ' checked="checked"' : '').'>';
			}
			print '</td>';
			if (!$i) {
				$totalarray['nbfield']++;
		...
		foreach ($elementPrerequis->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.'.$key]['checked'])) {
				...
				if (preg_match('/tdoverflow/', $cssforfield) && !in_array($val['type'], array('ip', 'url')) && !is_numeric($elementPrerequis->$key)) {
				...
				if (!empty($val['isameasure']) && $val['isameasure'] == 1) {
					...
					$totalarray['val']['t.'.$key] += $elementPrerequis->$key;
				}
			}
		}
		// Extra fields
		...
		// Fields from hook
		$parameters = array('arrayfields'=>$arrayfields, 'object'=>$elementPrerequis, 'obj'=>$obj, 'i'=>$i, 'totalarray'=>&$totalarray);
		$reshook = $hookmanager->executeHooks('printFieldListValue', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
		print $hookmanager->resPrint;
...
$reshook = $hookmanager->executeHooks('printFieldListFooter', $parameters, $elementPrerequis, $action); // Note that $action and $elementPrerequis may have been modified by hook
print $hookmanager->resPrint;
