...
/**
 *  \file       autorisation_note.php
 *  \ingroup    formationhabilitation
 *  \brief      Tab for notes on Autorisation
 */
...
$hookmanager->initHooks(array('autorisationnote', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissionnote = $user->rights->formationhabilitation->autorisation->write; // Used by the include of actions_setnotes.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
	$permissionnote = 1;
...
$help_url = '';
$title = $langs->trans('Autorisation').' - '.$langs->trans("Notes");
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("Autorisation"), -1, $object->picto);
