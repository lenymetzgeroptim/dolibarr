...
/**
 *   	\file       uservolet_list.php
 *		\ingroup    formationhabilitation
 *		\brief      List page for uservolet
 */
...
require_once __DIR__.'/class/uservolet.class.php';
...
$permissiontoread = $user->hasRight('formationhabilitation', 'uservolet', 'readall');
$permissiontoadd = $user->hasRight('formationhabilitation', 'uservolet', 'write');
$permissiontodelete = $user->hasRight('formationhabilitation', 'uservolet', 'delete');
...
$title = $langs->trans("UserVolets");
//$help_url = "EN:Module_UserVolet|FR:Module_UserVolet_FR|ES:Módulo_UserVolet";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/formationhabilitation/uservolet_card.php', 1).'?id='.((int) $id));
	exit;
}
...
// $newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/formationhabilitation/uservolet_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
