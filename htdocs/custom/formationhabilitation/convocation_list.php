...
/**
 *   	\file       convocation_list.php
 *		\ingroup    formationhabilitation
 *		\brief      List page for convocation
 */
...
require_once __DIR__.'/class/convocation.class.php';
...
$permissiontoread = $user->hasRight('formationhabilitation', 'convocation', 'readall');
$permissiontoadd = $user->hasRight('formationhabilitation', 'convocation', 'write');
$permissiontodelete = $user->hasRight('formationhabilitation', 'convocation', 'delete');
...
$title = $langs->trans("Convocations");
//$help_url = "EN:Module_Convocation|FR:Module_Convocation_FR|ES:Módulo_Convocation";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/formationhabilitation/convocation_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/formationhabilitation/convocation_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
