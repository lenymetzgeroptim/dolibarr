...
/**
 *  \file       visitemedical_document.php
 *  \ingroup    formationhabilitation
 *  \brief      Tab for documents linked to VisiteMedical
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->formationhabilitation->multidir_output[$object->entity ? $object->entity : $conf->entity]."/visitemedical/".get_exdir(0, 0, 0, 1, $object);
}

// Permissions
// (There are several ways to check permission.)
$permissiontoread = $user->hasRight('formationhabilitation', 'visitemedical', 'read');
$permissiontoadd  = $user->hasRight('formationhabilitation', 'visitemedical', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
...
// Header
// ------
$title = $langs->trans("VisiteMedical")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("VisiteMedical"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='visitemedical/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'visitemedical/'.dol_sanitizeFileName($object->ref).'/';
