...
/**
 *   	\file       formation_card.php
 *		\ingroup    formationhabilitation
 *		\brief      Page to create/edit/view formation
 */
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/formationhabilitation/formation_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/formationhabilitation/formation_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	if($action == 'confirm_programmer_formation' && $confirm == 'yes' && $permissiontoaddline){
		if($lineid > 0 && $id > 0){
			...
			if (!$error) {
				$objectline->fetch($lineid);
				$objectline->status = User_formation::STATUS_CLOTUREE;
				$result == $objectline->update($user);
				...
				$objectline->date_formation = $date;
				$objectline->status = User_formation::STATUS_PROGRAMMEE;
				$objectline->cout_pedagogique = $object->cout;
                $objectline->cout_mobilisation = $user_static->thm * ($object->nombre_heure / 3600);
                $objectline->cout_total = $objectline->cout_pedagogique + $objectline->cout_mobilisation;
...
if ($action == 'create') {
	...
	print load_fiche_titre($langs->trans("NewObj", $langs->transnoentitiesnoconv("Formation")), '', 'fa-graduation-cap_fas_#1f3d89');
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Formation"), '', 'fa-graduation-cap_fas_#1f3d89');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("Formation"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteFormation'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	// Confirmation to validate
	if ($action == 'validate') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('ValidateFormation'), $langs->trans('ConfirmValidateFormation'), 'confirm_validate', '', 0, 1);
	}
	...
	// Common attributes
	$keyforbreak='periode_recyclage';	// We change column just before this field
	//unset($object->fields['fk_project']);				// Hide field already shown in banner
	//unset($object->fields['fk_soc']);					// Hide field already shown in banner
	include DOL_DOCUMENT_ROOT.'/custom/formationhabilitation/core/tpl/commonfields_view.tpl.php';
