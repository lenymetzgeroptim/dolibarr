...
// 	print $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans("ConfirmMassDraftDeletion"), $langs->trans("ConfirmMassDeletionQuestion", count($toselect)), "deleteline", null, '', 0, 200, 500, 1);
// }

if ($massaction == 'predelete' && $permissiontodeleteline) {
...
if ($massaction == 'preclose' && $permissiontoaddline) {
...
if ($massaction == 'prevalidate' && $permissiontovalidateline) {
	...
	foreach($unique_arr as $lineid) {
		$objectlinetmp->fetch($lineid);

		if($objectlinetmp->element == 'userautorisation') {
			$objectparentlinetmp->fetch($objectlinetmp->fk_autorisation);
			if(!in_array($objectlinetmp->fk_autorisation, $parentobjectid)) {
				$parentobjectid[] = $objectlinetmp->fk_autorisation;
			}
			else {
				setEventMessages("Impossible de valider plusieurs ligne d'une même autorisation", "Impossible de valider plusieurs ligne d'une même autorisation", 'errors');
				$error++;
				break;
			}
		}
		elseif($objectlinetmp->element == 'userhabilitation') {
			$objectparentlinetmp->fetch($objectlinetmp->fk_habilitation);
			if(!in_array($objectlinetmp->fk_habilitation, $parentobjectid)) {
				$parentobjectid[] = $objectlinetmp->fk_habilitation;
			}
			else {
				setEventMessages("Impossible de valider plusieurs ligne d'une même habilitation", "Impossible de valider plusieurs ligne d'une même habilitation", 'errors');
				$error++;
			}
		}
	...
	if(!$error) {
		// Texte pour les ref des autorisations/habilitations qui seront clôturées
		$objectToClose = $objectlinetmp->getObjectToClose(GETPOST('fk_user'), implode(',', $unique_arr), implode(',', $parentobjectid));
		$txt_objectToClose = '';
		foreach($objectToClose as $idobject => $refobject) {
			$txt_objectToClose .= $refobject.', ';
		}
		$txt_objectToClose = rtrim($txt_objectToClose, ', ');
