...
/**
 *   	\file       volettable_list.php
 *		\ingroup    formationhabilitation
 *		\brief      List page for volettable
 */
...
require_once __DIR__.'/class/volettable.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('formationhabilitation', 'volettable', 'write');
	$permissiontodelete = $user->hasRight('formationhabilitation', 'volettable', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("formationhabilitation")) {
	accessforbidden('Module formationhabilitation not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'VoletTable';
	$objectlabel = 'VoletTable';
...
$title = $langs->trans("VoletTables");
//$help_url = "EN:Module_VoletTable|FR:Module_VoletTable_FR|ES:Módulo_VoletTable";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/formationhabilitation/volettable_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-formationhabilitation page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/formationhabilitation/volettable_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendVoletTableRef";
$modelmail = "volettable";
$objecttmp = new VoletTable($db);
