...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('formationhabilitation', 'test', 'write');
	$permissiontodelete = $user->hasRight('formationhabilitation', 'test', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/formationhabilitation/test_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/formationhabilitation/test_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
