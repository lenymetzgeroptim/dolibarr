...
require_once __DIR__.'/class/charge.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('workload', 'charge', 'write');
	$permissiontodelete = $user->hasRight('workload', 'charge', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	$objectclass = 'Charge';
	$objectlabel = 'Charge';
	$uploaddir = $conf->workload->dir_output;
...
require_once DOL_DOCUMENT_ROOT.'/custom/workload/core/boxes/box_graph_plan_charge.php';
$graph = new box_graph_plan_charge($db, $param);
$graph->showBox();
...
$title = $langs->trans("Charges");
//$help_url = "EN:Module_Charge|FR:Module_Charge_FR|ES:Módulo_Charge";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/workload/charge_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/workload/charge_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendChargeRef";
$modelmail = "charge";
$objecttmp = new Charge($db);
