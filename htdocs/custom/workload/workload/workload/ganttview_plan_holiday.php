...
if($user->id == '412') {
	...
	foreach ($commArray as $commItem) {
		// Parcourir le deuxième tableau
		foreach ($absArray as $absItem) {
			// Si les IDs correspondent
			if ($commItem->id === $absItem->id) {
				// Ajouter ou mettre à jour les champs dans commArray
				$commItem->date_start = $absItem->date_start;
				$commItem->date_end = $absItem->date_end;
				$commItem->holidayref = $absItem->holidayref;
...
$data = array_merge($commArray, $propArray, $absArray);
...
foreach($ressources as $ressource) {
	$dates[] = $ressource->date_end;	
	// var_dump($ressource->date_end);
}
...
$title = $langs->trans("Projet : Collaborateurs - absence");
...
if (count($ressources) > 0) {
	...
	foreach ($ressources as $key => $val) {
		...
		$condition = $occupied == 'on' ? isset($val->avl) || (($val->idref !== "CO" || $val->ref !=="HL") && $val->fk_projet !== null) : $val->fk_projet !== null;
		if($condition) { 
			...
			}elseif($val->idref == "HL" && $val->status == 2){
				$members[$membercursor]['member_css'] = 'gtaskred';
			}elseif($val->idref == "HL" && $val->status == 3){
				$members[$membercursor]['member_css'] = 'gtaskredcomplete';
			}elseif($val->idref == "HL" && $val->status == 6){
				$members[$membercursor]['member_css'] = 'gtaskredcomplete';
			}
			...
			}elseif(isset($val->element_id) && $val->idref == "HL" && $val->status == 2) {
				$members[$membercursor]['member_name'] = $emmployee.'- Congé validé';
			}elseif(isset($val->element_id) && $val->idref == "HL" && $val->status == 3) {
				$members[$membercursor]['member_name'] = $emmployee.'- En appro 2';
			}elseif(isset($val->element_id) && $val->idref == "HL" && $val->status == 6) {
				$members[$membercursor]['member_name'] = $emmployee.'- En appro 1';
			}
			...
			if($val->idref === "HL"){
				$label = 'absence';
				$members[$membercursor]['member_resources'] = '<span title="Salarié absent prévu sur le projet">Du '.dol_print_date($val->date_start, '%d-%m-%Y').' au '.dol_print_date($val->date_end, '%d-%m-%Y').'</span>';
			}
