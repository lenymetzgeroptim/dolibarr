...
//if (g)
{
	...
	echo "/* g.AddTaskItem(new JSGantt.Intertem(member_id, 'label', 'start_date', 'end_date', 'css', 'link', milestone, 'Resources', Compl%, Group, Parent, Open, 'Dependency', 'label','note', g)); */\n";
	$level = 0;
	$tnums = count($members);
	...
	// 	return $a['member_member_id'] - $b['member_member_id'];
	// });
	...
	for ($tcursor = 0; $tcursor < $tnums; $tcursor++) {
		...
		if (empty($old_member_id) || $old_member_id != $t['member_member_id']) {
			...
			$tmpt = array(
				...
				'member_name'=>$membertmp->getNomUrl(-1, '', 0, 0, 24, 1, 'login', '', 1),
...
function constructGanttLine($tarr,$member, $member_dependencies, $level = 0, $member_id = null)
{
	...
	*/
	...
	$note = dol_concatdesc($note, $langs->trans("Plan de charge").' : '.($member['member_planned_workload'] ? convertSecondToTime($member['member_planned_workload'], 'allhourmin') : ''));
