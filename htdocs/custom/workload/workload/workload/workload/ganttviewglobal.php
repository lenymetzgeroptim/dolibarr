...
$title = $langs->trans("Collaborateurs : projets/devis");
...
<script>
	document.addEventListener("DOMContentLoaded", function () {
		...
		const monthWidth = 28; // En pixels, adapté à votre structure HTML
...
if (count($ressources) > 0) {
	...
	foreach ($ressources as $key => $val) {
		...
		$occupied == 'on' ? $condition = in_array($val->id, $employees) : $condition = ($val->element_id != null && $val->id > 0) || in_array($val->id, $availableusers);
		if($condition) { 
		// Commandes array are sorted by "commande, position, date"
			...
			if($val->idref === "CO") {
				$members[$membercursor]['member_name'] = '<span title="'.$val->projet_ref.' - '.$val->ref.' - '.'Projet / Commande">'.$refproj.' - '.$refcomm.'</span>';
			}elseif($val->idref === "PR"){
				$members[$membercursor]['member_name'] = '<span title="'.$val->projet_ref.' - '.$val->ref.' Projet / Devis">'.$refproj.' - '.$refpropal.'</span>';
			}elseif($val->idref == "PROJ"  && isset($val->ref)) {
				$members[$membercursor]['member_name'] = $refproj.' - '.'Projet';
			}
