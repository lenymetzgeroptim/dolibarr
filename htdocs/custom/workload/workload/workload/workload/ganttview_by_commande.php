...
$commArray = $rsc->getContactsAndOrders($arr_user, $date_start, $date_end, 'order');
...
// $data = array_merge($projArray);
$data = array_merge($commArray);
// foreach($projArray as $key => $proj) {
// var_dump($data);
// 	$projs[] = $proj->element_id;
// }
...
if (empty($date_end)) {
	...
	$date_end = dol_get_last_day(dol_print_date($datecommend, 'Y'));
}
...
$title = $langs->trans("Cde : collaborateurs - Projet");
...
<script>
	document.addEventListener("DOMContentLoaded", function () {
		...
		const monthWidth = 30; // En pixels, adapté à votre structure HTML
...
print '</td></tr>';
// Compétences
print '<tr><td>'.$langs->trans("Res. agence").'</td><td>';
print img_picto('', 'group', 'class="pictofixedwidth"');
...
//$listproj = $rsc->listProjectsContact();
...
if (count($ressources) > 0) {
	...
	foreach ($ressources as $key => $val) {
		...
		$occupied == 'on' ? $condition = !isset($val->id) : $condition = $val->element_id != null || $condition = $val->element_id != null;
		if($condition) { 
			// Commandes array are sorted by "commande, position, date"
			$member->fetch($val->id, '');
		
			$idparent = (!isset($val->element_id) ? $val->element_id : '-'.$val->element_id); // If start with -, id is a project id
			if(isset($val->fk_projet)) {
				$members[$membercursor]['member_id'] = $val->fk_projet;
			}
			
			$members[$membercursor]['member_alternate_id'] = ($membercursor + 1); // An id that has same order than position (required by ganttchart)
			$members[$membercursor]['member_member_id'] = $val->element_id;
			...
			? $emmployee =  $member->getNomUrl(2).' '.$member->login
			: $employee = $member->login;

			if(isset($val->id)) {
				$members[$membercursor]['member_name'] = '<span title="ressource dans les contacts de la commande">'.$emmployee.'</span> -'.$refproj;
			}else{
				$members[$membercursor]['member_name'] = '<span title="Pas de ressource sur cette commande"> X </span> -'.$refproj;
			}
			...
			$pdc_proj = !empty($listofproject) ? sizeof($listofproject) : 0;
			$missing1 = sizeof($listofproject) - $val->pdc_etp_cde;
			$missing2 = $val->pdc_etp_proj - $val->pdc_etp_cde;
			$pdc_etp_cde = floatval($val->pdc_etp_cde);
			var_dump($missing1);

			if(!isset($val->id)) {
				$missing2 > 0
				? $members[$membercursor]['member_resources'] = '<span class="classfortooltip badge badge-warning" title="'.$missing2.' manquant(s) (commande / projet)"><i class="fa fa-exclamation-triangle"></i>'.$pdc_etp_cde.' / '.$val->pdc_etp_proj.'</span>'
				: $members[$membercursor]['member_resources'] = '<span class="classfortooltip badge badge-info" title="'.$missing2.' manquant(s) (commande / projet)">'.$pdc_etp_cde.' / '.$val->pdc_etp_proj.'</span>';
			} 
		
			if(isset($val->id)) {
				$missing1 > 0
				? $members[$membercursor]['member_resources'] = '<span class="classfortooltip badge badge-warning" title="'.$missing1.' manquant(s) (commande / projet)"><i class="fa fa-exclamation-triangle"></i>'.$pdc_etp_cde.' / '.$pdc_proj.'</span>'
				: $members[$membercursor]['member_resources'] = '<span class="classfortooltip badge badge-info" title="'.$missing1.' manquant(s) (commande / projet)">'.$pdc_etp_cde.' / '.$pdc_proj.'</span>';
			}
	...
	if (!empty($conf->use_javascript_ajax)) {
		...
		include_once DOL_DOCUMENT_ROOT.'/custom/workload/ganttchart_by_commande.inc.php';
	...
	print '<div class="opacitymedium">'.$langs->trans("NoOrder").'</div>';
}
