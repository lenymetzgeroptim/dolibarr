...
//if (g)
{
	...
	for ($tcursor = 0; $tcursor < $tnums; $tcursor++) {
		...
		if (empty($old_member_id) || $old_member_id != $t['member_member_id']) {
			// Break on project, create a fictive commande for project id $t['member_member_id']
			// $membertmp = new Commande($db);
			$membertmp = new Project($db);
...
function constructGanttLine($tarr,$member, $member_dependencies, $level = 0, $member_id = null)
{
	...
	if (!isset($member["member_idref"])) {
		...
		$link = DOL_URL_ROOT.'/projet/contact.php?withproject=1&id='.$member["member_member_id"];
	}
