...
//             zoomLevel += zoomStep;
//             ganttContainer.style.transform = `scale(${zoomLevel})`;
//         }
//     });
...
(async function initialize() {
    try {
        ...
        // Pendant que tout se charge, afficher les absences dès qu'on les a
        dataPromise.then(({ ressourcesAbs }) => {
            if (Array.isArray(ressourcesAbs)) {
                updateGanttChartAbs(ressourcesAbs);
            }
            showLoader(false);
        });
        ...
        const [data, filterData] = await Promise.all([dataPromise, filterPromise]);
