 ...
 *  \file       ot_contact.php
 *  \ingroup    ot
 *  \brief      Tab for contacts linked to Ot
 */
...
$hookmanager->initHooks(array('otcontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->ot->ot->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Ot')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Ot"), -1, $object->picto);
