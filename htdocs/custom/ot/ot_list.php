...
require_once __DIR__.'/class/ot.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('ot', 'ot', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("ot")) {
	accessforbidden('Module ot not enabled');
}
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/ot/ot_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-ot bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/ot/ot_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
foreach ($object->fields as $key => $val) {
	...
	if (!empty($arrayfields['t.'.$key]['checked'])) {
		...
		if (!empty($val['arrayofkeyval']) && is_array($val['arrayofkeyval'])) {
			if ($key == 'status') {
				$arrayofkeyval = array(''=>'') + $val['arrayofkeyval'];
				print $form->selectarray('search_'.$key, $arrayofkeyval, (isset($search[$key]) ? $search[$key] : ''), $val['notnull'], 0, 0, '', 1, 0, 0, '', 'maxwidth100'.($key == 'status' ? ' search_status width100 onrightofpage' : ''), 1);
			} else {
