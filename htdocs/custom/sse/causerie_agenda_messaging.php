...
/**
 *  \file       causerie_agenda.php
 *  \ingroup    sse
 *  \brief      Tab of events on Causerie
 */
...
$hookmanager->initHooks(array('causerieagenda', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->sse->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->rights->sse->causerie->read;
	$permissiontoadd = $user->rights->sse->causerie->write;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
}
...
$form = new Form($db);
 ...
 print dol_get_fiche_head($head, 'agenda', $langs->trans("Causerie"), -1, $object->picto);

 // Card 
$linkback = '<a href="'.dol_buildpath('/sse/causerie_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
...
if (!empty($object->id)) {
	...
	$messagingUrl = DOL_URL_ROOT.'/custom/sse/causerie_agenda_messaging.php?id='.$object->id;
	...
	// $messagingUrl = DOL_URL_ROOT.'/custom/sse/causerie_agenda.php?id='.$object->id;
	...
	print_barre_liste($langs->trans("Action sur la Causerie"), 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlcenter, '', 0, 1, 0);
	...
	show_actions_messaging_causerie_custom($conf, $langs, $db, $object, null, 0, $actioncode, '', $filters, $sortfield, $sortorder);
}
...
// 		//print load_fiche_titre($langs->trans("ActionsOnCauserie"), '', '');
