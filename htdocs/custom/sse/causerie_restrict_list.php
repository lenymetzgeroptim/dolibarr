<style>
    ...
    progress::-webkit-progress-value {
        background: #0064A2; /* Bleu Dolibarr */
        border-radius: 5px;
    }
...
/**
 *   	\file       causerie_list.php
 *		\ingroup    sse
 *		\brief      List page for causerie
 */
...
// load sse libraries
require_once __DIR__.'/class/causerie.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'causerielist'; // To manage different context of search
...
$hookmanager->initHooks(array('causerielist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->sse->causerie->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Causerie';
	$objectlabel = 'Causerie';
...
//$help_url="EN:Module_Causerie|FR:Module_Causerie_FR|ES:Módulo_Causerie";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("Causeries"));
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/sse/causerie_card.php', 1).'?id='.$id);
	exit;
}
...
print '
	<div class="info-message" style="margin-top: 20px; padding: 15px; border-left: 4px solid #007bff; background-color: #e9f7fe; border-radius: 5px;">
			<p style="font-size: 14px; line-height: 1.6; color: #333;">
				Cet espace vous permet de visualiser les causeries que vous avez créées ou pour lesquelles vous avez été désigné comme <strong>animateur</strong>.
			</p>
		</div>
';
...
$topicmail = "SendCauserieRef";
$modelmail = "causerie";
$objecttmp = new Causerie($db);
