...
foreach ($object->fields as $key => $val) {
	...
	if (in_array($val['type'], array('int', 'integer')) && $key != 'p_year') {
		$value = GETPOST($key, 'int');
	} elseif (in_array($val['type'], array('int', 'integer')) && $key == 'p_year') {
			$value =  $formother->select_year($year ? $year : -1, 'p_year', 20, 10, 10);
