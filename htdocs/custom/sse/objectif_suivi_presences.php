...
if ($enablepermissioncheck) {
	$permissiontoread = $user->rights->sse->goal->follow;
	$permissiontoadd = $user->rights->sse->goal->follow;
	$permissiontodelete = $user->rights->sse->goal->follow;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!$user->rights->sse->goal->follow) accessforbidden();
...
$help_url = '';
$title = $langs->trans("Visualisation des objectifs");
$morejs = array();
$morecss = array();
...
// $sql .= ' ORDER BY t.p_year DESC';
$sql .= " ORDER BY 
    t.p_year = YEAR(NOW()) DESC, 
    CASE 
        WHEN t.p_year = YEAR(NOW()) THEN t.nb_accomplished 
    END DESC, 
    t.p_year DESC, 
    t.nb_accomplished DESC,
	t.nbcauserie ASC";
...
// Graphs test
if($num == 0) {
	print '<div style="margin-top: 20px; padding: 15px; border-left: 4px solid #dc3545; background-color: #f8d7da; border-radius: 5px;">
    <p style="font-size: 14px; line-height: 1.6; color: #721c24;">
        <strong><span class="fa fa-exclamation-triangle" style="margin-right: 8px; color: #dc3545;"></span>Attention :</strong> Aucun objectif n\'est défini ou aucune causerie n\'est associée pour cette période.<br><br>
        Veuillez configurer un objectif pour activer le suivi des performances et des statistiques. Sans objectif défini, les graphiques et indicateurs (liés aux causeries des participants) ne seront pas disponibles.<br><br>
        Seuls les salariés actifs ou ayant été actifs durant cette période seront inclus dans les objectifs correspondants.
    </p>
</div>';
...
if (GETPOSTISSET('search_p_year')) {
	$year = GETPOST('search_p_year'); 
}

if (GETPOSTISSET('search_fk_user')) {
	$userid = GETPOST('search_fk_user');
}
if((!isset($year) || $year == '') && isset($userid) && $userid !== '-1') {
	$year = 'all';
}elseif(!isset($year) || $year == '' && ($userid == null || $userid == '-1')) {
	$year = date('Y');
}
...
// var_dump($year, $userid);
print '<div class="fichecenter"><div class="fichethirdleft">';

$tmp = $object->getCauserieByThemeGoalPieChart($year, $userid);
if ($tmp) {
	print $tmp;
	//print '<br>';
}
...
$tmp = $object->getCauserieByMonthChart($year, $userid);
...
print '
	<div class="info-message" style="margin-top: 20px; padding: 15px; border-left: 4px solid #007bff; background-color: #e9f7fe; border-radius: 5px;">
		<p style="font-size: 14px; line-height: 1.6; color: #333;">
			Les graphiques sont filtrés par participant pour afficher la répartition de sa participation par thème et par année (utiliser la loop). Si aucun participant n\'est sélectionné et si l\'année n\'est pas spécifiée, les graphiques afficheront par défaut l\'année en cours ainsi que le nombre de causeries par thème.
			Les autres filtres de la liste ne s\'appliquent pas sur les graphiques. 
		</p>
	</div>
';
