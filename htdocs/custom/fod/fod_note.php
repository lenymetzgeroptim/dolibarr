...
$langs->loadLangs(array("fod@fod", "companies"));
...
$hookmanager->initHooks(array('fodnote', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->fod->multidir_output[$object->entity]."/".$object->id;
}
...
$permissionnote = $object->fk_user_rsr == $user->id || $object->fk_user_raf == $user->id || $object->fk_user_pcr == $user->id || $user->admin || $userRA || $userPCR; // Used by the include of actions_setnotes.inc.php
$permissiontoadd = $user->rights->fod->fod->write; // Used by the include of actions_addupdatedelete.inc.php
...
$help_url = '';
llxHeader('', $langs->trans('Fod'), $help_url);
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("Fod"), -1, $object->picto);
