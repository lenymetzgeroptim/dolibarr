...
/**
 *  \file       fod_document.php
 *  \ingroup    fod
 *  \brief      Tab for documents linked to Fod
 */
...
$langs->loadLangs(array("fod@fod", "companies", "other", "mails"));
...
$hookmanager->initHooks(array('foddocument', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->fod->multidir_output[$object->entity ? $object->entity : $conf->entity]."/fod/".get_exdir(0, 0, 0, 1, $object);
}
...
$permissiontoread = 1;
$permissiontoadd = 1; // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
...
$title = $langs->trans("Fod").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'document', $langs->trans("Fod"), -1, $object->picto);
	...
	$permissiontoadd = 1;
	//$permtoedit = $user->rights->fod->fod->write;
	$permtoedit = 1;
	$param = '&id='.$object->id;

	//$relativepathwithnofile='fod/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile = 'fod/'.dol_sanitizeFileName($object->ref).'/';
