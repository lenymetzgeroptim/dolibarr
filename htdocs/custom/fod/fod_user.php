...
if ($action == 'adddataintervenant' && $user->id == $user_fiche->id) { // Add data intervenant 
	...
	if (!$error) {
		...
		$data_intervenant->fk_user = $user->id;
...
if (($action == 'updateline') && !$cancel && $user->id == $user_fiche->id) { // Update ligne du tableau
	...
	if (!$error) {
		if($lineid > 0){
			...
			$data_intervenant->fetch($lineid);
			$data_intervenant->dose = GETPOST("dose", 'double');
			$data_intervenant->niv_contamination = GETPOST("niv_contamination", 'int');
			$data_intervenant->portique = GETPOST("portique", 'int');
			$data_intervenant->description = GETPOST("description", "text");
			$data_intervenant->date = $date;
			$data_intervenant->fk_user = $userid;
...
if($userid > 0){
	...
	if($user->id != $userid && $user->admin == 0 && !$user->rights->fod->data_intervenant->readAll){
