...
if ($action == 'adddataintervenant' && $permissionToAddDataIntervenant) { // Ajouter ligne du tableau 
	...
	if (!$error) {
		...
		if (empty($projectstatic->statut)) {
			...
			$dataintervenant->fk_user = GETPOST("userid", 'int');
...
if (($action == 'updateline') && !$cancel && $permissionToModifyDataIntervenant) { // Modifier lugne du tableau
	...
	if (!$error) {
		if($dataid > 0){
			...
			$dataintervenant->fetch($dataid);
			$dataintervenant->dose = GETPOST("dose", 'double');
			$dataintervenant->niv_contamination = GETPOST("niv_contamination", 'int');
			$dataintervenant->portique = GETPOST("portique", 'int');
			$dataintervenant->description = GETPOST("description", "text");
			$dataintervenant->date = dol_mktime(12, 0, 0, GETPOST("timelinemonth", 'int'), GETPOST("timelineday", 'int'), GETPOST("timelineyear", 'int'));
			$dataintervenant->fk_user = GETPOST("userid", 'int');
