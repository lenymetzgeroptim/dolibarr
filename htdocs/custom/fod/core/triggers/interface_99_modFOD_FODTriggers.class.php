	...
	public function runTrigger($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		switch ($action) {
			...
			case 'FOD_CLOTURE':
				...
				$urlwithroot = $urlwithouturlroot.DOL_URL_ROOT; // This is to use external domain name found into config file
				$link = '<a href="'.$urlwithroot.'/custom/fod/fod_card.php?id='.$object->id.'">'.$object->ref.'</a>';
				$link12mois = '<a href="'.$urlwithroot.'/custom/fod/dataintervenant_list.php'.'">'.'12 mois'.'</a>';
			...
			case 'DATAINTERVENANT_CREATE':
				...
				if(($user_->getDoseMoisActuelle() > $user_->getCddmensuelle()) && (round($user_->getDoseMoisActuelle()-$object->dose, 3) <= $user_->getCddmensuelle())){
					...
					$urlwithouturlroot = preg_replace('/'.preg_quote(DOL_URL_ROOT, '/').'$/i', '', trim($dolibarr_main_url_root));
					$urlwithroot = $urlwithouturlroot.DOL_URL_ROOT; // This is to use external domain name found into config file
					$link = '<a href="'.$urlwithroot.'/custom/fod/dataintervenant_list.php?search_fk_user='.$user_->id.'">'.$user_->firstname.' '.$user_->lastname.'</a>';
