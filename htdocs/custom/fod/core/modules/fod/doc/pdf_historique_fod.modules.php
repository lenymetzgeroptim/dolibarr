 ...
 */
class pdf_historique_fod extends ModelePDFFod
{
	/**
		...
		*/
		...
		if ($conf->fod->dir_output.'/fod') {
			...
			if ($object->specimen) {
				...
				$file = $dir."/".$objectref."_historique.pdf";
			}
			if (!file_exists($dir)) {
				if (dol_mkdir($dir) < 0) {
			...
			if (file_exists($dir)) {
				...
				// Create pdf instance
				$pdf = pdf_getInstance_custom($this->format);
				...
				// 	$pdf->writeHTML($ligne);
				// }
				$pdf->writeHTML($object->historique);
