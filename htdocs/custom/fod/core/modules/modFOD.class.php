	...
	public function __construct($db)
	{
		...
		$this->tabs = array(
		    'data'=>'project:+FOD1:FOD:fod@fod:$object->array_options["options_fod"]:/custom/fod/fod_projet.php?projectid=__ID__',
		            'user:+FOD2:FOD:fod@fod:$object->array_options["options_cat_med"]&&$object->array_options["options_cat_med"]!=3:/custom/fod/fod_user.php?userid=__ID__',
		...
		*/
		...
		$this->rights[$r][4] = 'dataintervenant';
		$this->rights[$r][5] = 'write'; // In php code, permission will be checked by test if ($user->rights->fod->dataintervenant->write)
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1); // Permission id (must not be already used)
		$this->rights[$r][1] = "Supprimer des données d'intervenant sur n'importe qu'elle FOD"; // Permission label
		$this->rights[$r][4] = 'dataintervenant';
		$this->rights[$r][5] = 'delete'; // In php code, permission will be checked by test if ($user->rights->fod->dataintervenant->delete)
		$r++;*/
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1); // Permission id (must not be already used)
		$this->rights[$r][1] = "Lire toutes les données des intervenants (PCR)"; // Permission label
		$this->rights[$r][4] = 'dataintervenant';
		$this->rights[$r][5] = 'readAll'; // In php code, permission will be checked by test if ($user->rights->fod->dataintervenant->readAll)
		$r++;
        ...
        $this->menu[$r++]=array(
			...
			'mainmenu'=>'fod',
			'leftmenu'=>'Data',
			'url'=>'/fod/dataintervenant_list.php',
			'langs'=>'fod@fod',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000+$r,
			'enabled'=>'$conf->fod->enabled',  // Define condition to show or hide menu entry. Use '$conf->fod->enabled' if entry must be visible if module is enabled.
			'perms'=>'',			                // Use 'perms'=>'$user->rights->fod->level1->level2' if you want your menu with a permission rules
		...
		$this->menu[$r++]=array(
			...
			'position'=>1000+$r,
			'enabled'=>'$conf->fod->enabled',  // Define condition to show or hide menu entry. Use '$conf->fod->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms'=>'$user->rights->fod->dataintervenant->readAll',			                // Use 'perms'=>'$user->rights->fod->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
		...
		$this->export_sql_end[$r] .= ' FROM '.MAIN_DB_PREFIX.'fod_dataintervenant as t';
