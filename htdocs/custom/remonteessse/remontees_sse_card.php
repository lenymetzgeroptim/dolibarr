...
/**
 *   	\file       remontees_sse_card.php
 *		\ingroup    remonteessse
 *		\brief      Page to create/edit/view remontees_sse
 */
...
dol_include_once('/remonteessse/class/remontees_sse.class.php');
dol_include_once('/remonteessse/lib/remonteessse_remontees_sse.lib.php');
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'remontees_ssecard'; // To manage different context of search
$backtopage = GETPOST('backtopage', 'alpha');
$backtopageforcancel = GETPOST('backtopageforcancel', 'alpha');
$lineid   = GETPOST('lineid', 'int');

// Initialize technical objects
$object = new Remontees_sse($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction = $conf->remonteessse->dir_output.'/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('remontees_ssecard', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	$permissionpriseencompte = $user->rights->remonteessse->remontees_sse->prisencompte;
	$permissiontraitement = $permissionpriseencompte;
	$permissiontocloture = $permissionpriseencompte;
	$permissiontoread = $user->rights->remonteessse->remontees_sse->read_all || $object->fk_user == $user->id;
	$permissiontoadd = $user->rights->remonteessse->remontees_sse->read; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->rights->remonteessse->remontees_sse->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT && $object->fk_user == $user->id);
	$permissionnote = $user->rights->remonteessse->remontees_sse->read; // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->rights->remonteessse->remontees_sse->read; // Used by the include of actions_dellink.inc.php
...
$upload_dir = $conf->remonteessse->multidir_output[isset($object->entity) ? $object->entity : 1].'/remontees_sse';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/remonteessse/remontees_sse_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/remonteessse/remontees_sse_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'REMONTEESSSE_REMONTEES_SSE_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	$triggersendname = 'REMONTEESSSE_REMONTEES_SSE_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_REMONTEES_SSE_TO';
	$trackid = 'remontees_sse'.$object->id;
...
$title = $langs->trans("Remontees_sse");
$help_url = '';
...
if ($action == 'create') {
	...
	print load_fiche_titre($langs->trans("NewObject2", $langs->transnoentitiesnoconv("Remontees_sse")), '', 'object_'.$object->picto);
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Remontees_sse"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	$head = remontees_ssePrepareHead($object);
	print dol_get_fiche_head($head, 'card', $langs->trans("Remontees_sse"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteRemontees_sse'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	// Confirmation validate (collaborateur)
	if ($action == 'validate') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('ValidateRemontees_sse'), $langs->trans('ConfirmValidateRemontees_sse'), 'confirm_validate', '', 0, 1);
	}
	// Confirmation prise en compte (QSE)
	if ($action == 'priseencompte') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('PriseEnCompteRemontees_sse'), $langs->trans('ConfirmPriseEnCompteRemontees_sse'), 'confirm_priseencompte', '', 0, 1);
	}
	// Confirmation annulation (QSE)
	if ($action == 'priseencompte_annulation') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('CancelRemontees_sse'), $langs->trans('ConfirmCancelRemontees_sse'), 'confirm_priseencompte_annulation', '', 0, 1);
	}
	// Confirmation clotûre (QSE)
	if ($action == 'cloture' && !empty($object->date_traitement) && !empty($object->detail_traitement)) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('ClotureRemontees_sse'), $langs->trans('ConfirmClotureRemontees_sse'), 'confirm_cloture', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/remonteessse/remontees_sse_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
