...
foreach ($object->fields as $key => $val) {
	...
	elseif ($val['type'] == 'date') {
		$now = dol_now();
		$value = dol_mktime(0, 0, 0, GETPOST($key.'month', 'int'), GETPOST($key.'day', 'int'), GETPOST($key.'year', 'int')) ? dol_mktime(0, 0, 0, GETPOST($key.'month', 'int'), GETPOST($key.'day', 'int'), GETPOST($key.'year', 'int')) : $object->$key;
