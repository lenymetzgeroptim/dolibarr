...
class modRemonteesSSE extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		// );
		$this->const = array(
		    1 => array('MAIN_AGENDA_ACTIONAUTO_REMONTEESSSE_CREATE', 'chaine', '1', '', 0),
		    2 => array('MAIN_AGENDA_ACTIONAUTO_REMONTEESSSE_VALIDATE', 'chaine', '1', '', 0),
		    3 => array('MAIN_AGENDA_ACTIONAUTO_REMONTEESSSE_PRISEENCOMPTE', 'chaine', '1', '', 0),
		    4 => array('MAIN_AGENDA_ACTIONAUTO_REMONTEESSSE_ANNULATION', 'chaine', '1', '', 0),
		    5 => array('MAIN_AGENDA_ACTIONAUTO_REMONTEESSSE_CLOTURE', 'chaine', '1', '', 0),
			6 => array('REINITIALISATION_REMONTEESSSE', 'yesno', '0', '', 0)
		    );
		...
		$this->dictionaries = array(
			...
			'tabname'=>array(MAIN_DB_PREFIX."lieux_activites", MAIN_DB_PREFIX."genres_remonteessse"),
			// Label of tables
			'tablib'=>array("Sites d'activités", "Genres Rémontées Q3SE/RP"),
			// Request to select fields
			'tabsql'=>array('SELECT f.rowid as rowid, f.nom, f.active FROM '.MAIN_DB_PREFIX.'lieux_activites as f', 'SELECT f.rowid as rowid, f.nom, f.active FROM '.MAIN_DB_PREFIX.'genres_remonteessse as f'),
			// Sort order
		...
		*/
		...
		$this->cronjobs = array(
			 0 => array(
			     ...
			     'jobtype' => 'method',
			     'class' => '/remonteessse/class/remonteessse.class.php',
			     'objectname' => 'Remonteessse',
		...
		$this->rights[$r][4] = 'remonteessse';
		$this->rights[$r][5] = 'read_all'; // In php code, permission will be checked by test if ($user->rights->remonteessse->remonteessse->read)
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1); // Permission id (must not be already used)
		$this->rights[$r][1] = 'Créer/Modifier des Remontées Q3SE/RP'; // Permission label
		$this->rights[$r][4] = 'remonteessse';
		$this->rights[$r][5] = 'read'; // In php code, permission will be checked by test if ($user->rights->remonteessse->remonteessse->write)
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1); // Permission id (must not be already used)
		$this->rights[$r][1] = 'Supprimer des Remontéees Q3SE/RP'; // Permission label
		$this->rights[$r][4] = 'remonteessse';
		$this->rights[$r][5] = 'delete'; // In php code, permission will be checked by test if ($user->rights->remonteessse->remonteessse->delete)
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1); // Permission id (must not be already used)
		$this->rights[$r][1] = 'Prise en compte des Remontées Q3SE/RP'; // Permission label
		$this->rights[$r][4] = 'remonteessse';
		$this->rights[$r][5] = 'prisencompte'; // In php code, permission will be checked by test if ($user->rights->remonteessse->remonteessse->read)
		$r++;
        ...
        $this->menu[$r++]=array(
            ...
            'enabled'=>'$conf->remonteessse->enabled',
            // Use 'perms'=>'$user->rights->remonteessse->level1->level2' if you want your menu with a permission rules
            'perms'=>'$user->rights->remonteessse->remonteessse->read',
            'target'=>'',
            // 0=Menu for internal users, 1=external users, 2=both
            'user'=>2
        );
		/* END MODULEBUILDER LEFTMENU REMONTEESSSE */



		// Exports profiles provided by this module
		$r = 1;
		/* BEGIN MODULEBUILDER EXPORT REMONTEESSSE */
		/*
		$langs->load("remonteessse@remonteessse");
		$this->export_code[$r]=$this->rights_class.'_'.$r;
		$this->export_label[$r]='RemonteesSseLines';	// Translation key (used only if key ExportDataset_xxx_z not found)
		$this->export_icon[$r]='remonteessse@remonteessse';
		// Define $this->export_fields_array, $this->export_TypeFields_array and $this->export_entities_array
		$keyforclass = 'RemonteesSse'; $keyforclassfile='/remonteessse/class/remonteessse.class.php'; $keyforelement='remonteessse@remonteessse';
		include DOL_DOCUMENT_ROOT.'/core/commonfieldsinexport.inc.php';
		//$this->export_fields_array[$r]['t.fieldtoadd']='FieldToAdd'; $this->export_TypeFields_array[$r]['t.fieldtoadd']='Text';
		//unset($this->export_fields_array[$r]['t.fieldtoremove']);
		//$keyforclass = 'RemonteesSseLine'; $keyforclassfile='/remonteessse/class/remonteessse.class.php'; $keyforelement='remonteessseline@remonteessse'; $keyforalias='tl';
		//include DOL_DOCUMENT_ROOT.'/core/commonfieldsinexport.inc.php';
		$keyforselect='remonteessse'; $keyforaliasextra='extra'; $keyforelement='remonteessse@remonteessse';
		include DOL_DOCUMENT_ROOT.'/core/extrafieldsinexport.inc.php';
		//$keyforselect='remonteessseline'; $keyforaliasextra='extraline'; $keyforelement='remonteessseline@remonteessse';
		//include DOL_DOCUMENT_ROOT.'/core/extrafieldsinexport.inc.php';
		//$this->export_dependencies_array[$r] = array('remonteessseline'=>array('tl.rowid','tl.ref')); // To force to activate one or several fields if we select some fields that need same (like to select a unique key if we ask a field of a child to avoid the DISTINCT to discard them, or for computed field than need several other fields)
		//$this->export_special_array[$r] = array('t.field'=>'...');
		//$this->export_examplevalues_array[$r] = array('t.field'=>'Example');
		//$this->export_help_array[$r] = array('t.field'=>'FieldDescHelp');
		$this->export_sql_start[$r]='SELECT DISTINCT ';
		$this->export_sql_end[$r]  =' FROM '.MAIN_DB_PREFIX.'remonteessse as t';
		//$this->export_sql_end[$r]  =' LEFT JOIN '.MAIN_DB_PREFIX.'remonteessse_line as tl ON tl.fk_remonteessse = t.rowid';
		$this->export_sql_end[$r] .=' WHERE 1 = 1';
		$this->export_sql_end[$r] .=' AND t.entity IN ('.getEntity('remonteessse').')';
		$r++; */
		/* END MODULEBUILDER EXPORT REMONTEESSSE */

		// Imports profiles provided by this module
		$r = 1;
		/* BEGIN MODULEBUILDER IMPORT REMONTEESSSE */
		/*
		 $langs->load("remonteessse@remonteessse");
		 $this->export_code[$r]=$this->rights_class.'_'.$r;
		 $this->export_label[$r]='RemonteesSseLines';	// Translation key (used only if key ExportDataset_xxx_z not found)
		 ...
		 $this->export_sql_end[$r] .=' AND t.entity IN ('.getEntity('remonteessse').')';
		 $r++; */
		/* END MODULEBUILDER IMPORT REMONTEESSSE */
	}
	...
	public function init($options = '')
	{
		...
		$myTmpObjects = array();
		$myTmpObjects['RemonteesSse'] = array('includerefgeneration'=>0, 'includedocgeneration'=>0);

		foreach ($myTmpObjects as $myTmpObjectKey => $myTmpObjectArray) {
			if ($myTmpObjectKey == 'RemonteesSse') {
				continue;
			}
			if ($myTmpObjectArray['includerefgeneration']) {
				$src = DOL_DOCUMENT_ROOT.'/install/doctemplates/'.$moduledir.'/template_remonteessses.odt';
				$dirodt = DOL_DATA_ROOT.'/doctemplates/'.$moduledir;
				$dest = $dirodt.'/template_remonteessses.odt';
