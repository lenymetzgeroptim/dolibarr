...
/**
 * \file        class/remontees_sse.class.php
 * \ingroup     remonteessse
 * \brief       This file is a CRUD class file for Remontees_sse (Create/Read/Update/Delete)
 */
...
/**
 * Class for Remontees_sse
 */
class Remontees_sse extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'remontees_sse';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'remonteessse_remontees_sse';
	...
	/**
	 * @var string String with name of icon for remontees_sse. Must be the part after the 'object_' into object_remontees_sse.png
	 */
	...
	public $fields=array(
		...
		'fk_user' =>array('type'=>'integer:User:user/class/user.class.php:0:(statut:=:1)', 'label'=>'UserRemontee', 'enabled'=>1, 'visible'=>1, 'position'=>22,),
		'date_remontee' => array('type'=>'date', 'label'=>'DateRemontee', 'enabled'=>'1', 'position'=>21, 'notnull'=>1, 'visible'=>1,),
		'genre' => array('type'=>'sellist:genres_remontees_sse:nom:rowid::active=1', 'label'=>'Genre', 'enabled'=>'1', 'position'=>30, 'notnull'=>0, 'visible'=>1,),
		...
		'fk_project' => array('type'=>"integer:Project:projet/class/project.class.php:0:(fk_statut:=:1)", 'label'=>'Projet', 'enabled'=>'1', 'position'=>23, 'notnull'=>0, 'visible'=>1,),
		// 'fk_project' => array('type'=>"integer:Project:projet/class/project.class.php:0:(rowid:IN:SELECT FROM llx_element_contact as ec WHERE:ec.element_id:=:t.rowid)", 'label'=>'Projet', 'enabled'=>'1', 'position'=>23, 'notnull'=>0, 'visible'=>1,),
		// 'fk_project' => array('type'=>'integer:Project:projet/class/project.class.php:0:fk_statut=1 AND rowid IN (SELECT DISTINCT p.rowid FROM llx_projet as p, llx_element_contact as ec WHERE p.entity IN (1) AND ec.element_id = p.rowid AND ec.fk_c_type_contact IN (1031119,1031121,1031114,1031117,161,160,1031120,1031113,1031118) AND ec.fk_socpeople = __USER_ID__)', 'label'=>'Projet', 'enabled'=>'1', 'position'=>23, 'notnull'=>0, 'visible'=>1,),
	...
	//  */
	// public $table_element_line = 'remonteessse_remontees_sseline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_remontees_sse';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Remontees_sseline';
	...
	//  */
	// protected $childtablesoncascade = array('remonteessse_remontees_ssedet');

	// /**
	//  * @var Remontees_sseLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		if ($user->rights->remonteessse->remontees_sse->read) {
			// $this->fields['myfield']['visible'] = 1;
			// $this->fields['myfield']['noteditable'] = 0;
			$this->fields['fk_project']['type'] = 'integer:Project:projet/class/project.class.php:0:(rowid:IN:'.implode(',', $ids).')';
		 }
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->remonteessse->remontees_sse->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->remonteessse->remontees_sse->remontees_sse_advance->validate))))
		 {
		...
		if (!$error && !$notrigger) {
			// Call trigger
			$result = $this->call_trigger('REMONTEES_SSE_VALIDATE', $user);
			if ($result < 0) {
				$error++;
			}
			// End call triggers
	...
	public function priseencompte($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('REMONTEES_SSE_PRISEENCOMPTE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->oldref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'remonteessse/remontees_sse/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'remonteessse/remontees_sse/".$this->db->escape($this->oldref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->remonteessse->dir_output.'/remontees_sse/'.$oldref;
				$dirdest = $conf->remonteessse->dir_output.'/remontees_sse/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->remonteessse->dir_output.'/remontees_sse/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function priseencompte_annulation($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('REMONTEES_SSE_ANNULATION', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				// Now we rename also files into index
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'remontees_sse/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'remontees_sse/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++; $this->error = $this->db->lasterror();
				}
	...
	public function cloture($user, $notrigger = 0)
	{
		...
		if (!$error && !$notrigger) {
			// Call trigger
			$result = $this->call_trigger('REMONTEES_SSE_CLOTURE', $user);
			if ($result < 0) {
				$error++;
			}
			// End call triggers
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'REMONTEES_SSE_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'REMONTEES_SSE_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'REMONTEES_SSE_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		if($this->fk_user != $user->id && !$user->rights->remonteessse->remontees_sse->read_all){
			$option = 'nolink';
			$notooltip = 1;
		}

		$label = img_picto('', 'remonteesSSE_16@remonteessse').' <u>'.$langs->trans("Remontees_sse").'</u>';
		...
		$url = dol_buildpath('/remonteessse/remontees_sse_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowRemontees_sse");
		...
		$hookmanager->initHooks(array('remontees_ssedao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new Remontees_sseLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_remontees_sse = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->REMONTEESSSE_REMONTEES_SSE_ADDON)) {
			$conf->global->REMONTEESSSE_REMONTEES_SSE_ADDON = 'mod_remontees_sse_standard';
		}

		if (!empty($conf->global->REMONTEESSSE_REMONTEES_SSE_ADDON)) {
			$mybool = false;

			$file = $conf->global->REMONTEESSSE_REMONTEES_SSE_ADDON.".php";
			$classname = $conf->global->REMONTEESSSE_REMONTEES_SSE_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_remontees_sse';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->REMONTEES_SSE_ADDON_PDF)) {
				$modele = $conf->global->REMONTEES_SSE_ADDON_PDF;
			}
		}
	...
	public function relanceCreateur()
	{
		...
		$sql .= " FROM ".MAIN_DB_PREFIX."remonteessse_remontees_sse as f";
		...
		if ($result) {
			while ($obj = $this->db->fetch_object($result)) {				
				...
				$remontees_sse = New Remontees_SSE($this->db);
				$remontees_sse->fetch($obj->rowid);
				$urlwithouturlroot = preg_replace('/'.preg_quote(DOL_URL_ROOT, '/').'$/i', '', trim($dolibarr_main_url_root));
				$urlwithroot = $urlwithouturlroot.DOL_URL_ROOT; // This is to use external domain name found into config file
				$link = '<a href="'.$urlwithroot.'/custom/remonteessse/remontees_sse_card.php?id='.$remontees_sse->id.'">'.$remontees_sse->ref.'</a>';
...
/**
 * Class Remontees_sseLine. You can also remove this and generate a CRUD class for lines objects.
 */
class Remontees_sseLine extends CommonObjectLine
{
	// To complete with content of an object Remontees_sseLine
	// We should have a field rowid, fk_remontees_sse and position
