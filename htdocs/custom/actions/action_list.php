...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('actions', 'action', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("actions")) {
	accessforbidden('Module actions not enabled');
}
...
// Ajouter les champs des extrafields
...
// Exécuter les hooks pour ajouter des champs supplémentaires
$parameters = array();
$reshook = $hookmanager->executeHooks('printFieldListSelect', $parameters, $object, $action); 
$sql .= $hookmanager->resPrint;
$sql = preg_replace('/,\s*$/', '', $sql);

$sqlfields = $sql; // Champ $sql à supprimer pour le total du comptage

$sql .= " FROM " . MAIN_DB_PREFIX . $object->table_element . " as t";

// Jointure pour les champs extrafields
...
// Ajouter les conditions de recherche supplémentaires
...
// Inclure les conditions de recherche des extrafields
include DOL_DOCUMENT_ROOT.'/core/tpl/extrafields_list_search_sql.tpl.php';

// Exécuter les hooks pour ajouter des conditions supplémentaires
$parameters = array();
...
if (!getDolGlobalInt('MAIN_DISABLE_FULL_SCANLIST')) {
	// Méthode rapide pour obtenir et compter la liste complète convertie en requête SQL COUNT
...
if ($sortfield == 't.ref') {
	$sql .=  "ORDER BY CAST(SUBSTRING(t.ref, 3) AS UNSIGNED) ". $sortorder;
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/actions/action_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', 'Action', $help_url, '', 0, 0, $morejs, $morecss, '', 'bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/actions/action_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste('Action', $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);
...
while ($i < $imaxinloop) {
	...
	// $sql .= " WHERE g.fk_usergroup = 6 OR g.fk_usergroup = 19";
	// $sql .= " AND u.statut = 1";
	...
	// if (($object->intervenant == $user->id )|| ($intervenantIsInGroup && isset($object->ref))){
