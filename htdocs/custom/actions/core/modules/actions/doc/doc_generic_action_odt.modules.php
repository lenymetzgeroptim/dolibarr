...
/**
 *	\file       htdocs/core/modules/actions/doc/doc_generic_action_odt.modules.php
 *	\ingroup    actions
 *	\brief      File of class to build ODT documents for actions
 */

dol_include_once('/actions/core/modules/actions/modules_action.php');
 ...
 */
class doc_generic_action_odt extends ModelePDFAction
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'ACTIONS_MYOBJECT_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$texte .= '<input type="hidden" name="param1" value="ACTIONS_MYOBJECT_ADDON_PDF_ODT_PATH">';
		$texte .= '<table class="nobordernopadding centpercent">';

		// List of directories area
		$texte .= '<tr><td>';
		$texttitle = $langs->trans("ListOfDirectories");
		$listofdir = explode(',', preg_replace('/[\r\n]+/', ',', trim(getDolGlobalString('ACTIONS_MYOBJECT_ADDON_PDF_ODT_PATH'))));
		$listoffiles = array();
		...
		$texte .= getDolGlobalString('ACTIONS_MYOBJECT_ADDON_PDF_ODT_PATH');
		$texte .= '</textarea>';
		...
		if (getDolGlobalString('ACTIONS_MYOBJECT_ADDON_PDF_ODT_PATH')) {
		...
		$texte .= '<input type="hidden" value="ACTIONS_MYOBJECT_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	 ...
	 *
	 *	@param		Action	$object				Object source to build document
	...
	public function write_file($object, $outputlangs, $srctemplatepath, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->actions->dir_output) {
			...
			$dir = $conf->actions->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->actions->dir_temp);
				if (!is_writable($conf->actions->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->actions->dir_temp);
				...
				// Line of free text
				$newfreetext = '';
				$paramfreetext = 'ACTIONS_MYOBJECT_FREE_TEXT';
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
						'PATH_TO_TMP'	  => $conf->actions->dir_temp,
