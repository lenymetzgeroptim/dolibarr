...
/**
 *   	\file       constat_list.php
 *		\ingroup    constat
 *		\brief      List page for constat
 */
...
// load constat libraries

require_once __DIR__.'/../constat/class/constat.class.php';
require_once __DIR__.'/../actions/class/action.class.php';
...
if (!isModEnabled("constat")) {
	accessforbidden('Module constat not enabled');
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Constat';
	$objectlabel = 'Constat';
	$uploaddir = $conf->constat->dir_output;
...
//$help_url = "EN:Module_Constat|FR:Module_Constat_FR|ES:Módulo_Constat";
$help_url = '';
$title = $langs->trans("Constats");
$morejs = array();
$morecss = array();
$exclude = $objectAct->getElementElement($idaction);
...
// Ajoutez les champs des extrafields
...
dol_syslog("Executing count query: ".$sqlforcount, LOG_DEBUG);

// Complete request and execute it with limit
if ($sortfield == 't.ref') {
	$sql .= " ORDER BY t.rowid " . $sortorder; 
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
    ...
    $id = $obj->rowid;
    header("Location: ".dol_buildpath('/constat/constat_card.php', 1).'?id='.$id);
    exit;
...
$arrayofmassactions = array(
	...
	'lié le constat'=>img_picto('', 'link', 'class="pictofixedwidth"').$langs->trans("lié le constat"),
...
if($massaction == 'lié le constat') {
	$objectAct->insertIntoElementElement($arrayofselected, $idaction);
	
}

print '<form method="POST" id="searchFormList" action="'.$_SERVER["PHP_SELF"].'?idaction='.$idaction.'">'."\n";
if ($optioncss != '') {
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/constat/constat_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendConstatRef";
$modelmail = "constat";
$objecttmp = new Constat($db);
...
if (isset($extrafields->attributes[$object->table_element]['computed']) && is_array($extrafields->attributes[$object->table_element]['computed']) && count($extrafields->attributes[$object->table_element]['computed']) > 0) {
	...
	//var_dump($object->Getsuperieur($user , $inManager));
...
while ($i < $imaxinloop) {
	...
	$yourAntennaUserArray = array();
	//var_dump($liste_chef_projet);
	// fonction mais pas bien 
	//var_dump($projet->title);

	foreach($liste_chef_projet as $valeur){
			
		global $db;

		$sql = "SELECT u.fk_user";
		$sql .= " FROM " . MAIN_DB_PREFIX . "user AS u";
		$sql .= " JOIN " . MAIN_DB_PREFIX . "usergroup_user AS g ON u.rowid = g.fk_user";
		$sql .= " WHERE g.fk_usergroup = 9";
		$sql .= " AND u.statut = 1";
		$sql .= " AND u.rowid = " .$valeur."";
		...
		if ($result) {
				...
				while ($i < $nume) {
					...
					$yourAntennaUserArray[] = $obj->fk_user; 
					$i++;
				}
			} else {
				dol_print_error($db);
	...
	foreach (array_keys($yourAntennaUserArray) as $index ) {
		if ($yourAntennaUserArray[$index] == 16 && $liste_chef_projet[$index] !== 16) {
			$yourAntennaUserArray[$index] = $liste_chef_projet[$index];
		}
		if ($yourAntennaUserArray[$index] == null ){
			$yourAntennaUserArray[$index] = $liste_chef_projet[$index];
		}
	}


	$intervenantIsInGroup = false;
     ...
     */
		...
		//if (( $object->fk_user_creat == $user->id) || (in_array($user->id, $liste_chef_projet) && in_array($obj->status, [0,1, 3, 4, 5, 7, 9])) || in_array($user->id, $yourAntennaUserArray)||($intervenantIsInGroup && isset($object->ref))){
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_constat', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
