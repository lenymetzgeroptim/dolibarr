...
require_once DOL_DOCUMENT_ROOT . '/custom/gpeccustom/lib/hrm_position.lib.php';
// dol_include_once('/custom/gpeccustom/position.php');
...
$upload_dir = $conf->gpeccustom->multidir_output[isset($object->entity) ? $object->entity : 1] . '/position';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/custom/gpeccustom/position_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($fk_job))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/custom/gpeccustom/position_card.php', 1) . '?id=' . ($id > 0 ? $id : '__ID__');
			}
		}
	}
...
function displayPositionCard(&$object)
{
	...
	if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
		...
		// Object card
		// ------------------------------------------------------------
		$linkback = '<a href="'.DOL_URL_ROOT.'/custom/gpeccustom/position_list.php">'.$langs->trans("BackToList").'</a>';
...
if ($action !== 'edit' && $action !== 'create') {
	...
	$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', DOL_URL_ROOT.'/custom/gpeccustom/position_agenda.php?id='.$object->id);
