...
dol_include_once('/gpeccustom/class/jobfield.class.php');
dol_include_once('/gpeccustom/lib/gpeccustom_jobfield.lib.php');
...
$upload_dir = $conf->mymodule->multidir_output[isset($object->entity) ? $object->entity : 1].'/jobfield';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/gpeccustom/jobfield_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/gpeccustom/jobfield_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
$title = $langs->trans("Domaine")." - ".$langs->trans('Card');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/gpeccustom/jobfield_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	*/
	...
	if ($action != 'presend') {
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/gpeccustom/jobfield_agenda.php', 1).'?id='.$object->id);
