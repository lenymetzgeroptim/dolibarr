...
$permissiontoadd = $user->rights->gpeccustom->gpeccustom->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/gpeccustom/position_list.php', 1);
	$idBacktoPage = GETPOST('fk_job', 'aZ09');
	if ($idBacktoPage > 0) {
		$backtopage = dol_buildpath('/gpeccustom/position.php', 1) . '?id=' . $idBacktoPage;
	}
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				if ($fk_job > 0) {
					$backtopage = dol_buildpath('/gpeccustom/position.php', 1) . '?fk_job=' . ($fk_job > 0 ? $fk_job : '__ID__');
				} else {
					$backtopage = dol_buildpath('/gpeccustom/position_card.php', 1) . '?id=__ID__';
				}
			}
		}
	}
...
if ($job->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' . dol_buildpath('/gpeccustom/position_list.php', 1) . '?restore_lastsearch_values=1' . (!empty($fk_job) ? '&fk_job=' . $fk_job : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
		...
		$id = $obj->rowid;
		header("Location: " . dol_buildpath('/gpeccustom/position.php', 1) . '?id=' . $id);
		exit;
	}
	...
	$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/gpeccustom/position.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']).'&fk_job='.((int) $fk_job), '', $permissiontoadd);
