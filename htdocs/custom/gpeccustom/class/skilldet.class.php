...
class Skilldet extends CommonObject
{
	...
	public $fields=array(
		...
		'fk_skill' => array('type'=>'integer:Skill:/custom/gpeccustom/class/skill.class.php', 'label'=>'fk_skill', 'enabled'=>'1', 'position'=>5, 'notnull'=>1, 'visible'=>0,),
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->gpeccustom->dir_output.'/skilldet/'.$oldref;
				$dirdest = $conf->gpeccustom->dir_output.'/skilldet/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->gpeccustom->dir_output.'/skilldet/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/custom/gpeccustom/skilldet_card.php', 1).'?id='.$this->id;
