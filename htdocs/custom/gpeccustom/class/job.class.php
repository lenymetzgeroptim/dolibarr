...
class Job extends CommonObject
{
	...
	//  */
	protected $childtablesoncascade = array("@SkillRank:custom/gpeccustom/class/skillrank.class.php:fk_object:objecttype='job'");
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->gpeccustom->dir_output.'/job/'.$oldref;
				$dirdest = $conf->gpeccustom->dir_output.'/job/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->gpeccustom->dir_output.'/job/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/custom/gpeccustom/job_card.php', 1).'?id='.$this->id;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/gpeccustom/doc/";
