...
//  $search_array_options = $extrafields->getOptionalsFromPost($object->table_element, '', 'search_');
 ...
 $object->fields = array(
	 ...
	 'fk_job' => 
		 array(
		 'type' => 'integer:Job:gpeccustom/job/class/job.class.php',
		 'label' => 'Emploi',
		 'enabled' => '1',
		 'position' => 10,
		 'notnull' => 1,
 ...
 $help_url = '';
 $title = $langs->trans('Salariés en Poste Non Évalués');
 $morejs = array();
 $morecss = array();
 //
 // print '</div>';
 ...
 $sql = "SELECT t.rowid,t.fk_user,t.fk_job,t.description, DATE_FORMAT(t.date_start ,'%Y-%m-%d') as date_start, t.date_start, DATE_FORMAT(t.date_end ,'%Y-%m-%d') as date_end,";
 ...
 if (!$sortfield) {
	 // reset($object->fields);					// Reset is required to avoid key() to return null.
	 $sortfield = "t.fk_job"; // Set here default search field. By default 1st field in definition.
 ...
 print '<form method="POST" id="searchFormList2" action="'.$_SERVER["PHP_SELF"].'">'."\n";
 if ($optioncss != '') {
 ...
 print_barre_liste($title, '', $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', '', '', 'object_'.$object->picto, 0, '', '', $limit, 0, 0, 1);
...
//  print '<tr><td class="tdoverflowmax200">Salariés en Poste Non Évalués</td></tr>';
//  print '<th colspan="1" style="text-align: left; font-weight: bold; font-size: 1.2em;">';
// print $langs->trans("Salariés en Poste Non Évalués");
// print '</th>';
print '</tr>';
 // Fields title search
 // --------------------------------------------------------------------
 ...
 while ($i < $imaxinloop) {
	 ...
	 if ($mode == 'kanban') {
		 ...
		 foreach ($object->fields as $key => $val) {
			 ...
			 if (in_array($key, array('fk_job', 'fk_user'))) $cssforfield = 'tdoverflowmax100';
