...
/**
 *  \file       cvtec_contact.php
 *  \ingroup    gpeccustom
 *  \brief      Tab for contacts linked to CVTec
 */
...
$hookmanager->initHooks(array('cvteccontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->gpeccustom->cvtec->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('CVTec')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("CVTec"), -1, $object->picto);
