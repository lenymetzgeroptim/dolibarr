...
/**
 * \file    test/phpunit/GpeccustomFunctionalTest.php
 * \ingroup gpeccustom
...
/**
 * Class GpeccustomFunctionalTest
 *
 ...
 *
 * @package Testgpeccustom
 */
class GpeccustomFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	private static $module_id = 500040; // TODO: autodetect?
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modGpeccustom')->click();
		} else {
			// Disable the module
			$this->byHref('modGpeccustom')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/gpeccustom/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('gpeccustom/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/gpeccustom/admin/about.php');
		$this->authenticate();
		return $this->assertContains('gpeccustom/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('gpeccustomwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modGpeccustom_GpeccustomTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modGpeccustom_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
