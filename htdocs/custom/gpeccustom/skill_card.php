...
/**
 *    \file       htdocs/custom/gpeccustom/skill_card.php
 *    \ingroup    hrm
...
require_once DOL_DOCUMENT_ROOT . '/custom/gpeccustom/lib/hrm_skill.lib.php';
...
global $user;
$urltogo = DOL_URL_ROOT.'/custom/gpeccustom/skill_list.php?restore_lastsearch_values=1';
...
// Permissions
// $permissiontoread   = $user->rights->gpeccustom->all->read;
// $permissiontoadd    = $user->rights->gpeccustom->all->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
// $permissiontodelete = $user->rights->gpeccustom->all->delete;
...
$upload_dir = $conf->gpeccustom->multidir_output[isset($object->entity) ? $object->entity : 1] . '/skill';
...
if (empty($reshook)) {
	...
	$backurlforlist = DOL_URL_ROOT.'/custom/gpeccustom/skill_list.php';
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = DOL_URL_ROOT.'/custom/gpeccustom/skill_card.php?id=' . ($id > 0 ? $id : '__ID__');
			}
		}
	}
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' . DOL_URL_ROOT.'/custom/gpeccustom/skill_list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	//dol_include_once('/custom/gpeccustom/tpl/hrm_skillde.fiche.tpl.php');
...
if ($action != "create" && $action != "edit") {
	...
	$backtopage = dol_buildpath('/custom/gpeccustom/skill_card.php', 1) . '?id=' . $id;
	$param = "";
	$massactionbutton = "";
	//$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/custom/gpeccustom/skilldet_card.php', 1) . '?action=create&backtopage=' . urlencode($_SERVER['PHP_SELF']) . $param_fk . '&backtopage=' . $backtopage, '', $permissiontoadd);
	...
	while ($i < ($limit ? min($num, $limit) : $num)) {
		...
		//print '<a class="timeline-btn" href="' . dol_buildpath("custom/custom/gpeccustom/skilldet_card.php?action=edit&id=" . $objectline->id, 1) . '"><i class="fa fa-pencil" title="' . $langs->trans("Modify") . '" ></i></a>';
		// add trash
		//@todo change to proper call dol_
		//print '<a class="timeline-btn" href="'.dol_buildpath("custom/custom/gpeccustom/skilldet_card.php?action=delete&id=".$objectline->id,1)  .'"><i class="fa fa-trash" title="'.$langs->trans("Delete").'" ></i></a>';
		// print '</td>';
	...
	$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', DOL_URL_ROOT.'/custom/gpeccustom/skill_agenda.php?id='.$object->id);
