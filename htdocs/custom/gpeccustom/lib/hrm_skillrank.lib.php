...
function skillrankPrepareHead($object)
{
	...
	$head[$h][0] = dol_buildpath("/gpeccustom/skillrank_card.php", 1).'?id='.$object->id;
	...
	if (isset($object->fields['note_public']) || isset($object->fields['note_private'])) {
		...
		$head[$h][0] = dol_buildpath('/gpeccustom/skillrank_note.php', 1).'?id='.$object->id;
	...
	$upload_dir = $conf->gpeccustom->dir_output."/skillrank/".dol_sanitizeFileName($object->ref);
	$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
	$nbLinks = Link::count($db, $object->element, $object->id);
	$head[$h][0] = dol_buildpath("/gpeccustom/skillrank_document.php", 1).'?id='.$object->id;
	...
	$head[$h][0] = dol_buildpath("/gpeccustom/skillrank_agenda.php", 1).'?id='.$object->id;
...
function displayRankInfos($selected_rank, $fk_skill, $inputname = 'TNote', $mode = 'view')
{
	...
	require_once DOL_DOCUMENT_ROOT . '/custom/gpeccustom/class/skilldet.class.php';
