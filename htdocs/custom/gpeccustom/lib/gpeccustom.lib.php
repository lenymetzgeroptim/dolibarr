...
function gpeccustomAdminPrepareHead()
{
	...
	$head[$h][0] = dol_buildpath("/gpeccustom/admin/job_user_extrafields.php", 1);
	$head[$h][1] = $langs->trans("Attributs suplémantaires (Job Profile)");
	$nbExtrafields = $extrafields->attributes['hrm_job_user']['count'];
	if ($nbExtrafields > 0) {
		$head[$h][1] .= '<span class="badge marginleftonlyshort">'.$nbExtrafields.'</span>';
	}
	$head[$h][2] = 'jobsProfileAttributes';
	$h++;

	$head[$h][0] = dol_buildpath("/gpeccustom/admin/cvtec_extrafields.php", 1);
	$head[$h][1] = $langs->trans("Attributs suplémantaires (CVTec)");
	...
	/*
	$head[$h][0] = dol_buildpath("/gpeccustom/admin/myobject_extrafields.php", 1);
