...
/**
 *    \file       htdocs/custom/gpeccustom/job_card.php
 *    \ingroup    hrm
...
require_once DOL_DOCUMENT_ROOT . '/custom/gpeccustom/lib/hrm_job.lib.php';
...
$upload_dir = $conf->gpeccustom->multidir_output[isset($object->entity) ? $object->entity : 1] . '/job';
...
if (empty($conf->gpeccustom->enabled)) accessforbidden();
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/custom/gpeccustom/job_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/custom/gpeccustom/job_card.php', 1) . '?id=' . ($id > 0 ? $id : '__ID__');
			}
		}
	}
...
if (($id || $ref) && $action == 'edit') {
	...
	// Common attributes
	include DOL_DOCUMENT_ROOT . '/custom/gpeccustom/core/tpl/commonfields_edit.tpl.php';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' . dol_buildpath('/custom/gpeccustom/job_list.php', 1) . '?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$genallowed = $user->rights->gpeccustom->job->read; // If you can read, you can build the PDF to read content
			$delallowed = $user->rights->gpeccsutom->job->write; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('gpeccustom:JobCustom', $object->element . '/' . $objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', DOL_URL_ROOT.'/custom/gpeccustom/job_agenda.php?id='.$object->id);
