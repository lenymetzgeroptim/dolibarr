...
require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php'; // À placer en haut du fichier si pas déjà

$object->fields = dol_sort_array($object->fields, 'position');

foreach ($object->fields as $key => $val) {
	if (abs($val['visible']) != 1 && abs($val['visible']) != 3 && abs($val['visible']) != 4) continue;
	if (array_key_exists('enabled', $val) && isset($val['enabled']) && !verifCond($val['enabled'])) continue;

	print '<tr class="field_'.$key.'"><td';
	print ' class="titlefieldcreate';
	if (isset($val['notnull']) && $val['notnull'] > 0) print ' fieldrequired';
	...
	// Récupération de la valeur selon le type
	if (in_array($val['type'], array('int', 'integer'))) {
		...
		$value = GETPOSTISSET($key) ? GETPOST($key, 'nohtml') : $object->$key;
	} elseif (preg_match('/^html/', $val['type'])) {
		$value = GETPOSTISSET($key) ? GETPOST($key, 'restricthtml') : $object->$key;
	...
	if (!empty($val['noteditable'])) {
		...
		if ($key === 'description') {
			// Affichage WYSIWYG uniquement pour "description"
			$doleditor = new DolEditor(
				'description',        // name
				$value,               // value
				'',                   // id HTML
				250,                  // height
				'dolibarr_notes',     // CSS class
				'',                   // toolbar config
				false,                // fullscreen
				true,                 // wysiwyg = true
				getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'), // use wysiwyg only if allowed
				10,                   // nb rows
				'100%'                // width
			);
			$doleditor->Create();
