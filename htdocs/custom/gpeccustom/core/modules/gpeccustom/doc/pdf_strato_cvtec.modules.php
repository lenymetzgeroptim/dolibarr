 ...
 */
class pdf_strato_cvtec extends ModelePDFCVTec
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->contract->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				$pdf->SetSubject($outputlangs->transnoentities("CV"));
				$pdf->SetCreator("Dolibarr ".DOL_VERSION);
				$pdf->SetAuthor($outputlangs->convToOutputCharset($user->getFullName($outputlangs)));
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref)." ".$outputlangs->transnoentities("CV")." ".$outputlangs->convToOutputCharset($object->thirdparty->name));
				...
				global $user;
				$array_user = $this->get_substitutionarray_contact($user, $outputlangs, $array_key = 'object');
				var_dump($array_user);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "Contract")
	{
		...
		*/
		...
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell($w, 3, $outputlangs->transnoentities("Date")." : ".dol_print_date($now, "day", false, $outputlangs, true), '', 'R');
