...
class Skill extends CommonObject
{
	...
	public $fields=array(
		...
		'ref' => array('type' => 'varchar(30)', 'label' => 'Ref', 'default' => '(PROV)', 'enabled' => 1, 'visible' => -1, 'notnull' => 1, 'position' => 12, 'index' => 1),
		...
		'status' =>array('type'=>'smallint(6)', 'label'=>'Statut', 'enabled'=>'1', 'visible'=>1, 'position'=>500, 'index'=>1, 'arrayofkeyval'=>array('0'=>'Draft', '1'=>'Validated', '6' => 'Closed'),)
	);
	public $rowid;
	public $label;
	public $description;
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->gpeccustom->dir_output.'/skill/'.$oldref;
				$dirdest = $conf->gpeccustom->dir_output.'/skill/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->gpeccustom->dir_output.'/skill/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/custom/gpeccustom/skill_card.php', 1).'?id='.$this->id;
