...
require_once __DIR__.'/class/jobfield.class.php';
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('gpeccustom', 'jobfield', 'read');
	$permissiontoadd = $user->hasRight('gpeccustom', 'jobfield', 'write');
	$permissiontodelete = $user->hasRight('gpeccustom', 'jobfield', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'JobField';
	$objectlabel = 'JobField';
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/gpeccustom/jobfield_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/gpeccustom/jobfield_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
