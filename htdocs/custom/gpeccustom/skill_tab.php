...
// Permissions
$permissiontoread = $user->hasRight('gpeccustom', 'gpeccustom', 'read');
$permissiontoadd  = $user->hasRight('gpeccustom', 'gpeccustom', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
...
if (empty($reshook)) {
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = DOL_URL_ROOT.'/custom/gpeccustom/skill_list.php?id=' . ($id > 0 ? $id : '__ID__');
			}
		}
	}
	...
	if ($action == 'addSkill') {
		...
		if (!empty($TNote)) {
			...
			header("Location: " . DOL_URL_ROOT.'/custom/gpeccustom/skill_tab.php?id=' . $id. '&objecttype=job');
			exit;
		}
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($objecttype == 'job') {
		...
		$listLink = dol_buildpath('/custom/gpeccustom/job_list.php', 1);
	} elseif ($objecttype == "user") {
