...
/**
 *    \file       htdocs/custom/gpeccustom/evaluation_card.php
 *    \ingroup    hrm
...
require_once DOL_DOCUMENT_ROOT.'/custom/gpeccustom/class/evaluation.class.php';
...
require_once DOL_DOCUMENT_ROOT.'/custom/gpeccustom/class/skillrank.class.php';
require_once DOL_DOCUMENT_ROOT.'/custom/gpeccustom/lib/hrm_evaluation.lib.php';
require_once DOL_DOCUMENT_ROOT.'/custom/gpeccustom/lib/hrm_skillrank.lib.php';
...
// Permissions
$permissiontoread = $user->rights->gpeccustom->evaluation->read;
$permissiontoadd = $user->rights->gpeccustom->evaluation->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
$permissiontovalidate = (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && $user->rights->gpeccustom->evaluation_advance->validate) || (empty($conf->global->MAIN_USE_ADVANCED_PERMS) && $permissiontoadd);
$permissiontoClose = $user->rights->gpeccustom->evaluation->write;
$permissiontodelete = $user->rights->gpeccustom->evaluation->delete/* || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT)*/;
$permissiondellink = $user->rights->gpeccustom->evaluation->write; // Used by the include of actions_dellink.inc.php
$upload_dir = $conf->gpeccustom->multidir_output[isset($object->entity) ? $object->entity : 1].'/evaluation';
...
// 	accessforbidden();
// }
if (empty($conf->gpeccustom->enabled)) {
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/custom/gpeccustom/evaluation_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/custom/gpeccustom/evaluation_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
			}
		}
	}
	...
	$(document).ready(function() {
	  $("#btn_valid").click(function(){
		 ...
		 $.ajax({
             ...
             window.location.href = "'.dol_buildpath('/custom/gpeccustom/evaluation_card.php', 1).'?id='.$id.'&action=validate&token='.newToken().'";
             return false;
		 });
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/gpeccustom/evaluation_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	if (!empty($object->table_element_line) && $object->status == Evaluation::STATUS_DRAFT) {
		...
		$conf->modules_parts['tpl']['gpeccustom']='/custom/gpeccustom/core/tpl/'; // Pour utilisation du tpl hrm sur cet écran
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$genallowed = $user->rights->gpeccustom->evaluation->readall; // If you can read, you can build the PDF to read content
			$delallowed = $user->rights->gpeccustom->evaluation->write; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('gpeccustom:Evaluation', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', DOL_URL_ROOT.'/custom/gpeccustom/evaluation_agenda.php?id='.$object->id);
