...
require_once DOL_DOCUMENT_ROOT.'/custom/ecmcustom/class/htmlecm.form.class.php';
require_once DOL_DOCUMENT_ROOT.'/custom/ecmcustom/class/ecmdirectory.class.php';


// Load translation files required by the page
$langs->loadLangs(array("ecm", "companies", "other", "users", "orders", "propal", "bills", "contracts", "categories", "ecmcustom@ecmcustom"));
...
if ($module == 'ecmcustom') {
	$upload_dir = $conf->ecmcustom->dir_output.'/'.$urlsection;
...
$extrafields->fetch_name_optionals_label($ecmdir->table_element);
...
if ($action == 'add' && $permtoadd) {
	if ($cancel) {
		if (!empty($backtopage)) {
			...
			header("Location: ".DOL_URL_ROOT.'/custom/ecmcustom/index.php?action=file_manager'.($module ? '&module='.$module : ''));
			exit;
		}
	}
	...
	if (!$error) {
		if (!empty($backtopage)) {
			...
			header("Location: ".DOL_URL_ROOT.'/custom/ecmcustom/index.php?action=file_manager');
			exit;
		}
	}
