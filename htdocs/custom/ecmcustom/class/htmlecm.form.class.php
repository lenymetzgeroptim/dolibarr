 ...
 */
require_once DOL_DOCUMENT_ROOT.'/custom/ecmcustom/class/ecmdirectory.class.php';
 ...
 */
class FormEcm extends FormFile
{
	/**
	 ...
	 */
	public function selectAllSections($selected = 0, $select_name = '', $module = 'ecmcustom')
	{
		global $conf, $langs;
		$langs->load("ecm");
	 ...
	 */
	public function list_of_documents_custom($filearray, $object, $modulepart, $param = '', $forcedownload = 0, $relativepath = '', $permonobject = 1, $useinecm = 0, $textifempty = '', $maxlength = 0, $title = '', $url = '', $showrelpart = 0, $permtoeditline = -1, $upload_dir = '', $sortfield = '', $sortorder = 'ASC', $disablemove = 1, $addfilterfields = 0, $disablecrop = -1)
	{
		// phpcs:enable
				...
				if ($file['name'] != '.'
					...
					if (!$editline) {
						...
						if ($useinecm == 1 || $useinecm == 5) {	// ECM manual tree only
							...
							$backtopage = DOL_URL_ROOT.'/custom/ecmcustom/index.php?&section_dir='.urlencode($relativepath).$newparam;
							print '<a class="editfielda editfilelink" href="'.DOL_URL_ROOT.'/custom/ecmcustom/file_card.php?urlfile='.urlencode($file['name']).$param.'&backtopage='.urlencode($backtopage).'" rel="'.urlencode($file['name']).'">'.img_edit('default', 0, 'class="paddingrightonly"').'</a>';
						}
						...
						if (empty($useinecm) || $useinecm == 2 || $useinecm == 6) {	// 6=Media file manager
							...
							if ($permtoeditline) {
								$paramsectiondir = (in_array($modulepart, array('medias', 'ecmcustom')) ? '&section_dir='.urlencode($relativepath) : '');
...
// phpcs:disable PEAR.NamingConventions.ValidFunctionName.ScopeNotCamelCaps
	 ...
	 */
	public function list_of_autoecmfiles_custom($upload_dir, $filearray, $modulepart, $param, $forcedownload = 0, $relativepath = '', $permissiontodelete = 1, $useinecm = 0, $textifempty = '', $maxlength = 0, $url = '', $addfilterfields = 0)
	{
		// phpcs:enable
		global $user, $conf, $langs, $hookmanager, $form;
	...
	public function form_attach_new_file($url, $title = '', $addcancel = 0, $sectionid = 0, $perm = 1, $size = 50, $object = '', $options = '', $useajax = 1, $savingdocmask = '', $linkfiles = 1, $htmlname = 'formuserfile', $accept = '', $sectiondir = '', $usewithoutform = 0, $capture = 0, $disablemulti = 0, $nooutput = 0)
	{
		...
		if ((!empty($conf->global->MAIN_USE_JQUERY_FILEUPLOAD) && $useajax) || ($useajax == 2)) {
			...
			$out .= '<input class="flat minwidth300 maxwidth200onsmartphone" type="file"';
			...
			$out .= '<input style="margin-left: 8px;" type="submit" class="button small reposition" name="sendit" value="'.$langs->trans("Upload").'"';
			$out .= (empty($conf->global->MAIN_UPLOAD_DOC) || empty($perm) ? ' disabled' : '');
			$out .= '>';
			if ($sectionid) {	// Acitviation and desactivation of sending email notification
				$langs->load('link');
				$out .= '<span class="wraponsmartphone"><input style="margin-right: 2px; margin-left: 4px;" type="checkbox" id="emailing" name="emailing" checked value="1"><label for="emailing">Activation des notifications</label></span>';
			}
