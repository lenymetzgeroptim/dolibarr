...
if (!isset($mode) || $mode != 'noajax') {    // For ajax call
	...
	include_once DOL_DOCUMENT_ROOT.'/custom/ecmcustom/class/ecmdirectory.class.php';
	...
	if (empty($url)) {
		$url = DOL_URL_ROOT.'/custom/ecmcustom/index.php';
	}
}
...
if ($modulepart == 'ecmcustom') {
	$fullpathselecteddir = $conf->ecmcustom->dir_output.'/'.($selecteddir != '/' ? $selecteddir : '');
	$fullpathpreopened = $conf->ecmcustom->dir_output.'/'.($preopened != '/' ? $preopened : '');
			...
			if (count($files) > 2) {    /* The 2 accounts for . and .. */
				...
				foreach ($files as $file) {    // $file can be '.', '..', or 'My dir' or 'My file'
					...
					if($id > 0){
						...
						$ecmdirstatic->fetch($id);
						$liste_groupe = explode(',', $ecmdirstatic->array_options['options_groupe_user']);

						$groupe_user = new UserGroup($db);
						$liste_groupe_user = $groupe_user->listGroupsForUser($user->id, false);
						if(empty($liste_groupe) || $user->admin){
							$autorisation = 1;
						}
						else{
							foreach($liste_groupe_user as $groupe_id => $groupe_info){
								if(in_array($groupe_id, $liste_groupe)){
									$autorisation = 1;
									break;
								}
							}
					...
					if ($autorisation && $file != '.' && $file != '..' && ((!empty($val['fullrelativename']) && $val['id'] >= 0) || dol_is_dir($fullpathselecteddir.(preg_match('/\/$/', $fullpathselecteddir) ? '' : '/').$file))) {
						...
						print DOL_URL_ROOT.'/custom/ecmcustom/dir_card.php?module='.urlencode($modulepart).'&section='.$val['id'].'&relativedir='.urlencode($val['fullrelativename']);
						...
						if (preg_match('/^'.preg_quote($val['fullrelativename'].'/', '/').'/', $preopened)) {
							...
							if ($modulepart == 'ecmcustom') {
								$newfullpathselecteddir = $conf->ecmcustom->dir_output.'/'.($val['fullrelativename'] != '/' ? $val['fullrelativename'] : '');
							} elseif ($modulepart == 'medias') {
