...
if($element == "ecmcustom"){
	$dir = $conf->ecmcustom->dir_output;
}
else {
...
if (isset($action) && !empty($action)) {
	...
	if ($action == 'build' && !empty($element)) {
		...
		$diroutputslash = str_replace('\\', '/', $dir);
		$diroutputslash .= '/';

		// Scan directory tree on disk
		$disktree = dol_dir_list($dir, 'directories', 1, '', array('^temp$'), '', '', 0);
		...
		foreach ($disktree as $dirdesc) {    // Loop on tree onto disk
			...
			foreach ($sqltree as $dirsqldesc) {
				if ($dir.'/'.$dirsqldesc['fullrelativename'] == $dirdesc['fullname']) {
					$dirisindatabase = 1;
					break;
				}
			}
		...
		foreach ($sqltree as $dirdesc) {    // Loop on each sqltree to check dir is on disk
			$dirtotest = $dir.'/'.$dirdesc['fullrelativename'];
