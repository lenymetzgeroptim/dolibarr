...
if (!isset($mode) || $mode != 'noajax') {    // For ajax call
	...
	$rootdirfordoc = $conf->ecmcustom->dir_output;
...
if ($type == 'directory') {
	...
	if (in_array($module, $automodules)) {
		...
		$perm = $user->rights->ecmcustom->delete;

		$formfile->list_of_autoecmfiles_custom($upload_dir, $filearray, $module, $param, 1, '', $perm, 1, $textifempty, $maxlengthname, $url, 1);
	} else {
		// Manual list
		if ($module == 'medias') {
			/*
		...
		if ($module == 'medias') {
			...
			$title = 'none';
		} elseif ($module == 'ecmcustom') { // DMS/ECM -> manual structure
			if ($user->rights->ecmcustom->read) {
				// Buttons: Preview
				$useinecm = 2;
			}
		...
		$formfile->list_of_documents_custom($filearray, '', "ecmcustom", $param, 1, $relativepath, $perm, $useinecm, $textifempty, $maxlengthname, $title, $url, 0, $perm, '', $sortfield, $sortorder);
	}
}
