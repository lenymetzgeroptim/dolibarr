...
if ($permtoadd) {
	...
	print '<a id="acreatedir" href="'.DOL_URL_ROOT.'/custom/ecmcustom/dir_add_card.php?action=create&module='.urlencode($module).$websitekeyandpageid.'&backtopage='.urlencode($_SERVER["PHP_SELF"].'?file_manager=1'.$websitekeyandpageid).'" class="inline-block valignmiddle toolbarbutton paddingtop" title="'.dol_escape_htmltag($langs->trans('ECMAddSection')).'">';
...
if ($permtoadd && GETPOSTISSET('website')) {	// If on file manager to manage medias of a web site
	...
	print '</a>';
} elseif ($permtoadd && $module == 'ecmcustom') {	// If on file manager medias in ecm
...
if ((!empty($conf->use_javascript_ajax) && empty($conf->global->MAIN_ECM_DISABLE_JS)) || !empty($section)) {
	...
	include_once DOL_DOCUMENT_ROOT.'/custom/ecmcustom/class/htmlecm.form.class.php';
	//$formfile = new FormFile($db);
	$formfile = new FormEcm($db);
...
if ($action == 'convertimgwebp' && $permtoadd) {
	if ($module == 'medias') {
		...
		$imagefolder = $conf->ecmcustom->dir_output.'/'.dol_sanitizePathName(GETPOST('section_dir', 'alpha'));
	}
...
if (empty($action) || $action == 'editfile' || $action == 'file_manager' || preg_match('/refresh/i', $action) || $action == 'delete') {
	...
	if (!empty($conf->use_javascript_ajax) && empty($conf->global->MAIN_ECM_DISABLE_JS)) {
		...
		include DOL_DOCUMENT_ROOT.'/custom/ecmcustom/ajax/ajaxdirtree.php';
...
include DOL_DOCUMENT_ROOT.'/custom/ecmcustom/ajax/ajaxdirpreview.php'; // Show content of a directory on right side
