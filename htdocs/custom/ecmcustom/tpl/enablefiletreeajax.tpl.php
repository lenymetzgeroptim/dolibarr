...
$(document).ready(function() {
	...
	$('#filetree').fileTree({
		...
		script: '<?php echo DOL_URL_ROOT.'/custom/ecmcustom/ajax/ajaxdirtree.php?token='.urlencode($_SESSION['token']).'&modulepart='.urlencode($module).(empty($preopened) ? '' : '&preopened='.urlencode($preopened)).'&openeddir='.urlencode($openeddir).(empty($paramwithoutsection) ? '' : $paramwithoutsection); ?>',
	...
	$('#refreshbutton').click( function() {
		...
		$.get( "<?php echo DOL_URL_ROOT.'/custom/ecmcustom/ajax/ecmdatabase.php'; ?>", {
			action: "build",
			element: "ecmcustom"
		},
		function(response) {
...
function loadandshowpreview(filedirname,section)
{
	...
	var url = '<?php echo dol_buildpath('/custom/ecmcustom/ajax/ajaxdirpreview.php', 1); ?>?action=preview&module=<?php echo $module; ?>&section='+section+'&file='+urlencode(filedirname)<?php echo (empty($paramwithoutsection) ? '' : "+'".$paramwithoutsection."'"); ?>;
