...
$result = restrictedArea_custom($user, 'ecmcustom', 0);

//emails and firstname from group users
$usergroups = explode(',', $ecmdir->array_options['options_groupe_user']);
...
//names and emails
$emails = array_unique($usersEmail, SORT_STRING);
...
//Activiation of notification
$allowsendingmail = (GETPOST('emailing', 'int') ? 1 : 0);
...
// Add directory
if ($action == 'add' && $user->rights->ecmcustom->setup) {
...
if ($action == 'confirm_deletesection' && GETPOST('confirm', 'alpha') == 'yes') {
	$result = $ecmdir->delete_custom($user);
...
if ($action == 'refreshmanual') {
	$ecmdirtmp = new EcmCustomDirectory($db);

	// This part of code is same than into file ecm/ajax/ecmdatabase.php TODO Remove duplicate
	clearstatcache();

	$diroutputslash = str_replace('\\', '/', $conf->ecmcustom->dir_output);
	$diroutputslash .= '/';

	// Scan directory tree on disk
	$disktree = dol_dir_list($conf->ecmcustom->dir_output, 'directories', 1, '', '^temp$', '', '', 0);
	...
	foreach ($disktree as $dirdesc) {    // Loop on tree onto disk
		...
		foreach ($sqltree as $dirsqldesc) {
			if ($conf->ecmcustom->dir_output.'/'.$dirsqldesc['fullrelativename'] == $dirdesc['fullname']) {
				$dirisindatabase = 1;
				break;
			}
		}
	...
	foreach ($sqltree as $dirdesc) {    // Loop on each sqltree to check dir is on disk
		$dirtotest = $conf->ecmcustom->dir_output.'/'.$dirdesc['fullrelativename'];
		if (!dol_is_dir($dirtotest)) {
			$ecmdirtmp->id = $dirdesc['id'];
			$ecmdirtmp->delete_custom($user, 'databaseonly');
			//exit;
		}
	}
...
$module = 'ecmcustom';
include DOL_DOCUMENT_ROOT.'/custom/ecmcustom/tpl/filemanager.tpl.php';
