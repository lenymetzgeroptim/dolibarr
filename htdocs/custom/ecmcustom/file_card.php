...
require_once DOL_DOCUMENT_ROOT.'/custom/ecmcustom/class/ecmfiles.class.php';
...
$fullpath = $conf->ecmcustom->dir_output.'/'.$relativepath.$urlfile;

$relativetodocument = 'ecmcustom/'.$relativepath; // $relativepath is relative to ECM dir, we need relative to document
...
if ($action == 'update' && $permtoadd) {
	...
	$olddirrelativetodocument = 'ecmcustom/'.$olddir; // Relative to document
	$newdirrelativetodocument = 'ecmcustom/'.$olddir;
...
$s = img_picto('', 'object_dir').' <a href="'.DOL_URL_ROOT.'/custom/ecmcustom/index.php">'.$langs->trans("ECMRoot").'</a> -> '.$s.' -> ';
if ($action == 'edit') {
...
if ($action != 'edit') {
	...
	if ($user->rights->ecmcustom->setup) {
