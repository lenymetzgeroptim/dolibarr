...
function ecm_prepare_dasboard_head($object)
{
	...
	$head[$h][0] = DOL_URL_ROOT.'/custom/ecmcustom/index.php';
	$head[$h][1] = $langs->trans("ECMSectionsManual").$form->textwithpicto('', $helptext, 1, 'info', '', 0, 3);
	$head[$h][2] = 'index';
	$h++;

	if (!empty($conf->global->ECM_AUTO_TREE_ENABLED)) {
		$head[$h][0] = DOL_URL_ROOT.'/custom/ecmcustom/index_auto.php';
	...
	complete_head_from_modules($conf, $langs, $object, $head, $h, 'ecmcustom');

	complete_head_from_modules($conf, $langs, $object, $head, $h, 'ecmcustom', 'remove');
 ...
 * @return  array				Array of tabs to show
 */
function ecm_prepare_head($object, $module = 'ecmcustom', $section = '')
{
	...
	if ($module == 'ecmcustom') {
		$head[$h][0] = DOL_URL_ROOT.'/custom/ecmcustom/dir_card.php?section='.$object->id;
		$head[$h][1] = $langs->trans("Directory");
		$head[$h][2] = 'card';
		$h++;
	} else {
		$head[$h][0] = DOL_URL_ROOT.'/custom/ecmcustom/dir_card.php?section='.$section.'&module='.$module;
		$head[$h][1] = $langs->trans("Directory");
		$head[$h][2] = 'card';
		$h++;
	}
...
function ecm_file_prepare_head($object)
{
	...
	$head[$h][0] = DOL_URL_ROOT.'/custom/ecmcustom/file_card.php?section='.$object->section_id.'&urlfile='.urlencode($object->label);
	$head[$h][1] = $langs->trans("File");
	$head[$h][2] = 'card';
	$h++;

	// Notes
	$head[$h][0] = DOL_URL_ROOT.'/custom/ecmcustom/file_note.php?section='.$object->section_id.'&urlfile='.urlencode($object->label);
...
function ecm_prepare_head_fm($object)
{
	...
	$head[$h][0] = DOL_URL_ROOT.'/custom/ecmcustom/index.php?action=file_manager';
	$head[$h][1] = $langs->trans('ECMFileManager');
	$head[$h][2] = 'file_manager';
	$h++;

	$head[$h][0] = DOL_URL_ROOT.'/custom/ecmcustom/search.php';
