...
require_once DOL_DOCUMENT_ROOT.'/custom/ecmcustom/lib/ecm.lib.php';
require_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
require_once DOL_DOCUMENT_ROOT.'/ecm/class/ecmdirectory.class.php';


// Load translation files required by page
$langs->loadLangs(array('ecmcustom', 'companies', 'other'));
...
// Load ecm object
$ecmdir = new EcmCustomDirectory($db);
...
$permtoupload = 0;
if ($module == 'ecmcustom') {
	$permtoread = $user->rights->ecmcustom->read;
	$permtoadd = $user->rights->ecmcustom->setup;
	$permtoupload = $user->rights->ecmcustom->upload;
	$permtodelete = $user->rights->ecmcustom->delete;
}
if ($module == 'medias') {
...
// Remove file
if ($action == 'confirm_deletefile' && $confirm == 'yes' && $permtodelete) {
	$langs->load("other");
...
// Remove dir
if ($action == 'confirm_deletedir' && $confirm == 'yes' && $permtodelete) {
	$backtourl = DOL_URL_ROOT."/custom/ecmcustom/index.php";
	if ($module == 'medias') {
	...
	if ($module == 'ecmcustom' && $ecmdir->id > 0) {	// If manual ECM and directory is indexed into database
		// Fetch was already done
		$result = $ecmdir->delete_custom($user, 'all', $deletedirrecursive);
		if ($result <= 0) {
			$langs->load('errors');
...
if ($action == 'update' && !GETPOST('cancel', 'alpha') && $permtoadd) {
	...
	if ($module == 'ecmcustom') {
		...
		$olddir = $conf->ecmcustom->dir_output.'/'.$olddir;
	} else {
	...
	if ($module == 'ecmcustom') {
		...
		if ($result > 0) {
			...
			if ($oldlabel != $ecmdir->label && file_exists($olddir)) {
				...
				$newdir = $conf->ecmcustom->dir_output.'/'.$newdir;
				//print $olddir.'-'.$newdir;
...
if ($module == 'ecmcustom' && $ecmdir->id > 0) {
...
if ($module == 'ecmcustom') {
	...
	$morehtmlref = '<a href="'.DOL_URL_ROOT.'/custom/ecmcustom/index.php">'.$langs->trans("ECMRoot").'</a> -> '.$s;
}
if ($module == 'medias') {
	$s = 'medias -> ';
	$result = 1;
...
if ($action != 'edit' && $action != 'delete') {
	...
	if ($permtoadd) {
		print '<a class="butAction" href="'.DOL_URL_ROOT.'/custom/ecmcustom/dir_add_card.php?action=create'.($module ? '&module='.$module : '').'&catParent='.$section.'">'.$langs->trans('ECMAddSection').'</a>';
	} else {
...
// List of document
if ($user->rights->ecmcustom->read)
{
	$param='&amp;section='.$section;
	$formfile->list_of_documents($filearray,'','ecmcustom',$param,1,$relativepath,$user->rights->ecmcustom->upload);
}
*/
