 ...
 * \brief   HolidayCustom setup page.
 */
...
$hookmanager->initHooks(array('holidaycustomsetup', 'globalsetup'));
...
// ORGANISATION_CONGES
$formSetup->newItem('ORGANISATION_CONGES')->setAsTextarea();

// HOLIDAY_FRAC_ANC_DATESTART
$formSetup->newItem('HOLIDAY_FRAC_ANC_DATESTART');

// HOLIDAY_FRAC_ANC_DATEEND
$formSetup->newItem('HOLIDAY_FRAC_ANC_DATEEND');

// HOLIDAY_FDT_APPROVER
$formSetup->newItem('HOLIDAY_FDT_APPROVER')->setAsYesNo();

// HOLIDAY_VALIDATE_ONLY_RTT
// $formSetup->newItem('HOLIDAY_VALIDATE_ONLY_RTT')->setAsYesNo();

// HOLIDAY_VALIDATE_TYPE
$TField = getLabelList('c_holiday_types', 'label');
$TField[-1] = 'Tous'; 
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/holidaycustom/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=holidaycustom-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
...
$help_url = '';
$page_name = "HolidaySetup";

llxHeader('', $langs->trans($page_name), $help_url, '', 0, 0, '', '', '', 'mod-holidaycustom page-admin');
