...
$search_status       = GETPOSTISSET('search_status') ? GETPOST('search_status', 'aZ09comma') : GETPOST('search_statut', 'aZ09comma');
...
// Validator
if (!empty($search_valideur) && $search_valideur != -1 && !empty($search_valideur2) && $search_valideur2 != -1) {
	$sql .= " AND ((cp.fk_validator = '".$db->escape($search_valideur)."'\n";
	$sql .= " OR ha1.fk_user IS NOT NULL)";
	$sql .= " OR (ef.fk_validator2 = '".$db->escape($search_valideur2)."'\n";
...
elseif (!empty($search_valideur2) && $search_valideur2 != -1) {
...
if (!empty($search_status) && $search_status != -1) {
	$sql .= " AND cp.statut IN (".implode(",",$search_status).")";
}
...
if ($resql) {
	...
	if (!empty($arrayfields['cp.fk_validator']['checked'])) {
		//if ($user->rights->holidaycustom->readall || $user->rights->holidaycustom->readall_simple) {
			...
			// }
			print $form->select_dolusers($search_valideur, "search_valideur", 1, "", 0, $include, '', 0, 0, 0, $morefilter, 0, '', 'maxwidth150');
			print '</td>';
		// } else {
	...
	if (!empty($arrayfields['ef.fk_validator2']['checked'])) {
		//if ($user->rights->holidaycustom->readall || $user->rights->holidaycustom->readall_simple) {
			...
			print $form->select_dolusers($search_valideur2, "search_valideur2", 1, "", 0, $include, '', 0, 0, 0, $morefilter, 0, '', 'maxwidth150');
			print '</td>';
		// } else {
		// 	print '<td class="liste_titre">&nbsp;</td>';
		// }
	...
	if (!empty($arrayfields['cp.statut']['checked'])) {
		...
		$value = $object->getArrayStatut();
		print $form->multiselectarray('search_status', $value, $search_status);
		print '</td>';
	}
...
function showMyBalance($holiday, $user_id)
{
	...
	$alltypeleaves = $holiday->getTypesNoCP(1, -1); // To have labels
