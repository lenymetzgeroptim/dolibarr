...
$result = restrictedArea($user, 'holidaycustom', '', '');
...
if ($id > 0) {
	...
	if (!empty($user->rights->holidaycustom->readall) || !empty($user->rights->holidaycustom->readall_simple)) {
		$canread = 1;
	}
	if (!empty($user->rights->holidaycustom->read) && in_array($id, $childids)) {
		$canread = 1;
	}
	if (!$canread) {
		accessforbidden();
...
$search_start1_day	 = GETPOST('search_start1_day', 'int');
$search_start1_month = GETPOST('search_start1_month', 'int');
$search_start1_year	 = GETPOST('search_start1_year', 'int');
$search_start2_day	 = GETPOST('search_start2_day', 'int');
$search_start2_month = GETPOST('search_start2_month', 'int');
$search_start2_year	 = GETPOST('search_start2_year', 'int');
...
$search_end1_day	 = GETPOST('search_end1_day', 'int');
$search_end1_month 	 = GETPOST('search_end1_month', 'int');
$search_end1_year	 = GETPOST('search_end1_year', 'int');
$search_end2_day	 = GETPOST('search_end2_day', 'int');
$search_end2_month   = GETPOST('search_end2_month', 'int');
$search_end2_year	 = GETPOST('search_end2_year', 'int');
$search_employee     = GETPOST('search_employee', 'int');
$search_valideur     = GETPOST('search_valideur', 'int');
$search_valideur2     = GETPOST('search_valideur2', 'int');
...
$arrayfields = array(
	'cp.ref'=>array('label'=>$langs->trans("Ref"), 'checked'=>1, 'disabled'=>1),
...
$arrayfields = array_merge($arrayfields, $arrayfields2);

unset($extrafields->attributes[$extrafieldsobjectkey]['label']['fk_validator2']);
// Extra fields
include DOL_DOCUMENT_ROOT.'/core/tpl/extrafields_list_array_fields.tpl.php';
$extrafields->attributes[$extrafieldsobjectkey]['label'] = $extrafields_save;
...
if (empty($reshook)) {
	...
	$objectclass = 'Holiday';
	$objectlabel = 'Holiday';
	$permissiontoread = $user->rights->holidaycustom->read;
	$permissiontodelete = $user->rights->holidaycustom->delete;
	$permissiontoapprove = $user->rights->holidaycustom->approve;
	$uploaddir = $conf->holidaycustom->dir_output;
...
if($search_valideur2 > 0){
	...
	$sql .= " AND ha2.fk_user = ".$search_valideur2." AND ha2.validation_number = 2";
	$sql .= ")";
}
...
// Start date
$sql .= dolSqlBetweenDateFilter("cp.date_debut", $search_start1_day, $search_start1_month, $search_start1_year, $search_start2_day, $search_start2_month, $search_start2_year);
// End date
$sql .= dolSqlBetweenDateFilter("cp.date_fin",  $search_end1_day, $search_end1_month, $search_end1_year, $search_end2_day, $search_end2_month, $search_end2_year);
// Create date
...
if (!empty($search_valideur2) && $search_valideur2 != -1) {
	$sql .= " AND (ef.fk_validator2 = '".$db->escape($search_valideur2)."'\n";
	$sql .= " OR ha2.fk_user IS NOT NULL)";
...
if (empty($user->rights->holidaycustom->readall) && (empty($user->rights->holidaycustom->readall_simple) || $search)) {
...
if ($resql) {
	...
	if ($search_start1_day) {
		$param .= '&search_start1_day='.urlencode($search_start1_day);
	}
	if ($search_start1_month) {
		$param .= '&search_start1_month='.urlencode($search_start1_month);
	}
	if ($search_start1_year) {
		$param .= '&search_start1_year='.urlencode($search_start1_year);
	}
	if ($search_start2_day) {
		$param .= '&search_start2_day='.urlencode($search_start2_day);
	}
	if ($search_start2_month) {
		$param .= '&search_start2_month='.urlencode($search_start2_month);
	}
	if ($search_start2_year) {
		$param .= '&search_start2_year='.urlencode($search_start2_year);
	}
	if ($search_end1_day) {
		$param .= '&search_end1_day='.urlencode($search_end1_day);
	}
	if ($search_end1_month) {
		$param .= '&search_end1_month='.urlencode($search_end1_month);
	}
	if ($search_year1_year) {
		$param .= '&search_start1_year='.urlencode($search_start1_year);
	}
	if ($search_end2_day) {
		$param .= '&search_end2_day='.urlencode($search_end2_day);
	}
	if ($search_end2_month) {
		$param .= '&search_end2_month='.urlencode($search_end2_month);
	}
	if ($search_year2_year) {
		$param .= '&search_year2_year='.urlencode($search_year2_year);
	}
	...
	// if ($search_fk_user_approbation1 > 0) {
	// 	$param .= '&search_fk_user_approbation1='.urlencode($search_fk_user_approbation1);
	// }
	// if ($search_fk_user_approbation2 > 0) {
	// 	$param .= '&search_fk_user_approbation2='.urlencode($search_fk_user_approbation2);
	// }
	...
	if ($id > 0) {		// For user tab
		...
		if (!empty($user->rights->holidaycustom->write) && in_array($user_id, $childids)) {
			$cancreate = 1;
		}

		if ($cancreate) {
			print '<a href="'.DOL_URL_ROOT.'/custom/holidaycustom/card.php?action=create&fuserid='.$user_id.'" class="butAction">'.$langs->trans("AddCP").'</a>';
		}
	...
	if (!empty($arrayfields['cp.fk_validator']['checked'])) {
		if ($user->rights->holidaycustom->readall || $user->rights->holidaycustom->readall_simple) {
			print '<td class="liste_titre maxwidthonsmartphone left minwidth200">';
	...
	// Approver 2
	if (!empty($arrayfields['ef.fk_validator2']['checked'])) {
		if ($user->rights->holidaycustom->readall || $user->rights->holidaycustom->readall_simple) {
			print '<td class="liste_titre maxwidthonsmartphone left minwidth200">';
			$validator = new UserGroup($db);
			//$excludefilter = $user->admin ? '' : 'u.rowid = '.$user->id;
			...
			print $form->select_dolusers($search_valideur2, "search_valideur2", 1, "", 0, $valideurarray, '', 0, 0, 0, $morefilter, 0, '', 'maxwidth150');
			print '</td>';
		} else {
			print '<td class="liste_titre">&nbsp;</td>';
		}
	...
	if (!empty($arrayfields['cp.date_debut']['checked'])) {
		...
		$date_debut = dol_mktime(0, 0, 0, GETPOST('search_start1_month', 'int'), GETPOST('search_start1_day', 'int'), GETPOST('search_start1_year', 'int'));
		print $form->selectDate($date_debut ? $date_debut : '', "search_start1_", 0, 0, 1, '', 1, 0, 0, '', '', '', '', 1, '', $langs->trans('From'));
		print '</div>';
		print '<div class="nowrap">';
		$date_fin = dol_mktime(23, 59, 59, GETPOST('search_start2_month', 'int'), GETPOST('search_start2_day', 'int'), GETPOST('search_start2_year', 'int'));
		print $form->selectDate($date_fin ? $date_fin : '', "search_start2_", 0, 0, 1, '', 1, 0, 0, '', '', '', '', 1, '', $langs->trans('to'));
		print '</div>';
		print '</td>';
	}
	...
	if (!empty($arrayfields['cp.date_fin']['checked'])) {
		...
		print '<td class="liste_titre center nowraponall">';
		print '<div class="nowrap">';
		$date_debut = dol_mktime(0, 0, 0, GETPOST('search_end1_month', 'int'), GETPOST('search_end1_day', 'int'), GETPOST('search_end1_year', 'int'));
		print $form->selectDate($date_debut ? $date_debut : '', "search_end1_", 0, 0, 1, '', 1, 0, 0, '', '', '', '', 1, '', $langs->trans('From'));
		print '</div>';
		print '<div class="nowrap">';
		$date_fin = dol_mktime(23, 59, 59, GETPOST('search_end2_month', 'int'), GETPOST('search_end2_day', 'int'), GETPOST('search_end2_year', 'int'));
		print $form->selectDate($date_fin ? $date_fin : '', "search_end2_", 0, 0, 1, '', 1, 0, 0, '', '', '', '', 1, '', $langs->trans('to'));
		print '</div>';
		print '</td>';
	}
	...
	if (!empty($arrayfields['ef.fk_validator2']['checked'])) {
		print_liste_field_titre("Approbateur n°2", $_SERVER["PHP_SELF"], "ef.fk_validator2", "", $param, '', $sortfield, $sortorder);
	}
	...
	if ($id && empty($user->rights->holidaycustom->readall) && !in_array($id, $childids)) {
		$langs->load("errors");
		...
		// Lines
		$userstatic = new ExtendedUser2($db);
		...
		while ($i < min($num, $limit)) {
			...
			if (!empty($arrayfields['cp.fk_validator']['checked'])) {
				if (!$user->rights->holidaycustom->readall && $user->rights->holidaycustom->readall_simple && !in_array($userstatic->id, $childids) && !in_array($user->id, $object->listApprover1[0]) && !in_array($user->id, $object->listApprover2[0])) {					
			...
			if (!empty($arrayfields['ef.fk_validator2']['checked'])) {
				if (!$user->rights->holidaycustom->readall && $user->rights->holidaycustom->readall_simple && !in_array($userstatic->id, $childids) && !in_array($user->id, $object->listApprover1[0]) && !in_array($user->id, $object->listApprover2[0])) {					
					...
					foreach($list_validation2[0] as $userid){
						$user_static = $tab_user[$userid];
						print $user_static->getNomUrl(3, "").($listApprover2[1][$userid] == 1 ? ' <i class="fas fa-check" style="color: #00a300;"></i>' : ' <i class="fas fa-times" style="color: red"></i>');
						print '<br>';
					}
					print '</td>';
				}
			...
			unset($extrafields->attributes[$extrafieldsobjectkey]['label']['fk_validator2']);
			// Extra fields
			include DOL_DOCUMENT_ROOT.'/custom/holidaycustom/core/tpl/extrafields_list_print_fields.tpl.php';
			$extrafields->attributes[$extrafieldsobjectkey]['label'] = $extrafields_save;
