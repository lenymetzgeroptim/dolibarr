...
if (empty($user->rights->holidaycustom->read)) {
	accessforbidden();
}
...
$typeleaves = $holiday->getTypesALLCP();
...
if (count($typeleaves) == 0) {
	...
	$canedit = 0;
	if (!empty($user->rights->holidaycustom->define_holiday)) {
		$canedit = 1;
	}
	...
	print_liste_field_titre((empty($user->rights->holidaycustom->define_holiday) ? '' : 'Note'), $_SERVER["PHP_SELF"]);
	...
	foreach ($listUsers as $users) {
		...
		// Button modify
		print '<td>';
		if (!empty($user->rights->holidaycustom->define_holiday)) {	// Allowed to set the balance of any user
