...
if (empty($reshook)) {
	...
	if ($action == 'add') {
		...
		if (!$error) {
			...
			if (empty($duration_hour) && $needHour == 1) {
				...
				foreach($liste_projet as $projetid => $ref) {
					...
					if(!$projectstatic->array_options['options_projetstructurel']) {
						...
						foreach($liste_resp_projet as $userid) {
							if (!in_array($userid, $approver1id)) {
								$approver1id[] = $userid;
							}
						}
						...
						foreach($liste_taches as $taskstatic) {
							// Responsables de tâche
							$liste_resp_tache = $taskstatic->liste_contact(-1, 'internal', 1, 'TASKEXECUTIVE', 1);
		...
		if ($object->statut == Holiday::STATUS_DRAFT) {
			...
			if ($cancreate) {
				...
				if($object->date_debut != $date_debut || $object->date_fin != $date_fin) {
					...
					$userstatic->fetch($fuserid);
					$filter = " AND dateo <= '".$db->idate($date_fin)."' AND (datee >= '".$db->idate($date_debut)."' OR datee IS NULL) AND fk_statut = 1";		
					$liste_projet = $projectstatic->getProjectsAuthorizedForUser($userstatic, 1, 0, 0, $filter); 
					foreach($liste_projet as $projetid => $ref) {
						$projectstatic->fetch($projetid);
						if(!$projectstatic->array_options['options_projetstructurel']) {
							...
							$filter = " AND t.dateo <= '".$db->idate($date_fin)."' AND (t.datee >= '".$db->idate($date_debut)."' OR t.datee IS NULL)";
							$liste_taches = $taskstatic->getTasksArray(0, $userstatic, $projectstatic->id, 0, 0, '',  '-1', $filter, 0, 0, array(),  0,  array(),  0,  1);
							//$projectstatic->getLinesArray($userstatic);
							foreach($liste_taches as $taskstatic) {
								// Responsables de tâche
								$liste_resp_tache = $taskstatic->liste_contact(-1, 'internal', 1, 'TASKEXECUTIVE', 1);
								foreach($liste_resp_tache as $userid) {
				...
				if(!$error && $verif && $object->array_options['options_statutfdt'] > 1) {
					...
					$user_static->fetch($fuserid);

					$urlwithouturlroot = preg_replace('/'.preg_quote(DOL_URL_ROOT, '/').'$/i', '', trim($dolibarr_main_url_root));
					$urlwithroot = $urlwithouturlroot.DOL_URL_ROOT; // This is to use external domain name found into config file
					$link = '<a href="'.$urlwithroot.'/custom/holidaycustom/card.php?id='.$result.'">'.$object->ref.'</a>';
...
if ((empty($id) && empty($ref)) || $action == 'create' || $action == 'add') {
	...
	if ((!in_array($fuserid, $childids) && empty($user->rights->holidaycustom->writeall)) || (in_array($fuserid, $childids) && empty($user->rights->holidaycustom->write))) {
		...
		if (empty($conf->global->HOLIDAY_HIDE_BALANCE)) {
			...
			$typeleaves_CP_ANC_ACQUIS = $object->getTypesCP(1, 'CP_ANC_ACQUIS');
            $typeleaves_CP_ANC_PRIS = $object->getTypesCP(1, 'CP_ANC_PRIS');
			$typeleaves_RTT_ACQUIS = $object->getTypesCP(1, 'RTT_ACQUIS');
			$typeleaves_RTT_PRIS = $object->getTypesCP(1, 'RTT_PRIS');
			$typeleaves_ACP = $object->getTypesCP(1, 'ACP');

			$nb_N1_ACQUIS = $object->getCPforUser($user->id, $typeleaves_CP_N1_ACQUIS['rowid']);
			$nb_N1_ACQUIS = ($nb_N1_ACQUIS ? price2num($nb_N1_ACQUIS) : 0);
			$nb_N1_PRIS = $object->getCPforUser($user->id, $typeleaves_CP_N1_PRIS['rowid']);
			$nb_N1_PRIS = ($nb_N1_PRIS ? price2num($nb_N1_PRIS) : 0);
			$nb_N1_SOLDE = $nb_N1_ACQUIS-$nb_N1_PRIS;

			$nb_N_ACQUIS = $object->getCPforUser($user->id, $typeleaves_CP_N_ACQUIS['rowid']);
			$nb_N_ACQUIS = ($nb_N_ACQUIS ? price2num($nb_N_ACQUIS) : 0);
			$nb_N_PRIS = $object->getCPforUser($user->id, $typeleaves_CP_N_PRIS['rowid']);
			$nb_N_PRIS = ($nb_N_PRIS ? price2num($nb_N_PRIS) : 0);
			$nb_N_SOLDE = $nb_N_ACQUIS-$nb_N_PRIS;
			...
			$nb_ANC_ACQUIS = $object->getCPforUser($user->id, $typeleaves_CP_ANC_ACQUIS['rowid']);
			$nb_ANC_ACQUIS = ($nb_ANC_ACQUIS ? price2num($nb_ANC_ACQUIS) : 0);
			$nb_ANC_PRIS = $object->getCPforUser($user->id, $typeleaves_CP_ANC_PRIS['rowid']);
			$nb_ANC_PRIS = ($nb_ANC_PRIS ? price2num($nb_ANC_PRIS) : 0);
			$nb_ANC_SOLDE = $nb_ANC_ACQUIS-$nb_ANC_PRIS;

			$nb_RTT_ACQUIS = $object->getCPforUser($usertoprocess->id, $typeleaves_RTT_ACQUIS['rowid']);
			$nb_RTT_ACQUIS = ($nb_RTT_ACQUIS ? price2num($nb_RTT_ACQUIS) : 0);
			$nb_RTT_PRIS = $object->getCPforUser($usertoprocess->id, $typeleaves_RTT_PRIS['rowid']);
			$nb_RTT_PRIS = ($nb_RTT_PRIS ? price2num($nb_RTT_PRIS) : 0);
			$nb_RTT_SOLDE = $nb_RTT_ACQUIS-$nb_RTT_PRIS;

			$nb_ACP = $object->getCPforUser($user->id, $typeleaves_ACP['rowid']);
			...
			print '<br><table class="noborder nohover" style="text-align: center; width: 96%; margin: auto; max-width: 1000px;">';
			print '<tr>';
