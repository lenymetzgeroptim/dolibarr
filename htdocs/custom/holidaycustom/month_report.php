...
$langs->loadLangs(array('holiday', 'hrm', "holidaycustom@holidaycustom"));
...
$result = restrictedArea($user, 'holidaycustom', $id, '');
...
$arrayfields = array(
	...
	'ue.matricule'=>array('label'=>$langs->trans('Matricule'), 'checked'=>1),
	...
	'cp.date_create'=>array('label'=>$langs->trans('DateCreateCP'), 'checked'=>1),
	'cp.date_valid'=>array('label'=>$langs->trans('DateValidCP'), 'checked'=>1),
	'cp.statut'=>array('label'=>$langs->trans('Statut'), 'checked'=>1),
);
...
$sql = "SELECT cp.rowid, cp.ref, cp.fk_user, cp.date_debut, cp.date_fin, cp.fk_type, cp.description, cp.halfday, cp.statut, cp.date_valid, cp.date_create";
$sql .= " FROM ".MAIN_DB_PREFIX."holiday cp";
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."user u ON cp.fk_user = u.rowid";
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."user_extrafields ue ON u.rowid = ue.fk_object";
$sql .= " WHERE cp.rowid > 0";
//$sql .= " AND cp.statut = ".Holiday::STATUS_APPROVED2;
$sql .= " AND (";
...
if (!empty($arrayfields['ue.matricule']['checked'])) {
	print '<td class="liste_titre"></td>';
}

// Filter: Type
if (!empty($arrayfields['cp.fk_type']['checked'])) {
	$typeleaves = $holidaystatic->getTypesNoCP(-1, -1);
...
if (!empty($arrayfields['ue.matricule']['checked'])) {
	print_liste_field_titre($arrayfields['ue.matricule']['label'], $_SERVER["PHP_SELF"], 'ue.matricule', '', '', '', $sortfield, $sortorder);
}
if (!empty($arrayfields['cp.fk_type']['checked'])) {
...
if (!empty($arrayfields['cp.date_create']['checked'])) {
	print_liste_field_titre($arrayfields['cp.date_create']['label'], $_SERVER["PHP_SELF"], 'cp.date_create', '', '', '', $sortfield, $sortorder);
}
if (!empty($arrayfields['cp.date_valid']['checked'])) {
	print_liste_field_titre($arrayfields['cp.date_valid']['label'], $_SERVER["PHP_SELF"], 'cp.date_valid', '', '', '', $sortfield, $sortorder);
}
if (!empty($arrayfields['cp.statut']['checked'])) {
	print_liste_field_titre($arrayfields['cp.statut']['label'], $_SERVER["PHP_SELF"], 'cp.statut', '', '', '', $sortfield, $sortorder);
}
print getTitleFieldOfList($selectedfields, 0, $_SERVER["PHP_SELF"], '', '', '', '', $sortfield, $sortorder, 'center maxwidthsearch ')."\n";
print '</tr>';

if ($num == 0) {
	print '<tr><td colspan="15" class="opacitymedium">'.$langs->trans('None').'</td></tr>';
} else {
	while ($obj = $db->fetch_object($resql)) {
		...
		$holidaystatic->statut = $obj->statut;
		...
		if (!empty($arrayfields['ue.matricule']['checked'])) {
			print '<td>'.$user->array_options["options_matricule"].'</td>';
		}
		if (!empty($arrayfields['cp.fk_type']['checked'])) {
		...
		if (!empty($arrayfields['cp.date_create']['checked'])) {
			print '<td class="center">'.dol_print_date($db->jdate($obj->date_create), 'day');
			print '</td>';
		}
		if (!empty($arrayfields['cp.date_valid']['checked'])) {
			print '<td class="center">'.dol_print_date($db->jdate($obj->date_valid), 'day');
			print '</td>';
		}
		if (!empty($arrayfields['cp.statut']['checked'])) {
			print '<td class="center">'.$holidaystatic->getLibStatut(2);
			print '</td>';
		}
