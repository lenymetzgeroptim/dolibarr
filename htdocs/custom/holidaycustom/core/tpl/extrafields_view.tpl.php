	...
	foreach ($extrafields->attributes[$object->table_element]['label'] as $tmpkeyextra => $tmplabelextra) {
		...
		if ($extrafields->attributes[$object->table_element]['type'][$tmpkeyextra] == 'separate') {
			...
			if (($isdraft || !empty($extrafields->attributes[$object->table_element]['alwayseditable'][$tmpkeyextra]))
				&& $permok && ($enabled != 5 || $tmpkeyextra == 'nom_client') && ($action != 'edit_extras' || GETPOST('attribute') != $tmpkeyextra)
			...
			if ($action == 'edit_extras' && $permok && GETPOST('attribute', 'restricthtml') == $tmpkeyextra) {
				...
				if($tmpkeyextra == 'fk_validator2'){
					...
					if (empty($include_users)) {
						...
						$s = $form->select_dolusers($object->array_options['options_fk_validator2'], "options_fk_validator2", (($action == 'editvalidator') ? 0 : 1), $arrayofvalidatorstoexclude, 0, $include_users);
						print $form->textwithpicto($s, $langs->trans("AnyOtherInThisListCanValidate2"));
					}
				}
				else {
				...
				if($tmpkeyextra == 'fk_validator2' && $object->array_options['options_fk_validator2'] > 0){
					$approver2 = new User($db);
					$approver2->fetch($object->array_options['options_fk_validator2']);
					print $approver2->getNomUrl(-1);
					/*$include_users = $object->fetch_users_approver_holiday();
