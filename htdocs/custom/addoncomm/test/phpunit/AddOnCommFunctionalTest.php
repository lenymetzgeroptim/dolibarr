...
/**
 * \file    test/phpunit/AddOnCommFunctionalTest.php
 * \ingroup addoncomm
 ...
 *
 * @package Testaddoncomm
 */
class AddOnCommFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modAddOnComm')->click();
		} else {
			// Disable the module
			$this->byHref('modAddOnComm')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/addoncomm/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('addoncomm/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/addoncomm/admin/about.php');
		$this->authenticate();
		return $this->assertContains('addoncomm/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('addoncommwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modAddOnComm_AddOnCommTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modAddOnComm_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
