...
function switchChartType(newChartType) {
  if (currentChartType === newChartType || isLoading) {
    console.log(`Le graphique est déjà de type ${newChartType} ou un chargement est en cours.`);
    return;
  }

  isLoading = true; // Bloquer les nouveaux appels
  currentChartType = newChartType; // Mettre à jour le type de graphique actuel
  updateButtonIcons(newChartType); // Mettre à jour les icônes

  const fetchFunction = newChartType === 'line' ? fetchDataEvol : fetchData;
  fetchFunction((data) => {
    createChartType(data, newChartType);
    isLoading = false; // Débloquer les nouveaux appels une fois le chargement terminé
  });
}
...
function createChartType(data, type) {
  ...
  // la dernière sélection (année ou plage de dates)
  ...
  // Fonction pour gérer la mise à jour de la sélection et du champ de date
  ...
  // Fonction pour gérer la mise à jour de l'année et les dates associées
  function handleYearChange() {
      ...
      dateFinInput.value = (yearSelector.value === new Date().getFullYear().toString())
        ...
        : `31/12/${yearSelector.value}`; // Fin d'année si différente de l'année actuelle
  ...
  dateFinInput.removeEventListener('change', handleDateChange);
...
function updateDisplayEvol(data) {
  ...
  const groupedData = data.reduce((acc, item) => {
      ...
      keys.forEach(agenceKey => {
          ...
          // Ajouter le montant au bon type (CA ou Dépenses)
          if (type === "CA") {
              acc[agence].montant1 += parseFloat(item[fullString]) || 0;
          } else if (type === "Dépenses") {
              acc[agence].montant2 += parseFloat(item[fullString]) || 0;
          }
      });
  ...
  success: function (response) {
    ...
    // Charger les données selon le type de graphique
      ...
      if (response.dataFianance) {
        ...
        updateDisplayEvol(response.dataFiananceEvol);
      }
