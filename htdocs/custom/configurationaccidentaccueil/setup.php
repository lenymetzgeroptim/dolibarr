...
// Translations
$langs->loadLangs(array("admin", "configurationaccidentaccueil@configurationaccidentaccueil"));
...
$arrayofparameters = array(
	'JOUR_SANS_ACC_AVEC_ARRET'=>array('type'=>'string','enabled'=>1),
	'JOUR_SANS_ACC_AVEC_ARRET_COM'=>array('type'=>'string','enabled'=>1),
	'JOUR_SANS_ACC_SANS_ARRET'=>array('type'=>'string','enabled'=>1),
	'JOUR_SANS_ACC_SANS_ARRET_COM'=>array('type'=>'string','enabled'=>1),
	'JOUR_SANS_ACC_TRAJET'=>array('type'=>'string','enabled'=>1),
	'JOUR_SANS_ACC_TRAJET_COM'=>array('type'=>'string','enabled'=>1),
	//'SITUATIONS_DANGEREUSES'=>array('type'=>'string','enabled'=>1),
	//'REMONTEES_MOIS'=>array('type'=>'textarea','enabled'=>1),
	'DATE_MAJ'=>array('type'=>'string','enabled'=>1),
	'REMONTEES_SSE_VDRNORD'=>array('type'=>'textarea','enabled'=>1),
	'REMONTEES_SSE_GRANDOUEST'=>array('type'=>'textarea','enabled'=>1),
	'REMONTEES_SSE_SUDEST'=>array('type'=>'textarea','enabled'=>1),
);
...
if (empty($conf->configurationaccidentaccueil->enabled)) accessforbidden();
if (empty($user->rights->configurationaccidentaccueil->modify)) accessforbidden();
...
if ((float) DOL_VERSION >= 6) {
	include DOL_DOCUMENT_ROOT.'/custom/configurationaccidentaccueil/core/actions_setmoduleoptions.inc.php';
}
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/configurationaccidentaccueil/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'CONFIGURATIONACCIDENTACCUEIL_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'CONFIGURATIONACCIDENTACCUEIL_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
if ($id == $user->id){
	...
	$help_url = '';
	$page_name = "Configuration QSE Accueil";
	...
	print dol_get_fiche_head($head, 'conf_accident_accueil', $title, -1, 'user');
	...
	if ($action == 'edit') {
		...
		if (!empty($arrayofparameters)) {
			print '<table class="noborder centpercent">';
			print '<tr class="liste_titre"><td class="titlefield" style="width: 30%">'.$langs->trans("Parameter").'</td><td>'.$langs->trans("Value").'</td></tr>';
						...
						while (($file = readdir($handle)) !== false) {
							if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
								...
								if ($module->isEnabled()) {
									...
									$constforvar = 'CONFIGURATIONACCIDENTACCUEIL_'.strtoupper($myTmpObjectKey).'_ADDON';
