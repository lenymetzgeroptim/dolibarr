...
/**
 * \file    addonusergroup/admin/setup.php
 * \ingroup addonusergroup
 * \brief   AddOnUserGroup setup page.
 */
...
require_once '../lib/addonusergroup.lib.php';

// Translations
$langs->loadLangs(array("admin", "addonusergroup@addonusergroup"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('addonusergroupsetup', 'globalsetup'));
...
$item = $formSetup->newItem('EMailTextAddUserGroupRSR');
$item->setAsHtml();
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/addonusergroup/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=addonusergroup-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'ADDONUSERGROUP_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'ADDONUSERGROUP_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$head = addonusergroupAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "AddOnUserGroup_32@addonusergroup");

// Setup page goes here
//echo '<span class="opacitymedium">'.$langs->trans("AddOnUserGroupSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'ADDONUSERGROUP_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'ADDONUSERGROUP_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
