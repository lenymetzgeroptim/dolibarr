...
/**
 * \file    test/phpunit/RgpdFunctionalTest.php
 * \ingroup rgpd
 ...
 * @package Testrgpd
 */
class RgpdFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	private static $module_id = 500031; // TODO: autodetect?
	...
	{
		$this->url('/custom/rgpd/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('rgpd/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/rgpd/admin/about.php');
		$this->authenticate();
		return $this->assertContains('rgpd/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('rgpdwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modRgpd_RgpdTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modRgpd_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
