...
/**
 * \file    rgpd/admin/setup.php
 * \ingroup rgpd
 * \brief   Rgpd setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "rgpd@rgpd"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('rgpdsetup', 'globalsetup'));
...
$item = $formSetup->newItem('RGPD_PARAM');
$item->setAsHtml();

$item = $formSetup->newItem('DROIT_PARAM');
$item->setAsHtml();
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/rgpd/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=rgpd-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'RGPD_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'RGPD_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "fa-user-lock_fa_#61c2ff");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("RgpdSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'RGPD_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'RGPD_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
