...
//var_dump($extended_extrafields->attributes[$table_element]);
if (empty($reshook) && isset($extended_extrafields->attributes[$table_element]['label']) && is_array($extended_extrafields->attributes[$table_element]['label'])) {
	$lastseparatorkeyfound = '';
	$extended_extrafields_collapse_num = '';
	$extended_extrafields_collapse_num_old = '';
	$i = 0;
	...
	foreach ($extended_extrafields->attributes[$table_element]['label'] as $tmpkeyextra => $tmplabelextra) {
		...
		$enabled = 1;
		if ($enabled && isset($extended_extrafields->attributes[$table_element]['enabled'][$tmpkeyextra])) {
			$enabled = dol_eval($extended_extrafields->attributes[$table_element]['enabled'][$tmpkeyextra], 1, 1, '2');
		}
		if ($enabled && isset($extended_extrafields->attributes[$table_element]['list'][$tmpkeyextra])) {
			$enabled = dol_eval($extended_extrafields->attributes[$table_element]['list'][$tmpkeyextra], 1, 1, '2');
		}

		$perms = 1;
		if ($perms && isset($extended_extrafields->attributes[$table_element]['perms'][$tmpkeyextra])) {
			$perms = dol_eval($extended_extrafields->attributes[$table_element]['perms'][$tmpkeyextra], 1, 1, '2');
		}
		...
		if (!empty($extended_extrafields->attributes[$table_element]['langfile'][$tmpkeyextra])) {
			$langs->load($extended_extrafields->attributes[$table_element]['langfile'][$tmpkeyextra]);
		}
		...
		if ($extended_extrafields->attributes[$table_element]['type'][$tmpkeyextra] == 'separate') {
			$extended_extrafields_collapse_num = $tmpkeyextra;

			print $extended_extrafields->showSeparator($tmpkeyextra, $userField);

			$lastseparatorkeyfound = $tmpkeyextra;
		} else {
			$collapse_group = $extended_extrafields_collapse_num.(!empty($userField->id) ? '_'.$userField->id : '');
			print '<tr class="trextrafields_collapse'.$collapse_group;
			/*if ($extended_extrafields_collapse_num && $extended_extrafields_collapse_num_old && $extended_extrafields_collapse_num != $extended_extrafields_collapse_num_old) {
				print ' trextrafields_collapse_new';
			}*/
			if ($extended_extrafields_collapse_num && $i == count($extended_extrafields->attributes[$table_element]['label'])) {
				print ' trextrafields_collapse_last';
			}
			print '"';
			if (isset($extended_extrafields->expand_display) && empty($extended_extrafields->expand_display[$collapse_group])) {
				print ' style="display: none;"';
			}
			print '>';
			$extended_extrafields_collapse_num_old = $extended_extrafields_collapse_num;
			print '<td class="titlefield">';
			print '<table class="nobordernopadding centpercent">';
			print '<tr>';

			print '<td class="';
			if ((!empty($action) && ($action == 'create' || $action == 'edit')) && !empty($extended_extrafields->attributes[$table_element]['required'][$tmpkeyextra])) {
				print ' fieldrequired';
			}
			print '">';
			if (!empty($extended_extrafields->attributes[$table_element]['help'][$tmpkeyextra])) {
				// You can also use 'TranslationString:keyfortooltiponlick' for a tooltip on click.
				$tmptooltip = explode(':', $extended_extrafields->attributes[$table_element]['help'][$tmpkeyextra]);
			...
			if (($isdraft || !empty($extended_extrafields->attributes[$table_element]['alwayseditable'][$tmpkeyextra]))
				&& $permok && $enabled != 5 && ($action != 'edit_extras' || GETPOST('attribute') != $tmpkeyextra)
				&& empty($extended_extrafields->attributes[$table_element]['computed'][$tmpkeyextra])) {
			...
			print '<td id="'.$html_id.'" class="valuefield '.$userField->element.'_extras_'.$tmpkeyextra.' wordbreak '.$extended_extrafields->attributes[$table_element]['cssview'][$tmplabelextra].'"'.(!empty($cols) ? ' colspan="'.$cols.'"' : '').'>';

			// Convert date into timestamp format
			if (in_array($extended_extrafields->attributes[$table_element]['type'][$tmpkeyextra], array('date'))) {
			...
			if (in_array($extended_extrafields->attributes[$table_element]['type'][$tmpkeyextra], array('datetime'))) {
				$datenotinstring = empty($userField->array_options['options_'.$tmpkeyextra]) ? '' : $userField->array_options['options_'.$tmpkeyextra];
				// print 'X'.$userField->array_options['options_' . $tmpkeyextra].'-'.$datenotinstring.'x';
				if (!empty($userField->array_options['options_'.$tmpkeyextra]) && !is_numeric($userField->array_options['options_'.$tmpkeyextra])) {	// For backward compatibility
					$datenotinstring = $db->jdate($datenotinstring);
			...
			if ($action == 'edit_extras' && $permok && GETPOST('attribute', 'restricthtml') == $tmpkeyextra) {
				...
				print $extended_extrafields->showInputField($tmpkeyextra, $value, '', '', '', 0, $userField->id, $table_element);
				...
				print $extended_extrafields->showOutputField($tmpkeyextra, $value, '', $table_element);
			}
