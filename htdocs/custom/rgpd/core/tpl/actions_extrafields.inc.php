	...
	if (GETPOST("button") != $langs->trans("Cancel")) {
		...
		if (!$error) {
			...
			if (GETPOSTISSET("attrname") && preg_match("/^[a-z0-9_]+$/", GETPOST('attrname', 'aZ09')) && !is_numeric(GETPOST('attrname', 'aZ09'))) {
				...
				$result = $extrafields->addExtraField_custom(
				...
				if($result > 0) {
					$sql = "SELECT rowid FROM ".MAIN_DB_PREFIX."extrafields WHERE name = '".GETPOST('attrname', 'aZ09')."'";
					...
					if($result > 0) {
						...
						$result = $object->create_adminrgpd_extrafield($obj->rowid, $object->id);
					}
				}
		...
		if (!$error) {
			if (GETPOSTISSET("attrname") && preg_match("/^\w[a-zA-Z0-9-_]*$/", GETPOST('attrname', 'aZ09')) && !is_numeric(GETPOST('attrname', 'aZ09'))) {
				...
				if($result > 0) {
					...
					$result = $object->delete_adminrgpd_extrafield($obj->rowid, $object->id);
				}
				...
				if($result > 0) {
					...
					if($result > 0) {
						...
						$result = $object->create_adminrgpd_extrafield($obj->rowid, GETPOST('adminrgpd', 'integer'));
					}
				}
