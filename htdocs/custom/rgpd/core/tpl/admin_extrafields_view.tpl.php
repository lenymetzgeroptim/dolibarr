...
$newcardbutton = dolGetButtonTitle($langs->trans('NewAttribute'), '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?action=create_extrafield&id='.$object->id, '', $permissiontoaddfield);
/*} else {
	$newcardbutton = '';
}*/
...
$extrafields->fetch_name_optionals_label($elementtype, false, $object->id);
...
if (isset($extrafields->attributes[$elementtype]['type']) && is_array($extrafields->attributes[$elementtype]['type']) && count($extrafields->attributes[$elementtype]['type'])) {
	foreach ($extrafields->attributes[$elementtype]['type'] as $key => $value) {
		...
		if($permissiontoaddfield) {
			print '<a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=edit_extrafield&id='.$object->id.'&token='.newToken().'&attrname='.urlencode($key).'#formeditextrafield">'.img_edit().'</a>';
		}
		if($permissiontodeletefield) {
			print '&nbsp; <a class="paddingleft" href="'.$_SERVER["PHP_SELF"].'?action=delete_extrafield&id='.$object->id.'&token='.newToken().'&attrname='.urlencode($key).'">'.img_delete().'</a>';
		}
		print '</td>'."\n";
		print "</tr>";
	}
