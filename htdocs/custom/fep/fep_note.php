...
$langs->loadLangs(array("fep@fep", "companies"));
...
$hookmanager->initHooks(array('fepnote', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->fep->multidir_output[$object->entity]."/".$object->id;
}

$permissionnote = $user->rights->fep->fep->write; // Used by the include of actions_setnotes.inc.php
$permissiontoadd = $user->rights->fep->fep->write; // Used by the include of actions_addupdatedelete.inc.php
...
$help_url = '';
llxHeader('', $langs->trans('FEP'), $help_url);
