...
/**
 * \file    gpec/admin/setup.php
 * \ingroup gpec
 * \brief   GPEC setup page.
 */
...
require_once '../lib/gpec.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "gpec@gpec"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('gpecsetup', 'globalsetup'));
...
$arrayofparameters = array(
	'GPEC_MYPARAM1'=>array('type'=>'string', 'css'=>'minwidth500' ,'enabled'=>1),
	'GPEC_MYPARAM2'=>array('type'=>'textarea','enabled'=>1),
	//'GPEC_MYPARAM3'=>array('type'=>'category:'.Categorie::TYPE_CUSTOMER, 'enabled'=>1),
	//'GPEC_MYPARAM4'=>array('type'=>'emailtemplate:thirdparty', 'enabled'=>1),
	//'GPEC_MYPARAM5'=>array('type'=>'yesno', 'enabled'=>1),
	//'GPEC_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
	//'GPEC_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
	//'GPEC_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
if ($useFormSetup && (float) DOL_VERSION >= 15) {
	...
	// Setup conf GPEC_MYPARAM1 as a simple string input
	$item = $formSetup->newItem('GPEC_MYPARAM1');

	// Setup conf GPEC_MYPARAM1 as a simple textarea input but we replace the text of field title
	$item = $formSetup->newItem('GPEC_MYPARAM2');
	$item->nameText = $item->getNameText().' more html text ';

	// Setup conf GPEC_MYPARAM3
	...
	$formSetup->newItem('GPEC_MYPARAM5')->setAsEmailTemplate('thirdparty');

	// Setup conf GPEC_MYPARAM6
	$formSetup->newItem('GPEC_MYPARAM6')->setAsSecureKey()->enabled = 0; // disabled

	// Setup conf GPEC_MYPARAM7
	$formSetup->newItem('GPEC_MYPARAM7')->setAsProduct();
	*/
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/gpec/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=gpec-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'GPEC_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'GPEC_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$head = gpecAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "gpec@gpec");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("GPECSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'GPEC_'.strtoupper($myTmpObjectKey).'_ADDON';
